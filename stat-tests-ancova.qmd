...
Wir haben zwei Typen von Daten. Einmal messen wir den Durchmesser für jede Sorte (`D` im Namen der Versuchsgruppe) oder aber die Länge (`L` im Namen der Versuchsgruppe). Wir betrachten hier nur das Längenwachstum und deshalb filtern wir erstmal nach allen Versuchsgruppen mit einem `L` im Namen. Dann müssen wir die Daten noch in Long-Format bringen. Da wir dann auch noch auf zwei Arten die Daten über die Zeit darstellen wollen, brauchen wir einmal die Zeit als Faktor `time_fct` und einmal als numerisch `time_num`. Wir haben auch Gurken mit einer Länge von 0 cm. Diese Gurken schmeißen wir am Ende mal raus. Auch haben wir ab Woche 14 keine Messungen mehr an der Versuchsgruppe `Prolong`, also nehmen wir auch nur die Daten bis zur vierzehnten Woche.
...
In der @fig-ancova-gurke-03 sehen wir die Daten einmal als Scatterplot. Die Gerade stellt den Verlauf der Mittelwerte über die Versuchsgruppen dar.
...
Im Folgenden schauen wir uns dann die Auswertung der Gurkendaten einmal genauer an.

## Linearer Trendtest

![](images/caution.png){fig-align="center" width="50%"}

Für mehr Informationen bitte einmal in das Kapitel @sec-posthoc schauen.

https://yuzar-blog.netlify.app/posts/2022-12-29-emmeans2interactions/

```{r}
#| message: false
#| warning: false

time_fct_fit <- lm(length ~ versuchsgruppe + time_fct + versuchsgruppe:time_fct, gurke_time_len_tbl)
```
...
```{r}
emtrends(time_num_poly_fit, pairwise ~ versuchsgruppe, var = "time_num", infer = c(TRUE),
         max.degree = 3)
```
