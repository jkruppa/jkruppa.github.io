...
Im Folgenden selektieren mit der Funktion `select()` die beiden Spalten `jump_length` als $y$ und die Spalte `animal` als $x$. Danach müssen wir noch die Variable `animal` in einen Faktor mit der Funktion `as_factor()` umwandeln. Wir nehmen in diesem Beispiel an, dass die Bvariable `jump_length` nicht normalverteilt ist.

Bindungen (eng. *ties*) müssen beachtet werden.
...
#| message: false
#| echo: false
#| tbl-cap: Selektierter Datensatz für die einfaktorielle ANOVA mit einer nicht-normalverteilten Variable `jump_length` und einem Faktor `animal` mit drei Leveln.
...
Bevor wir jetzt das Modell verwenden, müssen wir uns nochmal überlegen, welchen Schluß wir eigentlich über die Nullhypothese machen. Wir immer können wir nur die Nullhypothese ablehnen. Daher überlegen wir uns im Folgenden wie die Nullhypothese in dem Kruskal-Wallis-Test aussieht. Dann bilden wir anhand der Nullhypothese noch die Alternativehypothese.
