...
In der @fig-mermaid-lsd sehen wir nochmal den schematischen Aufbau eines *latin suare design* dargestellt. VErmutlich hilft es auch sich einmal das finale Design in der @fig-exp-basic-5 einmal anzuschauen. Wichtig ist, dass wir den Block gar nicht betrachten. Wir haben hier eher fünfunzwanzig kleine Subblöcke, die wir durch die Position der `rows` und `cols` definieren.
...
flowchart LR
    ...
    B[("G_10 
        G_4
        G_8")]:::level -.- G(((nested))) -.-> L(inc_block):::factor
    C[("G_3
        G_15
...
-   dem Faktor Block `block` und den vier Leveln `I` bis `III`.
