...
```{r echo = TRUE}
#| message: false
pacman::p_load(tidyverse, magrittr, conflicted, see, 
               effectsize, parameters, lsmeans, broom)
...
-   $Y_n$ Anzahl in der Behandlung bzw. der Anteil, der in der Behandlung lebt
-   $X_n$ Anzahl in der Kontrolle bzw. der Anteil, der in der Kontrolle lebt
...
ggplot(mite_tbl, aes(sorte, count, color = block)) +
  theme_bw() +
  geom_point(size = 3) +
  scale_color_okabeito()
  
ggplot(mite_tbl, aes(sorte, log(count), color = block)) +
  theme_bw() +
  geom_point(size = 3) +
  scale_color_okabeito() 
...
fit %>% 
  model_parameters(exponentiate = FALSE)

fit %>% lsmeans(~variante, type = "response")

fit %>% 
  lsmeans(~variante, type = "response") %>% 
  tidy() %>% 
  mutate(WG_Abbott = (estimate[1] - estimate)/estimate[1])
