...
z_mean <- (z_dog + z_cat)/2
z_mean

sum((z_tbl$dog_abs - z_dog)^2)
sum((z_tbl$cat_abs - z_cat)^2)

```

$$
L=\cfrac
{\cfrac{1}{k-1}\sum_{j=1}^{k} n_j (\bar{Z}_{i.} - \bar{Z}_{..})^2}
{\cfrac {1}{n-k}\sum_{j=1}^k \sum_{i=1}^{n_j} (Z_{ij}-\bar{Z}_{i.})^2} 
$$

$$
L = 
\cfrac
{\cfrac{1}{2-1} \cdot \left(7 \cdot (1.57 - 1.92)^2 + 7 \cdot (2.28 - 1.92)^2\right)}
{\cfrac {1}{14-2} \cdot (10.39 + 11.43)} =
\cfrac{1.76}{1.82}
$$
...
$$
Z_{ij} = \begin{cases}
|Y_{ij} - \bar{Y}_{i\cdot}|, & \bar{Y}_{i\cdot} \text{ is a mean of the } i\text{-th group}, \\
|Y_{ij} - \tilde{Y}_{i\cdot}|, & \tilde{Y}_{i\cdot} \text{ is a median of the } i\text{-th group}. \end{cases} 
$$
...
{\cfrac{1}{k-1}\sum_{j=1}^{k} n_j (\bar{Z}_{i.} - \bar{Z}_{..})^2}
{\cfrac {1}{n-k}\sum_{j=1}^k \sum_{i=1}^{n_j} (Z_{ij}-\bar{Z}_{i.})^2} = 
\cfrac{\cfrac{SS_{trt}}{k-1}}{\cfrac {SS_{error}}{n-k}} =
\cfrac{MS_{trt}}{MS_{error}}
$$
...
```{r}
leveneTest(jump_length ~ animal, data = fac1_tbl, center = "mean")
