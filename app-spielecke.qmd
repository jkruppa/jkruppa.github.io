...
## Anwendungsbeispiel: Kopfgewicht von Brokoli

```{r}
broc_tbl <- read_excel("data/broccoli_weight.xlsx")

broc_tbl %>% 
  mutate(trt = str_c(fert_time, fert_amount, block)) %>% 
  pull(trt) %>% 
  table
...
fac1_tbl <- read_excel("data/flea_dog_cat_fox.xlsx") %>% 
...
n_rep <-  5
pois_tbl <- tibble(animal = gl(3, n_rep, labels = c("cat", "dog", "fox")),
                   count = round(c(rnorm(n_rep, 20, 1), rnorm(n_rep, 30, 1), rnorm(n_rep, 10, 1))))
...
```{r}
glm(count ~ animal, data = pois_tbl, family = "poisson") %>% 
...
```{r}
n_rep <-  20000
infected_tbl <- tibble(animal = gl(3, n_rep, labels = c("cat", "dog", "fox")),
                       infected = round(c(rbinom(n_rep, 1, 0.8), 
                                          rbinom(n_rep, 1, 0.5), 
                                          rbinom(n_rep, 1, 0.2))))
...
```{r}
glm(infected ~ animal, data = infected_tbl, family = "binomial") %>% 
  emmeans(~ animal, regrid = "response") 
```

$cat/dog = (0.8/0.2)/(0.5/0.5) = 4$

$cat/fox = (0.8/0.2)/(0.2/0.8) = 16$
...
```{r}
glm(infected ~ animal, data = infected_tbl, family = "poisson") %>% 
  emmeans(~ animal, type = "response") %>% 
  pairs()
```
