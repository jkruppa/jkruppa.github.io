...
## Interpretation kontinuierliches $x$
...
## Interpretation kategorielles $x$ (2 Level)
...
model_tbl %>% 
  ...
  set_names(c("animal", "mean of level", "difference to male", "sum of difference")) %>% 
  kable(align = "c", "pipe")
```
...
ggplot(model_tbl, aes(x = sex, y = jump_length, color = sex)) +
  ...
  scale_y_continuous(breaks = c(15, 18.06, 20, 18.06 + 4.90, 25, 30)) +
  geom_abline(intercept = 18.06-4.90, slope = 4.90)
...
model_tbl %>% 
  ...
  mutate(diff = c(0, diff(mean)),
         sum = cumsum(diff)) %>% 
  mutate(across(where(is.numeric), round, 2)) %>% 
  set_names(c("animal", "mean of level", "difference to cat", "sum of difference")) %>% 
  kable(align = "c", "pipe")
```
...
ggplot(model_tbl, aes(x = animal, y = jump_length, color = animal)) +
  ...
  scale_color_okabeito() +
  theme(legend.position = "none") +
  scale_y_continuous(breaks = c(15, 17.91, 17.91 + 2.61, 17.91 + 5.19, 25, 30)) +
  geom_abline(intercept = 17.91-2.61, slope = 2.61) +
  annotate("text", x = 1.5, y = 29, label = "jump_length == 17.91 + 2.61 %.% animal", parse = TRUE,
           hjust = 0.5)
