...
Wir wollen uns jetzt die verschiedenen Verteilungen einmal in der Anwendung anschauen. Wichtig ist nur, dass du weißt, das Parameter Zahlen sind, die eine Verteilungskurve beschreiben. Wir sprechen dabei in der Statistik auch von Verteilungs*familien*. Daher schreiben wir in R auch `family = gaussian`, wenn wir sagen wollen, dass unsere Daten einer Normalverteilung entstammen. Es gibt nämlich nicht nur die eine Normalverteilung sondern eben ganz viele verschiedene. Jede dieser Verteilungen hat dann nämlich andere Parameter - im Falle der Normalverteilung Mittelwert und Standardabweichung. Dabei lassen wir viel Mathematik recht und links liegen. Du kannst bei @dormann2013parametrische mehr zu dem Thema statistische Verteilungen anlesen. Dort gibt es auch nochmal mehr Informationen zu den einzelnen Eigenschaften, die eine Verteilung noch so haben kann. Wir konzentrieren uns hier auf die Lageparameter wie den Mittelwert für die Normalverteilung und die Streuung wie der Varianz von Verteilungen.
...
#| fig-height: 5
#| fig-width: 10
#| fig-cap: "Histogramm verschiedener Normalverteilungen mit unterschiedlichen Mittelwerten. **(A)** Drei Normalverteilungen mit Varianzhomogenität. **(B)** Drei Normalverteilungen unter Varianzheterogenität. *[Zum Vergrößern anklicken]*"
...
p1 <- ggplot(data = norm_tbl, aes(x = value, fill = key)) +
  theme_minimal() +
...
p2 <- ggplot(data = norm_tbl, aes(x = value, fill = key)) +
  theme_minimal() +
...
#| fig-width: 10
#| label: fig-normal-01
#| fig-cap: "Darstellung der Körpergröße in [cm] für die Geschlechter getrennt. Die Farben repräsentieren die jeweiligen Geschlechter. Die Männer sind blau und die Frauen in lila dargestellt. **(A)** Histogramm. **(B)** Densityplot. *[Zum Vergrößern anklicken]*"
...
p1 <- ggplot(data = gummi_1_tbl, aes(x = height, fill = gender)) +
  theme_minimal() +
...
p2 <- ggplot(data = gummi_1_tbl, aes(x = height, fill = gender)) +
  theme_minimal() +
...
Betrachten wir einmal die Mittelwerte und die Standardabweichung von Männern und Frauen in einer Tabelle. Wir sehen hier nochmal die gleichen Zahlen vom Mittelwert wie schon in der obigen Abbildung. Ich habe hier nochmal die Standartdabweichung ergänzt damit wir dann auch einen Test händisch rechnen könnten.

```{r}
#| message: false
#| echo: false
#| tbl-cap: "Die Mittelwerte und die Standardabweichungen der Körpergröße in [cm] getrennt nach Männern und Frauen aus den Gummibärchendaten."
#| label: tbl-data-height

stat_tbl <- gummi_1_tbl |> 
  mutate(gender = factor(gender, level = c("w", "m"), labels = c("weiblich", "männlich"))) |> 
  group_by(gender) |> 
  summarise(mean = round(mean(height), 1),
            sd = round(sd(height), 1))

stat_tbl |> 
  magrittr::set_names(c("Geschlecht", "Mittelwert", "Standardabweichung")) |> 
  kable(align = "c", "pipe")
```

Jetzt können uns schon das Säulendiagramm der Körpergrößen getrennt nach Männern und Frauen einmal anschauen. Ich habe erst die Körpergröße der Frauen als Säule dargestellt, damit die Ordnung wie oben im Histogramm erhalten beleibt.
...
#| fig-height: 3.5
#| fig-width: 3
#| fig-cap: "Säulendigramm der Körpergröße in [cm] von Männern und Frauen in dem Gummibärchendatensatz. *[Zum Vergrößern anklicken]*"

ggplot(stat_tbl, aes(x = gender, y = mean, fill = gender)) + 
  theme_minimal() +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = mean-sd, ymax = mean+sd),
                width = 0.2) +
  scale_fill_okabeito(order = c(7, 2), 
                      labels = c("männlich", "weiblich")) +
  theme(legend.position = "none",
        panel.grid.minor = element_blank()) +
  labs(x = "", y = "Mittlere Körpergröße in [cm]", fill = "Geschlecht") +
  scale_y_continuous(breaks = c(0, 50, 100, 150, 200, 
                                round(stat_tbl$mean[1]), round(stat_tbl$mean[2])))
```
...
```{r}
t.test(height ~ gender, data = gummi_tbl, var.equal = TRUE)
```
...
Wir können auch aus unseren Gummibärchendaten für die Körpergröße in \[cm\] jeweils den Mittelwert und die Standardabweichung getrennt für die Geschlechter berechnen und dann die theoretische Normalverteilung zeichnen. In @fig-normal-03 (B) und @fig-normal-03 (D) sehen wir die Verteilung der theoretischen Werte, wenn wir die Mittelwerte und die Standardabweichung aus den Verteilungen in @fig-normal-03 (A) schätzen. Spannenderweise bildet sich den *zufällig* gezogenen Daten auch eine leichte Schulter bei der Verteilung der Körpergrößen. Auch $n = `r nrow(gummi_1_tbl)`$ vollständige Beobachtungen bedeuten nicht, dass wir eine perfekte Normalverteilung erhalten.
...
p2 <- ggplot(data = gummi_theo_tbl, aes(x = height, fill = gender)) +
  theme_minimal() +
  geom_histogram(position = "dodge", color = "black") +
  labs(x = "", y = "", fill = "Geschlecht") +
  scale_fill_okabeito(order = c(2, 7), 
...
p3 <- ggplot(data = gummi_1_tbl, aes(x = height, fill = gender)) +
  theme_minimal() +
  geom_density(alpha = 0.75) +
  labs(x = "", y = "", fill = "Geschlecht") +
  scale_fill_okabeito(order = c(2, 7), 
...
p4 <- ggplot(data = gummi_theo_tbl, aes(x = height, fill = gender)) +
  theme_minimal() +
  geom_density(alpha = 0.75) +
  labs(x = "", y = "", fill = "Geschlecht") +
  scale_fill_okabeito(order = c(2, 7), 
...
#| fig-height: 5
#| fig-width: 12
#| fig-cap: "Der Beeswarm ist ein Dotplot für eine große Anzahl an Beobachtungen. Hier schauen wir uns einmal das Alter und die Körpergröße aufgeteilt nach Geschlecht an. Bei sehr vielen Beobachtungen kommt dann auch ein Bienenschwarm an die Grenze. **(A)** Alter nach Geschlecht. **(B)** Körpergröße nach Geschlecht. *[Zum Vergrößern anklicken]*"


p1 <- na.omit(gummi_tbl) |> 
...
gummi_3_tbl <- gummi_tbl |> 
  ...
  mutate(module = factor(module, 
                         labels = c("Bioinformatik", "Statistik für\n Bioverfahrenstechnik", 
                                    ...
                                    "Statistik", "Girl's Day", "PhD Borstel", "PhD Osnabrück", "Modellierung landwirtschaftlicher Daten", "Angewandte Statistik" ))) |> 
...
#| fig-height: 4.5
#| fig-width: 7
#| fig-cap: "Darstellung von dem Zusammenhang von `pnorm(q = 1.96)` und `qnorm(p = 0.025)`. Mit der Option `lower.tail` bestimmen wir auf welche Seite der Verteilung wir sein wollen. *[Zum Vergrößern anklicken]*"
ggplot(data.frame(x = c(-3.25, 3.25)), aes(x)) +
  theme_minimal() +
  #geom_vline(xintercept = 0) +
  geom_segment(x = 0, y = -0.1, xend = 0, yend = 0.4) +
  stat_function(fun = dnorm, xlim = c(-3.25, 1.96),
                geom = "area", fill = "#E69F00", alpha = 0.25) +
  stat_function(fun = dnorm, xlim = c(1.96, 3.25),
                geom = "area", fill = "#CC79A7", alpha = 0.5) +
  geom_segment(x = 1.96, y = 0, xend = 1.96, yend = 0.06, color = "#CC79A7",
               linewidth = 1) +
  stat_function(fun = dnorm, linewidth = 1) + 
  scale_x_continuous(breaks = c(0, 1.96), 
                     labels = c(expression(0),
                                expression(1.96))) +
  annotate("text", x = 0, y = 0.2, label = "97.5%", size = 14,
           color = "#E69F00") +
  annotate("text", x = 2.5, y = 0.15, label = expression("2.5%"), size = 6,
           color = "#CC79A7") +
  geom_curve(x = 2.5, y = 0.135, xend = 2.2, yend = 0.015,
  ...
  ylim(NA, 0.52) +
  ## left
  geom_segment(x = -3.25, y = 0.45, xend = 1.93, yend = 0.45, color = "#0072B2",
               linewidth = 0.4) +
  geom_segment(x = -3.25, y = 0.43, xend = -3.25, yend = 0.45, color = "#0072B2",
               linewidth = 0.25) +
  geom_segment(x = 1.93, y = 0.43, xend = 1.93, yend = 0.45, color = "#0072B2",
               linewidth = 0.4) +
  annotate("text", x = -0.66, y = 0.475, label = "Pr(X \u2264 q)", size = 6,
           color = "#0072B2") +  
  annotate("text", x = -0.66, y = 0.5175, label = "Pr(X \u2264 1.96)", size = 5,
           color = "#0072B2") + 
  ## right
  geom_segment(x = 1.99, y = 0.45, xend = 3.25, yend = 0.45, color = "#009E73",
               linewidth = 0.4) +
  geom_segment(x = 1.99, y = 0.43, xend = 1.99, yend = 0.45, color = "#009E73",
               linewidth = 0.25) +
  geom_segment(x = 3.25, y = 0.43, xend = 3.25, yend = 0.45, color = "#009E73",
               linewidth = 0.4)  +
  annotate("text", x = 2.62, y = 0.475, label = "Pr(X > q)", size = 6,
           color = "#009E73") +  
  annotate("text", x = 2.62, y = 0.5175, label = "Pr(X > 1.96)", size = 5,
           color = "#009E73") + 
  labs(x = "", y = "") 
```
...
#| fig-height: 4.5
#| fig-width: 7
#| fig-cap: "Die t-Verteilung für drei beispielhafte Freiheitsgrade. Je größer die Freiheitsgrade und damit die Fallzahl, desto näher kommt die t-Verteilung einer Normalverteilung nahe. *[Zum Vergrößern anklicken]*"
...
#| fig-height: 5
#| fig-width: 10
#| fig-cap: "Histogramme der Anzahl an Gummibärchen und die Anzahl an Farben in einer Tüte. Es gibt nicht mehr als sechs Farben. **(A)** Anzahl an Bärchen. **(B)** Anzahl an Farben. *[Zum Vergrößern anklicken]*"
...
p1 <- ggplot(data = gummi_2_tbl, aes(x = count_bears)) +
  theme_minimal() +
...
p2 <- ggplot(data = gummi_2_tbl, aes(x = count_color)) +
  theme_minimal() +
  geom_bar(position = "dodge", color = "black", fill = cbbPalette[3]) +
  scale_x_continuous(breaks = seq(1, 6, by = 1), limits = c(1, 6.5)) +
  labs(x = "", y = "Anzahl") +
...
#| fig-width: 10
#| label: fig-pois-03
#| fig-cap: "Darstellung Anzahl an Bärchen und Anzahl an Farben. Es gibt nicht mehr als sechs Farben. Auf der linken Seite die beobachteten Werte und auf der rechten Seite die theoretischen Werte. **(A)** Verteilung der beobachteten Anzahl an Bärchen. **(B)** Verteilung der theoretischen Anzahl an Bärchen. **(C)** Verteilung der beobachteten Anzahl an Farben. **(D)** Verteilung der theoretischen Anzahl an Farben. *[Zum Vergrößern anklicken]*"
...
p1 <- ggplot(data = gummi_2_tbl, aes(x = count_bears)) +
  ...
  scale_y_continuous(breaks = seq(0, height_max, 25), limits = c(0, height_max))

set.seed(20220831)
p2 <- ggplot(data = tibble(count_bears = rpois(stat_bears_tbl$n, stat_bears_tbl$mean)), 
       aes(x = count_bears)) +
  theme_minimal() +
...
p3 <- ggplot(data = gummi_2_tbl, aes(x = count_color)) +
  theme_minimal() +
...
set.seed(20220831)
p4 <- ggplot(data = tibble(count_color = rpois(stat_color_tbl$n, stat_color_tbl$mean)), 
       aes(x = count_color)) +
  theme_minimal() +
...
#| fig-height: 5
#| fig-width: 10
#| fig-cap: "Histogramme der am liebsten gemochten Gummibärchchen im Vergleich zum Inhalt der Tütchen. **(A)** Anzahl am liebsten gemochten Gummibärchen aufgeteilt nach Geschlecht. **(B)** Anzahl der Gummibärchen aufgeteilt nach der Farbe. *[Zum Vergrößern anklicken]*"
...
Die Binomialverteilung wird uns vor allem später in den logistischen Regression wieder begegnen. An dieser Stelle ist es wichtig zu wissen, dass wir es bei der Binomialverteilung mit binären Ereignissen zu tun haben. Wir haben nur Erfolg oder nicht. Daher haben wir nur das Ergebnis $0/1$ daher Null oder Eins. Dieses Ergebnis ist im Prinzip auch die Beschreibung eines Patienten, ob dieser krank oder nicht krank ist. Deshalb finden wir die Binomialverteilung auch häufig in einem medizinischen Kontext. Es gibt auch ein schönes Tutorial zur [Binomial Distribution](https://mse.redwoods.edu/darnold/math15/spring2013/R/Activities/binomial1.html) von David Arnold.
...
#| fig-height: 6
#| fig-width: 7
#| fig-cap: "Darstellung des Pascalsche (oder Pascal'sche) Dreieckes im Zusammenhang zum Binomialkoeffizienten. *[Zum Vergrößern anklicken]*"

tibble(x = 0:14, y = 1) |>
  ggplot(aes(x, y)) +
  theme_minimal() +
  annotate("text", x = c(1, 3, 5, 7, 9, 11, 13), y = 1, 
           label = c(1, 6, 15, 20, 15, 6, 1), size = 8, face = "bold") +
  annotate("text", x = c(2, 4, 6, 8, 10, 12), y = 2, 
           label = c(1, 5, 10, 10, 5, 1), size = 8, face = "bold") +
  annotate("text", x = c(3, 5, 7, 9, 11), y = 3, 
           label = c(1, 4, 6, 4, 1), size = 8, face = "bold") +
  annotate("text", x = c(4, 6, 8, 10), y = 4, 
           label = c(1, 3, 3, 1), size = 8, face = "bold") +
  annotate("text", x = c(5, 7, 9), y = 5, 
           label = c(1, 2, 1), size = 8, face = "bold") +
  annotate("text", x = c(6, 8), y = 6, 
           label = c(1, 1), size = 8, face = "bold") +
  annotate("text", x = c(7), y = 7, 
           label = c(1), size = 8, face = "bold") +
  geom_segment(aes(x = 6, y = 6-0.2, xend = 7 - 0.15, yend = 5 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00") +
  geom_segment(aes(x = 8, y = 6-0.2, xend = 7 + 0.15, yend = 5 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00") +
  geom_segment(aes(x = 7, y = 5-0.2, xend = 8 - 0.15, yend = 4 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00") +
  geom_segment(aes(x = 9, y = 5-0.2, xend = 8 + 0.15, yend = 4 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00")  +
  geom_segment(aes(x = 6, y = 2-0.2, xend = 7 - 0.15, yend = 1 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00") +
  geom_segment(aes(x = 8, y = 2-0.2, xend = 7 + 0.15, yend = 1 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00") +
  geom_segment(aes(x = 5, y = 3-0.2, xend = 6 - 0.15, yend = 2 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00") +
  geom_segment(aes(x = 7, y = 3-0.2, xend = 6 + 0.15, yend = 2 + 0.15), linewidth = 0.5, 
               arrow = arrow(length = unit(0.02, "npc"), type = "closed"),
               color = "#E69F00") +
  annotate("text", 
           x = 1:7+0.4, y = 1:7+0.4, size = 4, color = "#CC79A7",
           label = c(expression(bgroup("(",atop(6, 0),")")),
                     expression(bgroup("(",atop(5, 0),")")),
                     expression(bgroup("(",atop(4, 0),")")),
                     expression(bgroup("(",atop(3, 0),")")),
                     expression(bgroup("(",atop(2, 0),")")),
                     expression(bgroup("(",atop(1, 0),")")),
                     expression(bgroup("(",atop(0, 0),")")))) +
    annotate("text", 
           x = 8:13+0.4, y = 6:1+0.4, size = 4, color = "#CC79A7",
           label = c(expression(bgroup("(",atop(1, 1),")")),
                     expression(bgroup("(",atop(2, 2),")")),
                     expression(bgroup("(",atop(3, 3),")")),
                     expression(bgroup("(",atop(4, 4),")")),
                     expression(bgroup("(",atop(5, 5),")")),
                     expression(bgroup("(",atop(6, 6),")")))) +
    annotate("text", 
           x = c(3, 5, 9, 11)+0.4, y = 1+0.4, size = 4, color = "#CC79A7",
           label = c(expression(bgroup("(",atop(6, 1),")")),
                     expression(bgroup("(",atop(6, 2),")")),
                     expression(bgroup("(",atop(6, 4),")")),
                     expression(bgroup("(",atop(6, 5),")")))) +
    annotate("text", 
           x = c(5, 7, 9)+0.4, y = 3+0.4, size = 4, color = "#CC79A7",
           label = c(expression(bgroup("(",atop(4, 1),")")),
                     expression(bgroup("(",atop(4, 2),")")),
                     expression(bgroup("(",atop(4, 3),")")))) +
    annotate("text", 
           x = 6+0.4, y = 4+0.4, size = 4, color = "#CC79A7",
           label = c(expression(bgroup("(",atop(3, 1),")")))) +
    annotate("text", 
           x = c(4, 8, 10)+0.4, y = 2+0.4, size = 4, color = "#CC79A7",
           label = c(expression(bgroup("(",atop(5, 1),")")),
                     expression(bgroup("(",atop(5, 3),")")),
                     expression(bgroup("(",atop(5, 4),")")))) +
  theme(legend.position = "none",
...
#| fig-height: 3.5
#| fig-width: 7
#| fig-cap: "Darstellung der Uniformverteilung zwischen den beiden Punkten $a$ und $b$. *[Zum Vergrößern anklicken]*"
...
tibble(x = 0:14, y = 1) |>
  ...
  xlim(0, NA) +
  annotate("rect", xmin = 2, xmax = 6, ymin = 0, ymax = 3, linewidth = 0.5,
           fill = "white", color = "black") +
  geom_segment(x = 2, y = 0, xend = 2, yend = 3, linewidth = 1, color = "#CC79A7") +
  geom_segment(x = 6, y = 0, xend = 6, yend = 3, linewidth = 1, color = "#009E73") +
  scale_x_continuous(breaks = c(2, 6), labels = c("a", "b")) +
  scale_y_continuous(breaks = c(3), labels = expression(over(1, b-a)), limits = c(0, 4)) + 
  theme(legend.position = "none",
...
#| fig-height: 5
#| fig-width: 10
#| fig-cap: "Beispiel für eine uniforme Verteilung anhand der Anzahl der Gummibärchen pro Tüte nach Farbe. **(A)** Verteilung der beobachteten Anzahl. **(B)** Verteilung der theoretischen Anzahl. *[Zum Vergrößern anklicken]*"

p1 <- ggplot(count_bag_tbl, aes(value)) +
  theme_minimal() +
  geom_bar(position = "dodge", color = "black", 
           fill = c("darkred", "darkolivegreen3", "white", "coral1", "yellow", "orange")) +
  ylim(c(0, 2200)) +
...
p2 <- tibble(y = rep(nrow(count_bag_tbl)/6, 6),
...
Weitere Beispiele finden sich unter [Basic Probability Distributions in R](https://rstudio-pubs-static.s3.amazonaws.com/100906_8e3a32dd11c14b839468db756cee7400.html). Im Weiteren liefert @dormann2013parametrische eine gute Übersicht über verschiedene Verteilungen und deren Repräsentation in R. Das ist nur eine Auswahl an möglichen Verteilungen. Bitte hier nicht ins *rabbit hole* der Verteilungen gehen. Wir benötigen in unserer täglichen Arbeit nur einen kleinen Teil der Verteilungen. Es reicht, wenn du eine Vorstellungen der Verteilungen in diesem Kapitel hat. Wir besuchen auch gerne die R Shiny App [The distribution zoo](https://ben18785.shinyapps.io/distribution-zoo/) um mehr über die verschiedenen Verteilungen und deren Parameter zu erfahren.

::: callout-tip
## Spielwiese Gummibärchendatensatz

In der folgenden Spielwiese habe ich mich gefragt, wie eigentlich die Verteilung der Farben der Gummibärchen in einer Tüte aussehen würde, wenn die Auswahl vollkommen zufällig erfolgt. Ich nehme jetzt an, dass die Wahrscheinlichkeit zufällig eine Farbe zu ziehen bei $1/6$ liegt. Wir können davon ausgehen, dass Haribo eine so große Menge an Bärchen produziert, dass sich die Wahrscheinlichkeit durch die Entnahme eines Bärchen nicht ändert. Im Weiteren simulieren wir nur Tütchen mit einer Anzahl von neun Bärchen, damit wird es dann etwas einfacher. In dem folgenden R Code Chunk findest du den Code für die Simulation von 10 Millionen Tütchen.
...
simulated_lst <- map(1:n_sim, \(...) {
  sample(c("darkred", "green", 
           "white", "lightred", 
           "yellow", "orange"), 9, replace = TRUE) 
})
```

Dann müssen wir einmal die Liste, die wir wiederkriegen etwas bearbeiten. Zum einen möchte ich nur noch einmalige *Farb*einträge in der Liste stehen haben. Mich interessiert ja nur, welche Farbe *mindestens einmal* vorkam. Dann möchte ich die Länge der einzelnen Vektoren wiedergeben bekommen. Am Ende wandele ich noch alles in einen Vektor um.
...
Jetzt kann ich mir den Vektor einmal als Tabelle wiedergeben lassen und sehe wie viele Farben jeweils in den einzelnen Tütchen waren.
...
simulated_tab <- simulated_vec |> 
  janitor::tabyl()
```

Hier dann einmal die Ausgabe unsere Tabelle aus dem obigen R Code Chunk zu der Anzahl an Farben pro Tütchen in absoluten Häufigkeiten. Wir sehen, dass wir nur 3 von 10 Millionen Tütchen mit nur einer Farbe erhalten. Das ist echt wenig und erklärt auch so ein wenig, warum wir bei ca. `r nrow(read_excel("data/gummibears.xlsx"))` ausgezählten Tütchen noch keine einfarbige Tüte gefunden haben.
...
In den relativen Häufigkeiten umgerechnet sehen wir nochmal eine andere Art, wie häufig die Tütchen sind. Damit können wir ungefähr davon ausgehen, dass pro 3 Millionen Tütchen wir dann ein Tütchen mit nur einer Farbe finden werden. Wenn wir also eine ganz bestimmte Farbe haben wollen, dann müssen wir schon fast 20 Millionen Tütchen uns anschauen.

Dann schauen wir mal aus Interesse, welche drei Farben denn in den drei Tütchen nur alle waren.
...
#| code-fold: true
#| code-summary: "R Code anzeigen"
simulated_lst[which(simulated_vec == 1)] 
```

In der @fig-gummisample-4 sehen wir dann nochmal das Histogramm der Verteilung der Farben. Ich habe hier dann einmal auf der linken Seite die beobachteten Anzahlen an Farben dargestellt. Auf der rechten Seite findest du dann die simulierten Anzahlen an Farben aus den 10 Millionen Tütchen. Es ist schon spannend, wie ähnlich sich die Verteilungen sind. Es würde also noch ein paar mehrere hundert Tütchen brauchen, bis ich eine ähnliche Verteilung hätte wie die simulierte Verteilung. Dafür aber sehr lange, bis ich dann wirklich eine nur einfarbige Tüte finde.
...
#| message: false
#| warning: false
#| label: fig-gummisample-4
#| fig-align: center
#| fig-height: 5
#| fig-width: 10
#| cache: false
#| fig-cap: "Histogramm der Anzahl an Farben in einer Tüte. Es gibt nicht mehr als sechs Farben. **(A)** Beobachtete Daten. **(B)** Simulierte Daten. *[Zum Vergrößern anklicken]*"
...
p1 <- ggplot(data = gummi_2_tbl, aes(x = count_color)) +
  theme_minimal() +
  geom_bar(position = "dodge", color = "black", fill = cbbPalette[3]) +
  scale_x_continuous(breaks = seq(1, 6, by = 1), limits = c(1, 6.5)) +
  labs(x = "", y = "Anzahl") +
...
p2 <- p +
  scale_x_continuous(breaks = seq(1, 6, by = 1), limits = c(0.5, 6.5)) +
  labs(x = "", y = "Anzahl") +
  geom_label(stat='count', aes(label=..count..), size = 3) +
  theme(axis.text.y = element_blank())
...
Da unsere beiden Abbildungen ja so ähnlich aussehen nochmal ein anderer Vergleich der Häufigkeiten. Im Folgenden ist einmal die Tabelle der relativen Häufigkeiten der Farben aus unseren erhobenen Daten abgebildet.
...
Farben <- gummi_2_tbl$count_color
Farben |> 
  janitor::tabyl()
```
...
```{r}
#| echo: false
Farben <- simulated_vec
Farben |> 
  janitor::tabyl()
```

Wenn du die relativen Häufigkeiten der erhobenen Daten einmal mit den simulierten vergleichst, dann stellst du fest, dass wir mit den beobachteten Daten schon sehr nah an den simulierten Daten sind. Ist immer wieder erstaunlich für mich. So machen dann `r nrow(read_excel("data/gummibears.xlsx"))` ausgezählte Daten gar keinen so richtigen Unterschied mehr zu 10 Millionen simulierten Tütchen. Nur die Anzahl der Bärchen in den echten Tütchen variiert, aber das ist eben dann auch durch den Mittelwert sehr gut abgedeckt.
:::
