```{r echo = FALSE}
pacman::p_load(tidyverse, readxl, knitr, kableExtra, Hmisc, patchwork)
set.seed(20230523)
```
...
> One of the most common errors in interpreting the correlation coefficient is failure to consider that there may be a third variable related to both of the variables being investigated, which is responsible for the apparent correlation. Correlation does not imply causation. --- @bewick2003statistics, Misuse of correlation
...
#| fig-height: 4
#| fig-width: 12
#| fig-cap: "Visualisierung der Korrelation für drei Ausprägungen des Korrelationskoeffizient. *[Zum Vergrößern anklicken]*"
...
df <- tibble(x = rnorm(20, 0, 0.5), #seq(0, 1, length.out = 1000),
             ...
             y_2 = 0 + corr_vec[2] * x + rnorm(length(x), 0, r2_vec[1]),
             y_3 = 0 + corr_vec[3] * x + rnorm(length(x), 0, r2_vec[1]))
p1 <- ggplot(df, aes(x, y_1)) + geom_point(size = 3) +# xlim(0, 1) +
  ylim(-1, 1) + xlim(-1,1) +
  labs(x = "", y = "") +
  theme_classic() +
  stat_smooth(method = "lm", se = FALSE, color = "#56B4E9") +
  annotate("text", x = 0, y = 0.75, label = expression(r~'='~-1), size = 10,
...
p2 <- ggplot(df, aes(x, y_2)) + geom_point(size = 3) +# xlim(0, 1) +
  ...
  theme_classic() +
  stat_smooth(method = "lm", se = FALSE, color = "#56B4E9") +
  annotate("text", x = 0, y = 0.75, label = expression(r%~~%0), size = 10,
           color = "#0072B2", hjust = "left") +
  theme(axis.text = element_blank(),
        axis.text.y = element_blank()) 
p3 <- ggplot(df, aes(x, y_3)) + geom_point(size = 3) +# xlim(0, 1) +
  ylim(-1, 1) + xlim(-1,1) +
  labs(x = "", y = "") +
  theme_classic() +
  stat_smooth(method = "lm", se = FALSE, color = "#56B4E9") +
  annotate("text", x = -0.5, y = 0.75, label = expression(r~'='~+1), size = 10,
           color = "#0072B2", hjust = "left") +
  theme(axis.text = element_blank(),
        axis.text.y = element_blank()) 
