...
-- Mehrere Fragestellungen sollen gleichzeitig beantwortet werden innerhalb derselben Studie

Inflation des $\alpha$-Fehlers oder Alphafehler-Kumulierung -- Simultanes Testen von mehreren Hypothesen führt zu einer $\alpha$-Fehler Inflation -- Die Wahrscheinlichkeit, dass mindestens eine Nullhypothese fälschlicherweise abgelehnt wird, ist nicht mehr durch das Signifikanzniveau $\alpha$ kontrolliert, sondern kann sehr groß werden.

Situation $k$ Null- und Alternativhypothesen 𝐻 alle Nullhypothesen werden zum lokalen Niveau $\alpha = 0.05$ getestet tatsächlich sind alle Nullhypothesen gültig

Wahrscheinlichkeit für einen einzelnen Test diesen korrekterweise abzulehnen ist $(1 − \alpha)$

Da die Tests unabhängig sind, ist die Wahrscheinlichkeit alle $k$ Tests korrekterweise abzulehnen $(1 − \alpha)^k$

Wahrscheinlichkeit, dass mindestens eine fälschlicherweise Nullhypothese abgelehnt wird:
...
: Inflation des $\alpha$-Fehlers. Wenn 50 Hypothesen getestet werden, ist die Wahrscheinlichkeit mindestens eine falsche Testentscheidung zu treffen fast 100%! {#tbl-mult-alpha}
...
Aus @tbl-mult-null können wir entnehmen, dass wenn 100 Hypothesen getestet werden, werden 5 Hypothesen im Schnitt fälschlicherweise abgelehnt.

#### Bonferroni Korrektur

Damit die Wahrscheinlichkeit, dass mindestens eine Nullhypothese fälschlicherweise abgelehnt wird beim simultanen Testen von $k$ Hypothesen durch das globale (und multiple) Signifikanzniveau $\alpha$ kontrolliert ist, werden die Einzelhypothesen zum lokalen Signifikanzniveau $\alpha_{local} = \tfrac{\alpha}{k}$ getestet.

Dabei ist das Problem der Bonferroni Korrektur, dass die Korrektur sehr konservativ ist , d.h. das tatsächliche globale (und multiple) $\alpha$ Niveau liegt deutlich unter $\alpha$, d. h. die Nullhypothesen werden zu oft beibehalten.
