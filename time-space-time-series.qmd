...
::: {.callout-caution collapse="true"}
## Zerforschen: Zeitlicher Verlauf von Kühen

![](images/caution.png){fig-align="center" width="50%"}

Dieses Zerforschenbeipsiel ist etwas anders, denn wir haben hier Daten vorliegen und ich möchte einmal kurz und knackig zeigen, ob wir hier eine Prognose oder Vorhersage rechnen können. Später kannst du dir dann das nötige Handwerkszeug dann in entsprechenden Kapiteln nacharbeiten.

```{r}
cow_tbl <- read_excel("data/time_series_cows.xlsx") %>% 
  mutate(time = as.Date(time)) %>% 
  filter(time >= "2020-01-01" & time < "2022-01-01")
```
...
#| echo: false
#| warning: false
#| label: fig-time-zerforschen-cow-milk-1
#| fig-align: center
#| fig-height: 4
#| fig-width: 9
#| fig-cap: "Eine Zeitreihe."
cow_tbl %>% 
  ggplot(aes(time, milk_kgtag)) +
  geom_line() +
  theme_bw() +
  scale_x_date(date_breaks = "months" , date_labels = "%b") +
  labs(x = "", y = "Milchleistung [kg/d]") 
...
Bis jetzt haben wir uns die Visualisierung von Zeitreihen angeschaut. Häufig reicht die Visualisierung auch aus, wenn es um die Darstellung von Temperaturverläufen in einer Abschlussarbeit geht. Darüber hinaus wollen wir dann aber doch irgendwie eine statistische Analyse auf der Zeitreihe rechnen. Deshalb habe ich hier mal angefangen Beispiele zu Auswertungen von Zeitreihen zu sammeln und vorzustellen. Hauptsächlich nehme ich natürlich die drei Datensätze von weiter oben im Kapitel. Da die drei Datensätze zwar echte Daten aus dem agrarwissenschaftlichen Kontext repräsentieren, genügen die Datensätze dann doch nicht immer einer klassischen Zeitreihenanalyse. Deshalb findest du auch hier in dem Abschnitt immer mal wieder einen neuen Beispieldatensatz zur Demonstration.

Wir beginnen also erstmal nicht so typische Analysen von Zeitreihen. Danach schauen wir uns dann die klassische Zeitreihenanalyse einmal an. Dafür brauchen wir dann auch die klassischen Daten einer Zeitreihe.

-   Du findest im Abschnitt [einfache Glättungen](#sec-time-roll-mean) verschiedene Beispiele für die Glättung von Zeitreihen durch Mittelwert, Median oder aber auch der Summe. Du könntest auch `stat_smooth()` aus `{ggplot}` verwenden, aber hier zeige ich dir noch ein paar differenzierte Möglichkeiten.
-   Du findest im Abschnitt [Vergleich von Zeitreihen](#sec-time-two-compare), den Vergleich zweier Zeitreihen über den gesamten Verlauf der Zeitreihe. Dich interessiert hier nicht, ob die eine Zeitreihe eine verschobene Zeitreihe der anderen ist, sondern ob sich die Werte von $y_1$ und $y_2$ gegeben einem Zeitpunkt $t$ unterscheiden.

Im Anschluss an die beiden Abschnitte findest du dann nochmal die Vorhersage von zukünftigen zeitlichen Verläufen. Weil es dann hier wirklich etwas mehr Auswahl an Möglichkeiten gibt, habe ich die Vorhersage einmal für die verschiedenen R Pakete aufgeteilt.

### Einfache Glättungen {#sec-time-roll-mean}

{{< include time-space/time-space-roll-mean.qmd >}}

### Vergleich von Zeitreihen {#sec-time-two-compare}

{{< include time-space/time-space-time-compare.qmd >}}

### Definitionen und Überblick

In dem folgenden Abschnitt möchte ich gerne einmal einen Überblick über die wichtigsten Begriffe in der Analyse von Zeitreihen geben. Jetzt geht es also los mit der "richtigen" Zeitreihenanalyse. Teilweise sind es etwas speziellere Begriffe, so dass ich hier erstmal etwas zu den Begriffen schreibe und dann die einzelnen Begriffe nochmal tiefer erkläre. Als vertiefenden Einstieg kann ich hier auch das Buch "Forecasting: principles and practice" von @hyndman2018forecasting empfehlen. Hier gibt es dann auch eine [Webseite mit Videos zu dem Buch](https://otexts.com/fpp3/). Denn so ausführlich kann ich das Thema Zeitreihenanalyse dann doch nicht in einem Kapitel bearbeiten.
