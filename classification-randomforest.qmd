...
Die Wichtigkeit der Variablen wird anhand von drei verschiedenen Wichtigkeiten für eine Variable berechnet.
...
-   **Cover** ist die relative Beobachtung, die mit einem Prädiktor verbunden ist. Also der Anteil der Beobachtungen, die mit dieser variable zusammenhängt. Nehmen wir an Merkmal X wird dazu verwendet, um einen Terminalknoten für 10 Beobachtungen in einem Baum zu erschaffen. Im in einem weiteren Baum ist es ein Terminalkonten mit 20 Beobachtungen. Damit haben wir 30 absolute Beobachtungen, die mit Merkmal X verbunden sind. Die relative Beobachtung ist dann 30 geteilt durch die Summe aller absoluten Beobachtungen für alle Merkmale.
-   **Häufigkeit** bezieht sich auf die relative Häufigkeit, mit der eine Variable in den zusammengestellten Bäumen vorkommt. Nehmen wir an Merkmal X kommt in Baum A in einem Split und in Baum B in zwei Splits vor. Die absolute Häufigkeit von Merkmal X ist 3 und die relative Häufigkeit ist dann 3 durch die Summe aller absoluten Vorkommen für alle Merkmale.
...
#| fig-height: 5
#| fig-width: 6
#| fig-cap: "Visualisierung der *Variable importance* aus unseren `xgboost` Algorithmus."
