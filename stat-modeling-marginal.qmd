...
Was ist ein Datenraster (eng. *data grid*) eigentlich? Wir brauchen die Idee des Datenrasters um überhaupt die Vorhersagen und die kontrafaktischen Vergleiche zu verstehen. Im Prinzip beinhaltet das Datenraster die Information zu welchen Beobachtungen wir eine Vorhersage machen wollen. Wenn wir nochmal kutz zu dem Enyzmebeispiel kommen, zu welchen pH-Werten möchtest du dann eine Steigung oder aber eine Vorhersage der Enzymetätigkeit haben? Beginnen wir mit einem einfachen Beispiel um zu verstehen was die einzelnen Datenraster aussagen wollen.
...
```{r}
model_ln <- lm(jump_length ~ hatched,
               data = flea_model_tbl)
tidy(model_ln)
```
...
$$
\begin{aligned}
\operatorname{E}[\text{Sprungweite} \mid \text{Schlupfzeit}] &= 125 + (-53.8 \times \text{Schlupfzeit}) + (18.9 \times \text{Schlupfzeit}^2) + (-1.6 \times \text{Schlupfzeit}^3) \\[6pt]
...
#| warning: false
model_sq |> 
  emtrends(~ hatched, var = "hatched",
           at = list(hatched = c(1.25, 3.5, 5.75))) |> 
  test()
```
:::
...
plot_slopes(model_sq, 
            ...
            condition = "hatched") +
  labs(x = "Schlupfzeit in [h]", y = "Sprungweite in [cm]",
       title = "Steigung der Tangente über die Schlupfzeiten") +
  theme_marginal()
```
