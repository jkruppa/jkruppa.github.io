...
#| tbl-cap: "Beispiel für die Berechnung von einem Mittelwertseffekt an der Sprunglänge [cm] von Fuchs-, Hunde- und Katzenflöhen."
data_compact_tbl |> 
  kable(align = "c", "pipe")
...
tibble(x = c(1, 2, 3), y = 8.13) |> 
  ...
  geom_segment(x = 2.9, y = 11.27, xend = 3.1, yend = 11.27, color = "black",
               linewidth = 1, linetype = 1) +
  scale_x_continuous(breaks = c(0.5, 1, 2, 3), label = c(0, "Grp1", "Grp2", "Grp3")) +
...
tibble(x = c(1, 2, 3), y = 8.13) |> 
  ...
  geom_segment(x = 0.9, y = 10.27, xend = 1.1, yend = 10.27, color = "black",
               linewidth = 1, linetype = 1) +
  geom_rect(aes(xmin = 1.75, xmax = 2.25, ymin = 0, ymax = 5.5), fill = "#56B4E9",
            color = "black",  linewidth = 1) +
...
#  labs(x = "", y = "") +
 # scale_y_continuous(breaks = 0)  +
#  scal e_x_continuous(breaks = 1, label = "A") +
 # theme(legend.position = "none",
 #       axis.text.x = element_text(size = 16),
 #       axis.text.y = element_text(size = 14),
  #      panel.grid.major = element_blank(),
...
Die Implementierung des *Compact letter display* findest du im [Kapitel zum Post-hoc-Test](#sec-posthoc)
