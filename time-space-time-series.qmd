...
Am Ende wollen wir uns dann nochmal Daten einer Wetterstation in Hagebüchen an. Auch hier haben wir wieder sehr viele Daten vorliegen und wir müssen uns überlegen, welche der Daten wir nutzen wollen.
...
Auch hier haben wir in der @tbl-time-station gut `r nrow(station_tbl)` einzelne Messungen vorliegen. Das ist dann auch unserer größter Datensatz von Klimadaten.
...
Nachdem wir ordentlich an den Daten geschraubt haben können wir jetzt in @fig-weather-station-hage die zwei Konturplots sehen. Wir mussten noch das Spektrum der Farben einmal drehen, damit es auch mit den Temperaturfarben passt und wir haben noch ein paar Hilfslinien miteingezeichnet. Sie dazu auch [Isoplethendiagramm für Münster & Osnabrück](https://jkruppa.github.io/application/example-analysis-01.html) im *Skript zu beispielhaften Anwendung*.
...
#| fig-subcap: 
#|   - "Temperaturen der Wetterstation über den Tag."
#|   - "Temperaturen der Wetterstation über den Monat."
#| layout-nrow: 1
#| column: page

ggplot(station_tbl, aes(month, hour, z = temp_boden_durch)) +
  theme_minimal() +
...
ggplot(station_tbl, aes(month, day, z = temp_boden_durch)) +
  theme_minimal() +
  geom_contour_filled(bins = 13) +
  geom_contour(binwidth = 2, color = "black") +
  scale_fill_brewer(palette = "Spectral", direction = -1) +
