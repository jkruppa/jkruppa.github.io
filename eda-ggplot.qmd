...
![Ursprüngliche Abbildung, die nachgebaut werden soll.](images/eda/zerforschen_regression.jpeg){#fig-ur-regression fig-align="center" width="80%"}
...
Das `\(x){}` ist die Kurzschreibweise `function(x){}`
...
ggplot(data = func_tbl, aes(x = iodine, y = content)) +
  theme_bw() +
  geom_line(stat = 'function', fun = ki_func, color = cbbPalette[2], linetype = 'dashed') +
  geom_line(stat = 'function', fun = kio3_func, color = cbbPalette[3], linetype = 'dashed') 
```
...
ggplot(data = stat_tbl, aes(x = iodine, y = mean,
  ...
  scale_color_okabeito() +
  annotate("text", x = 1.525, y = 150, label = "Target range", hjust = 0,
           size = 5) + 
...
```{r}
nls(content ~ b1 * iodine^2 + b2 * iodine, data = filter(regression_tbl, type == "KI"), 
    start = c(b1 = 1, b2 = 1)) %>% 
...
```{r}
nls(content ~ b1 * iodine^2 + b2 * iodine, data = filter(regression_tbl, type == "KIO3"), 
    start = c(b1 = 1, b2 = 1)) %>% 
  parameters::model_parameters() %>% 
  select(Parameter, Coefficient)
```
