...
Häufig stellt sich auch die Frage, wie lang soll den so ein Fragebogen sein? Du hast so viele Fragen und wenn dann schon mal einer antwortet, dann kann der doch gefälligst dreizig Seiten Fragebogen ausfüllen? Du kannst dann ja später immer noch Fragen rausschmeißen.
...
::: callout-caution
## Moment, wo sind denn die stetigen bzw. kontinuierlichen Antworten?

Ja, das stimmt. Ich habe in diesem Kapitel nur eine Auswertung mit kategorialen Antworten gewählt. Das hat den einfachen Grund, dass mir bis jetzt eigentlich nur Likertskalen in Fragebögen oder eben andere Kategorien in der Beratung aufgekommen sind. Wenn ich dann mal ein anderes Beispiel finde, dann ergänze ich gerne nochmal die kontinuierlichen Antworten. Bis dahin liefert das Buch [Using R for Social Work Research \| Survey Research](https://bookdown.org/bean_jerry/using_r_for_social_work_research/survey-research.html) auch die Informationen zu den selten aufkommenden kontinuierlichen Daten.
:::
...
#| fig-height: 4
#| fig-width: 7
#| fig-cap: "Drastellung der Likertskala der Antworten zur Verbandkommunikation. Wir haben hier wiederum keine Sortierung in den Fragen. Die Antworten werden nach den positiven und negativen Antwortkategorien sortiert. Die neutrale Kategorie wird links an der Seite dargestellt."
tiger_ord_tbl |> 
...
tiger_ord_tbl |> 
  set_names(short_question_tbl$question) |>
  tbl_summary(by = "Ist Ihr Erlebnishof Haupterwerb oder Nebenerwerb?") |> 
  add_p()
```
...
::: {.callout-note collapse="true"}
## Bivariate Analyse mit der Funktion `tbl_summary()` für den Erwerb
...
#| tbl-cap: "Bivariate Analyse der einzelenen Fragen und deren Antwortverteilung aufgeteilt nach der Frage zum Erwerb. Anhand der Tabelle können fehlende Werte und ungleichmäßig beantwortete Fragen erkannt und dann ausgeschlossen werden."
tiger_ord_tbl |> 
  set_names(short_question_tbl$question) |>
  tbl_summary(by = "Ist Ihr Erlebnishof Haupterwerb oder Nebenerwerb?") |> 
  add_p()
