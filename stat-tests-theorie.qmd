...
#| fig-height: 4.5
#| fig-width: 7
#| fig-cap: "Darstellung der Null- und Alternativehypothese. Mit steigendem $T_{calc}$ wird die Wahrscheinlichkeit für die $H_0$ immer kleiner. Leider ist uns nichts über $H_A$ und deren Lage bekannt. Sollte die $H_A$ Verteilung zu weit nach links ragen, könnten wir die $H_0$ beibehalten, obwohl die $H_A$ gilt. *[Zum Vergrößern anklicken]*"
...
ggplot(data.frame(x = c(-3.25, 3.25)), aes(x)) +
  ...
  theme(axis.text = element_text(size = 12),
        axis.text.y = element_blank()) +
  labs(x = "", y = "") +
  annotate("text", x = -4, y = 0.35, label = "Zweiseitig", size = 12,
...
ggplot(data.frame(x = c(-3.25, 3.25)), aes(x)) +
  ...
  theme(axis.text = element_text(size = 12),
        axis.text.y = element_blank()) +
  labs(x = "", y = "") +
  annotate("text", x = -4, y = 0.35, label = "Einseitig", size = 12,
           color = "gray20", hjust = "left") 
```
