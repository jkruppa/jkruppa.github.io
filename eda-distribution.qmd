...
In @fig-gummi-mostliked betrachten wir die Verteilung der am meisten gemochten Gummibärchen aufgeteilt nach dem angegebenen Geschlecht im Vergeich zu den Gummibärchen in den Tütchen. Wir sehen, dass Haribo die Tütchen sehr gleichmäßig verteilt und auf die Geschmäcker keinerlei Rücksicht nimmt.
...
ggplot(data = gummi_4_tbl, aes(x = most_liked)) +
  ...
  #scale_x_continuous(breaks = seq(5, 15, by = 1), limits = c(5, 15)) +
  labs(x = "Lieblingsgeschmack", y = "Anzahl", fill = "Geschlecht") +
  #theme(legend.position = "none") +
  scale_fill_okabeito()
...
Im Weiteren liefert @dormann2013parametrische eine gute Übersicht über verschiedene Verteilungen.
