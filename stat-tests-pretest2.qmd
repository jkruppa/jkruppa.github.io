...
#| fig-height: 3.5
#| fig-width: 8.5
#| fig-cap: "Densityplot potenzieller Normalverteilungen. Die durchgezogene Linie stellt den Mittelwert in dem Densityplot dar. **(A)** Zweigipflige Verteilung aus vernutlich zwei oder mehr Verteilungen. **(B)** Eine zu schmale Verteilung aber dennoch approximativ normalverteilt. **(C)** Eine linksschiefe Verteilung mit einer linken Verteilungsschulter. *[Zum Vergrößern anklicken]*"
...
p2sample <- t_stat_tbl |> 
  ...
  annotate("label", -5.5, 0.3, label = shapiro_p, hjust = "left") +
...
p3sample <- chi_stat_tbl |> 
  ...
  annotate("label", -5.5, 0.1, label = shapiro_p, hjust = "left") +
  labs(title = "foo",
       subtitle = "bar", 
       caption = "foo") +
  theme(plot.title = element_text(size = 17),
