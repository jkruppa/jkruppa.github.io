...
#| fig-height: 4.5
#| fig-width: 6
#| fig-cap: "Darstellung einer linearen Simulation mit einer normalverteilten Kovariaten sowie einem poissonverteilten Messwert. *[Zum Vergrößern anklicken]*"
...
Wir können dann auch nochmal ein GLM rechnen und sehen, dass wir bei einer so hohen Fallzahl dann auch in einem GLM die passenden Effekte herausbekommen. Ich nutze hier die Funktion `parameters()` aus dem gleichnamigen R Paket damit ich die Schätzer noch wieder zurücktransformieren kann. Auch hier findest du dann mehr in dem [Kapitel zu der Poissonregression](#sec-poisson). Am Ende ist es dann nur wichtig, dass wir unsere voreingestellten Effekte wieder aus den Daten extrahieren konnten.

```{r}
#| message: false
#| warning: false
glm(y ~ c_1, data = cov1_pois_tbl, family = poisson) |>
  parameters::model_parameters(exponentiate = TRUE)
```

```{r}
cov1_pois_tbl |> 
  summarise(mean(y), mean(c_1))
```
...
cov2_pois_def <- defData(varname = "c_1", dist = "normal", formula = "10",
                         variance = 2) |> 
  defData(varname = "c_2", dist = "normal", formula = "5",
                         variance = 1) |> 
  defData(varname = "y", dist = "poisson", formula = "8 + 2 * c_1 + 1 * c_2")
```

```{r}
cov2_pois_tbl <- genData(1000, cov2_pois_def)
```
...
#| message: false
#| warning: false
glm(y ~ c_1 + c_2, data = cov2_pois_tbl) |> 
  parameters::parameters(exponeniate = TRUE)
```

#### Multikovariat abhängig $c_1 + c_2 + c_3$ {.unnumbered .unlisted}

Eine multivariate Poissonverteilung ist nicht einfach zu bauen. Jedenfalls um so viel schwerer, dass eine gerundete multivariate Normalverteilung mit Korrelationsstruktur viel einfacher zu erstellen ist.

[Correlated data: additional distributions](https://kgoldfeld.github.io/simstudy/articles/correlated.html#correlated-data-additional-distributions)
