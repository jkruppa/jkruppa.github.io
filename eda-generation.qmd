...
Wenn wir uns mit dem faktoriellen Design beschäftigen dann generieren wir zuerst das Faktorenraster und ergänzen dann den Messwert aus der Poissonverteilung.
...
Wir nutzen hier das R Paket `{data.table}` zusammen mit dem R Paket `{simstudy}`
...
```{r}
y_pois_def <- defDataAdd(varname = "y", formula = "-2 + 2*f_a + 6*f_b",
                         dist = "poisson")
```
...
y_pois_def <- defDataAdd(varname = "y", 
                         formula = "5 * intercept + 2 * f_a2 + 2 * f_a3 + 
...
#### Mischung aus 3 Normalverteilungen {.unnumbered .unlisted}

```{r}
mix_def <- defData(varname = "c_1", formula = 1, variance = 1)
mix_def <- defData(mix_def, varname = "c_2", formula = 5, variance = 4)
mix_def <- defData(mix_def, varname = "c_3", formula = 9, variance = 1)
mix_def <- defData(mix_def, varname = "c_mixed", 
                   formula = "c_1 | .3 + c_2 | .4 + c_3 | .3", 
                   dist = "mixture")
mix_dt <- genData(1000, mix_def)
mix_dt
```
...
p1_mixed_normal <- mix_dt |> 
  ggplot(aes(x = c_mixed)) +
  theme_modeling() +
  geom_density(fill = "#F5C710") +
  xlim(-5, 13) +
  labs(x = "Kovariaten gemeinsam", y = "Density", fill = "")

p2_mixed_normal <- mix_dt |> 
  pivot_longer(c_1:c_3) |> 
  ggplot(aes(x = value, fill = name)) +
  theme_modeling() +
  geom_density() +
  scale_fill_okabeito() +
  theme(legend.position = "top") +
  labs(x = "Individuelle Kovariaten", y = "Messwert (Y)", fill = "")
