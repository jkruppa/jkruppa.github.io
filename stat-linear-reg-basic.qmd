...
by_hand_tbl |> 
  mutate(a = height * weight,
         b = height^2,
         c = weight^2) |> 
  set_names(c("height", "weight", "$Y\\cdot X$", "$X^2$", "$Y^2$")) |> 
  kable(align = "c", "pipe")
```
...
#| tbl-cap: "Auszug aus dem Daten zu den kranken Ferkeln."
tibble(a = sum(by_hand_tbl$height * by_hand_tbl$weight), 
       b = sum(by_hand_tbl$height^2), 
       c = sum(by_hand_tbl$weight^2),
       height = sum(by_hand_tbl$height),
       weight = sum(by_hand_tbl$weight)) |> 
  set_names(c("height $(\\Sigma X)$", "weight $(\\Sigma Y)$", "$\\Sigma YX$", "$\\Sigma X^2$", "$\\Sigma Y^2$")) |> 
  kable(align = "c", "pipe")
```

$$
weight = b_0 + b_1 \cdot height
$$

$$
b_0 = \cfrac{(\Sigma Y)(\Sigma X^2) - (\Sigma X)(\Sigma XY)}{n(\Sigma X^2)(\Sigma X)^2)}
$$
...
$$
b_1 = \cfrac{n(\Sigma XY) - (\Sigma X)(\Sigma Y)}{n(\Sigma X^2)(\Sigma X)^2)} 
$$

Weitere Quellen zu der Thematik unter den folgenden Links und Tutorien.
