...
Bei der univariaten Imputation müssen wir unterscheiden, welche Art die Spalte bzw. Variable ist, die wir imputieren wollen. Wenn die Spalte numerisch ist, daher ein `double <dbl>` oder `integer <int>` können wir folgende Optionen der Funktion `impute_na()` wählen.

-   **mean**: Dem Mittelwert in der Variablenspalte.
-   **median**: Den Median in der Variablenspalte.
-   **mode**: Dem Modus beziehungsweise den häufigsten Wert in der Variablenspalte.

Wenn die Spalte kategorisch ist, daher ein `factor <fct>` oder `character <chr>` können wir folgende Optionen der Funktion `impute_na()` wählen.

-   **mode**: Dem Modus beziehungsweise den häufigsten Wert in der Variablenspalte..
-   **rpart**: Recursive Partitioning and Regression Trees. Siehe dazu auch das @sec-class-rf.
