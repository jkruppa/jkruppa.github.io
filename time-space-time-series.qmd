...
:   Das AR-Modell setzt den aktuellen Wert der Reihe in Beziehung zu ihren vergangenen Werten. Es geht davon aus, dass die Vergangenheitswerte in einem linearen Verhältnis zum aktuellen Wert stehen.

MA Modell

:   Das MA-Modell setzt den aktuellen Wert der Reihe mit dem weißen Rauschen oder den Fehlertermen der Vergangenheit in Beziehung. Es erfasst die Schocks oder unerwarteten Ereignisse der Vergangenheit, die sich noch immer auf die Reihe auswirken.
...
::: panel-tabset
## Laufender Durchschnitt 1 oder q(1)

```{r}
resid_ma_1 <- lm(erupt ~ time, ma_tbl) %>% 
  resid()
ma_tbl <- ma_tbl %>% 
  bind_cols(ma_1 = resid_ma_1)
```

## Laufender Durchschnitt 2 oder q(2)

```{r}
resid_ma_2 <- lm(ma_1 ~ time, ma_tbl) %>% 
  resid()
ma_tbl <- ma_tbl %>% 
  bind_cols(ma_2 = resid_ma_2)
```

## Laufender Durchschnitt 3 oder q(3)

```{r}
resid_ma_3 <- lm(ma_2 ~ time, ma_tbl) %>% 
  resid()
ma_tbl <- ma_tbl %>% 
  bind_cols(ma_3 = resid_ma_3)
```
:::
...
ggpairs(ma_tbl[,-2], 
        upper = list(continuous = wrap(cor_func, method = 'pearson', 
                                       symbol = expression('\u03C1 ='))),
        lower = list(continuous = wrap(lower_fun)),
        columnLabels = c("Original", "q(1)", "q(2)", "q(3)"),
        axisLabels = "internal") +
  theme_bw()
```
