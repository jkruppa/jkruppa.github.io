...
#| fig-height: 3
#| fig-width: 12
#| fig-cap: "foo. **(A)** ooo. **(B)** gggg. **(C)** gggg. *[Zum Vergrößern anklicken]*"
...
p1 <- gg_template %+%
  ...
  annotate("text", x = -8, y = 0.08, label = expression(p[A]~"<"~p[B]), size = 5, hjust = "left") +
...
p2 <- gg_template %+%
  ...
  scale_x_continuous(name = "", breaks = c(10, 5), 
                     label = c(expression(bar(y)[A]), expression(bar(y)[B]))) +
  annotate("text", x = 17, y = 0.08, label = expression(p[A]~">"~p[B]), size = 5, hjust = "left") +
