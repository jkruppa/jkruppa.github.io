...
::: column-margin
Die 2x2 Kreuztabelle wird als [Confusion matrix](https://en.wikipedia.org/wiki/Confusion_matrix) viel auch in der Klassifikation im machinellen Lernen genutzt. Im maschinellen Lernen heißt es dann meist nur anders...
:::
...
Wir können jetzt für jede gelbe Linie als Threshold des Testscore die Spezifität und die Sensitifität berechen. berechnen @tbl-data-roc-2 sind die Werte von Spezifität und Sensitifität für jede gelbe Linie ausgegeben. Wenn wir als Entscheidungsgrenze einen Testscore von 0.12 nehmen würden, dann würden wir 1 Hund als krank und 13 als gesund klassifizieren. Wir hätten 7 TN, 1 TP, 6 FN und 0 FP. Aus diesen Zahlen, die eine 2x2 Kreuztabelle entsprechen, können wir dann die Spezifität und die Sensitifität berechnen. Wir berechnen dann die Spezifität und die Sensitifität für jeden Threshold.

```{r}
#| message: false
#| echo: false
#| tbl-cap: Die Spezifität und die Sensitifität berechnet für jeden Threshold.
...
Wir können jetzt für jeden Threshold und damit jedes verbundene Spezifität und Sensitifität Paar einen Punkt in einen Plot einzeichnen. Wir lassen damit Stück für Stück die ROC Kurve wachsen.

In der @fig-roc-drawn-1 sehen wir die ROC Kurve mit dem Threshold von 0.32 vorliegen. Wir haben damit 7 TN, 4 TP, 3 FN und 0 FP klassifizierte Beobchtungen. Darauf ergibt sich eine Spezifität von 1 und eine Sensitifität von 0.57. In der ROC Kurve tragen wir auf die $x$-Achse die 1 - Spezifitätswerte auf. Damit haben wir eine schön ansteigende Kurve.

::: column-page
![Visualisierung des Anwachsen der ROC Kurve mit einem Threshold von 0.32 und damit 7 TN, 4 TP, 3 FN und 0 FP.](images/diag-roc-01.png){#fig-roc-drawn-1 fig-align="center" width="80%"}
:::
...
::: column-page
![Visualisierung des Anwachsen der ROC Kurve mit einem Threshold von 0.48 und damit 4 TN, 5 TP, 2 FN und 3 FP.](images/diag-roc-02.png){#fig-roc-drawn-2 fig-align="center" width="80%"}
:::
...
::: column-page
![Visualisierung des Anwachsen der ROC Kurve mit einem Threshold von 0.65 und damit 3 TN, 7 TP, 0 FN und 4 FP.](images/diag-roc-03.png){#fig-roc-drawn-3 fig-align="center" width="80%"}
:::

[Ein AUC von 1 bedeutet eine perfekte Trennung der beiden Gruppen durch den Testscore. Ein AUC von 0.5 bedeutet keine Trennung der beiden Gruppen durch den Testscore.]{.aside}
...
Wir berechnen die ROC Kurve nicht per Hand. Das wäre eine sehr große Fleißarbeit für jeden Threshold zwischen den Beobachtungen die jeweilige Spezifität und Sensitifität Paare zu berechnen. Wir nutzen daher das R Paket `pROC`. Das Pakt erlaubt es uns später auch recht einfach ROC Kurven miteinander zu vergleichen und einen statistischen Test zu rechnen.
