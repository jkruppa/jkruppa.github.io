...
shoot_length_tbl <- cutting_tbl %>% 
  filter(outcome == "shoot")

fit <- lmer(rsp ~ trt + block +(1 | week), data = shoot_length_tbl) 

fit %>% model_performance()
...
https://strengejacke.github.io/ggeffects/ https://strengejacke.github.io/ggeffects/articles/introduction_marginal_effects.html

https://vincentarelbundock.github.io/marginaleffects/ https://vincentarelbundock.github.io/modelsummary/
...
def <- defData(varname = "var1", formula = "0.1;0.2;0.4;0.2;0.1;0;0;0;0", dist = "categorical") %>% 
  defData(varname = "var2", formula = "0.1;0.2;0.4;0.2;0.1;0;0;0;0", dist = "categorical") %>% 
  defData(varname = "var3", formula = "0;0;0;0;0.1;0.2;0.4;0.2;0.1", dist = "categorical") %>% 
  defData(varname = "var4", formula = "0.1;0.2;0.4;0.2;0.1;0;0;0;0", dist = "categorical") %>% 
  defData(varname = "var5", formula = "0;0;0;0.1;0.2;0.4;0.2;0.1;0", dist = "categorical") %>% 
  defData(varname = "var6", formula = "0;0.1;0.2;0.4;0.2;0.1;0;0;0", dist = "categorical") %>% 
  defData(varname = "var7", formula = "0;0;0;0.1;0.3;0.3;0.3;0;0", dist = "categorical") %>% 
  defData(varname = "var8", formula = "0.1;0.2;0.4;0.2;0.1;0;0;0;0", dist = "categorical") %>% 
  defData(varname = "var9", formula = "0;0.1;0.1;0.1;0.2;0.1;0.2;0.2;0", dist = "categorical") 

salad_tbl <- genData(rpois(1, 90), def) %>% 
  select(-id) 

ggplot(salad_tbl %>% gather, aes(key, value, fill = key)) +
  geom_boxplot() +
  ylim(c(1,9))
...
t_vec <- map_dbl(1:1000, function(...){
  y1 <- rnorm(10, 5, 1)
  y2 <- rnorm(10, 5, 1)
  t.test(y1, y2)$statistic
})
  
ggplot(tibble(t_vec), aes(x = t_vec)) +
  geom_histogram()
...
https://data.library.virginia.edu/getting-started-with-generalized-estimating-equations/ https://statistikguru.de/lexikon/generalized-estimating-equations-gee.html
...
chicken_tbl %>% 
  ...
  group_by(grp) %>% 
  nest() %>% 
  mutate(lm_fit = map(data, ~lm(log(weight) ~ log(age), data = .x)),
