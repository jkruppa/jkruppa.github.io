...
boxplot_mult_tbl <- read_excel("data/zerforschen_boxplot_mult.xlsx") |> 
  pivot_longer(cols = fresh_weight:flower_number,
    ...
    mutate(trt = as_factor(trt),
           plant_measure = as_factor(plant_measure)) 
```
...
```{r}
p1 <- gg_template %+%
  filter(boxplot_mult_tbl, type == "substrate" & plant_measure == "fresh_weight")
p2 <- gg_template %+%
  filter(boxplot_mult_tbl, type == "substrate" & plant_measure == "dry_weight")
p3 <- gg_template %+%
  filter(boxplot_mult_tbl, type == "substrate" & plant_measure == "plant_height")
p4 <- gg_template %+%
  filter(boxplot_mult_tbl, type == "substrate" & plant_measure == "flower_number")
p5 <- gg_template %+%
  filter(boxplot_mult_tbl, type == "water" & plant_measure == "fresh_weight")
p6 <- gg_template %+%
  filter(boxplot_mult_tbl, type == "water" & plant_measure == "fresh_weight")
p7 <- gg_template %+%
  filter(boxplot_mult_tbl, type == "water" & plant_measure == "fresh_weight")
