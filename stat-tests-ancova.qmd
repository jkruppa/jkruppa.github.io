...
#| fig-height: 5
#| fig-width: 8
#| fig-cap: "Scatterplot des Längenwachstums der drei Gurkensorten über vierzehn Wochen."
...
Im Folgenden schauen wir uns dann die Auswertung der Gurkendaten einmal genauer an. Für mehr Informationen bitte einmal in das Kapitel @sec-posthoc schauen.

::: column-margin
Es gibt hier auch ein weiteres englischsprachiges Tutorium [Don't Ignore Interactions - Unleash the Full Power of Models with {emmeans} R-package](https://yuzar-blog.netlify.app/posts/2022-12-29-emmeans2interactions/)
:::
...
#| message: false
#| warning: false
time_num_rq_fit <- rq(length ~ versuchsgruppe + time_num + versuchsgruppe:time_num, tau = 0.5,
                      gurke_time_len_tbl)
```
...
emmip(time_num_rq_fit, versuchsgruppe ~ time_num, CIs = TRUE, 
      cov.reduce = FALSE) +
  theme_bw() +
  scale_color_okabeito()
```
...
#| message: false
#| warning: false
emmeans(time_num_rq_fit, pairwise ~ versuchsgruppe | time_num, infer  = TRUE,
