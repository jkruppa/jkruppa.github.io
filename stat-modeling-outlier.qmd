...
Eine weitere Möglichkeit Ausreißer zu finden bietet das R Paket `{dlookr}` mit der Funktion `diagnose_outlier()`. Neben dieser Funktion hat das R Paket `{dlookr}` noch eine Vielzahl an weiteren Funktionen, die im Bereich der explorativen Datenanalyse von nutzen sein mögen. Schau dir einfach einmal das R Paket und die Webseite näher an. Das [R Paket `dlookr`](https://choonghyunryu.github.io/dlookr/articles/transformation.html#imputation-of-outliers) hat eine große Auswahl an simplen Algorithmen für die Anpassung von Ausreißern.
