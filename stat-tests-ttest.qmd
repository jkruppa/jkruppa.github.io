...
Wie wir sehen fehlt der Mittelwertsuntschied als Effekt $\Delta$. Wir können den Mittelwertsunterschied selber berechnen oder aber die Funktion `tidy()` aus dem R Paket `broom` nutzen. Da der Funktion `tidy()` kriegen wir die Informationen besser sortiert und einheitich wiedergegeben. Da `tidy` eine Funktion ist, die mit vielen statistischen Tests funktioniert müssen wir wissen was die einzelnen `estimate` sind.
