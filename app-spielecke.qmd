...
![Ursprüngliche Abbildung, die nachgebaut werden soll. Zwei lineare Regressionen mit den jeweiligen Regressionsgleichungen.](images/eda/zerforschen_regression_linear_02.jpeg){#fig-simple-regression fig-align="center" width="100%"}
...
```{r}
shoot_n_fit <- shoot_n_tbl |> 
  lm(freshmass ~ n_in_shoot, data = _)
```
...
shoot_n_tbl |> 
  ...
  theme_minimal() +
  geom_point() +
  geom_function(fun = shoot_n_func, color = "red", linetype = 'dashed') + 
  labs(x = "N in shoot [% DM]", y = TeX(r"(Shoot fresh mass \[g plant$^{-1}$\] - 17 DAP)")) +
  scale_x_continuous(breaks = c(0, 2, 3, 4, 5, 6), limits = c(0, 6)) +
  scale_y_continuous(breaks = c(0, 5, 10, 15, 20, 25), limits = c(0, 25)) +
  annotate("text", x = 2, y = 22, hjust = "left", color = "black", size = 5, 
           label = TeX(r"($y = 4.35 \cdot x - 4.33$)")) +
  annotate("text", x = 2, y = 20, hjust = "left", color = "black", size = 5, 
...
![Ursprüngliche Abbildung, die nachgebaut werden soll. Zwei lineare Regressionen mit den jeweiligen Regressionsgleichungen.](images/eda/zerforschen_regression_03.jpeg){#fig-simple-regression fig-align="center" width="100%"}
...
```{r}
fit <- nls(freshmass ~ b1 * k_in_shoot^2 + b2 * k_in_shoot + c, data = shoot_k_tbl, 
...
```{r}
shoot_k_func <- \(x) {-12.16 * x^2 + 111.95 * x - 172.01}
```
...
#| warning: false
#| message: false
#| label: fig-zerforschen-shoot_k-01
#| fig-align: center
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Visualisierung ."
...
shoot_k_tbl |> 
  ...
  theme_minimal() +
  geom_point() +
  geom_function(fun = shoot_k_func, color = "red", linetype = 'dashed',
                xlim = c(3.8, 6.8)) + 
  labs(x = "K in shoot [% DM]", y = TeX(r"(Shoot fresh mass \[g plant$^{-1}$\] - 34 DAP)")) +
  scale_x_continuous(breaks = c(0, 2, 3, 4, 5, 6, 7, 8), limits = c(3, 7)) +
  scale_y_continuous(breaks = c(0, 20, 40, 60, 80, 100), limits = c(20, 100)) +
  geom_segment(aes(x = 4.6, y = shoot_k_func(4.6), xend = 4.6, yend = 20), linetype = 'dotted',
                  arrow = arrow(length = unit(0.3, "cm"), type = "closed")) +
  annotate("text", x = 3.5, y = 100, hjust = "left", color = "black", size = 5, 
           label = TeX(r"($y = -12.16 \cdot x^2 + 111.95 \cdot x - 172.01$)")) +
  annotate("text", x = 3.5, y = 95, hjust = "left", color = "black", size = 5, 
           label = TeX(r"($R^2 = 0.89$)")) +
    annotate("text", x = 4.8, y = 30, label = "4.4%",
           hjust = "left", size = 5)
```
