...
Dann einmal die Daten anschauen. Ich zeige die hier als `tibble` damit nicht so viel ausgegeben wird.

```{r}
fac2_dt |> as_tibble()
```

Jetzt brauchen wir noch die Definition für unseren poissonverteilten Messwert in `{simstudy}`. Das ist das Gleiche wie schon bevor.
...
Wir haben auch noch die Möglichkeit die Faktoren mit der Funktion `genFactor()` zu modifizieren, aber dann können wir wieder nicht mit den Leveln rechnen. Dann lohnt sich die Sache eigentlich nicht. Für mich hat die `expand_grid()` Variante mehr Charme. Die Variante ist einfahc und schnell.
:::
