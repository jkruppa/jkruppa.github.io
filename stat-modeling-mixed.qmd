...
[Wireworms controlled by fumigants in a latin square](https://kwstat.github.io/agridat/reference/cochran.wireworms.html)
...
```{r}
glmer_quasipoisson_fit <- glmmTMB(count_worms ~ trt + (1|row) + (1|col), 
...
```{r}
lmer(yield ~ nitro * gen + (1|block/gen), data = oats_tbl)
```
