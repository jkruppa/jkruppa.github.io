...
r <- rank(x)    # order of values, i.e. ranks without averaged ties
p <- (r - 0.5) / n      # assign to ranks using Blom's method
...
tibble(x = seq(-3, 3, by = 0.1),
  ...
  theme_minimal() +
  geom_line() +
  geom_segment(data = p_tbl, aes(x = p, xend = p, y = 0, yend = dnorm(p)),
               color = "#0072B2") +
  annotate("text", x = y, y = -0.01, label = round(y, 2), size = 3) 
