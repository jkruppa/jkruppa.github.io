...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Global emissions have not yet peaked"), # <2>
  ...
  sidebarLayout(
    sidebarPanel( 
      ...
      selectInput("method", "Method for line:", # <4>
                  c("Linear" = "lm",
                    "Loess" = "loess",
                    "Gam" = "gam")),
...
2.  Ein neuer Titel wird gesetzt.
3.  Der Slider für die Jahre wird erstellt. Wir wollen immer in 25 Jahre Schritten weitergehen.
4.  Das Dropdownmenü wird gesetzt. Wir haben hier die drei Methoden `lm`, `loess` und `gam` voreingestellt.
5.  Der Button für den Standardfehler ja/nein für den Smoother in der Abbildung.
6.  Unser Plot aus dem `server` wird dann `line_plot` heißen und wird hier wiedergeben.
:::
