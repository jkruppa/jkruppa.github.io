...
```{r}
y_norm_def <- defDataAdd(varname = "y", formula = "10 + 5*f_a + 5*f_b",
                         dist = "normal", variance = 2)
```

```{r}
addColumns(y_norm_def, fac2_dt) |>
  mutate(f_a = factor(f_a, labels = str_c("A.", 1:3)),
         f_b = factor(f_b, labels = str_c("B.", 1:2))) |> 
  ggplot(aes(f_a, color = f_b, y, group = f_b)) +
    stat_summary(fun = "mean", geom = "line", show.legend = FALSE) 
```

```{r}
y_pois_def <- defDataAdd(varname = "y", formula = "10 + 5*f_a + 5*f_b",
                         dist = "poisson")
```
...
y_binary_def <- defDataAdd(varname = "y", formula = "log(0.5)",
