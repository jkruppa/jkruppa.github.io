...
Und natürlich deren Kombination wie `all_nominal_predictors()`

Wir können auch Rollen definieren und diese Rollen dann auch über die Funktion `has_role()` ein- oder auschließen.

Da das Rezept wirklich in der Reihenfolge durchgeführt wird, wie du die einzelen Schritte angibts, empfiehkt sich folgende Reihenfolge. Du musst natürlich nicht jeden dieser Schritte auch immer durchführen.

::: column-margin
Bitte die Hinweise zur Ordnung der Schritte beachten: [Ordering of steps](https://recipes.tidymodels.org/articles/Ordering.html)
:::
...
5)  Eventuell Berücksichtung der Interaktion zwischen Variablen.
...
gummi_full_rec <- gummi_rec %>% 
  step_filter_missing(all_outcomes()) %>% 
