...
Wenn du deine Daten transformierts, dann werden auch die Abstandmaße ähnlicher. Zum Beispiel werden durch die Standardisierung die folgenden Abstandsmaße kelinere Werte ergeben: Euklidisch, Manhattan und die Korrelation. Das ist aber nicht so schlimm, wenn du nicht untransformierte Daten mit transformierten Daten vergleichst.

Wenn du dir die Distanzmatrix anschauen willst, so erlaubt die Funktion `fviz_dist()` aus dem R Paket `factorextra` dir die Distanzmatrix zu visualisieren. Wir nutzen die Distanzmatrix dann aber gleich um auf den Distanzen Cluster zu bilden.
