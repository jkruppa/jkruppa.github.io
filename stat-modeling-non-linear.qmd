...
## Modellieren von Michaelis-Menten

Die Daten in `enzyme.csv` geben die Geschwindigkeit $v$ des Enzyms saure Phosphatase ($\mu mol/min$) bei verschiedenen Konzentrationen des Substrats Nitrophenolphosphat, \[S\] (mM), an. Die Daten können mit der Michaelis-Menten-Gleichung modelliert werden und somit kann eine nichtlineare Regression kann verwendet werden, um $K_M$ und $v_{max}$ zu schätzen.
...
In @tbl-enzyme sehen wir einen Auszug aus den Enzymedaten.
...
Schauen wir uns die Daten einmal in der @fig-nls-04 an.
