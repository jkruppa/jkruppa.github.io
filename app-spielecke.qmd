...
## Mixed model addition {.unnumbered}

[Chapter 9 Random Effects](https://bookdown.org/steve_midway/DAR/random-effects.html)
...
p4 <- tibble(x = c(2, 3, 5, 7, 8,
  ...
  scale_y_continuous(limits = c(-5, 15)) +
  scale_color_okabeito() +
  labs(title = "Fixedm intercept, fixed slope",
       subtitle = "y ~ x") +
...
## Tukey HDS {.unnumbered}

https://www.mathsjournal.com/pdf/2021/vol6issue1/PartA/5-6-32-656.pdf

https://loonylabs.org/2021/03/14/the-tukey-test-in-statistics/

http://www.statisticslectures.com/topics/posthoconewayanova/

http://www.statoptima.com.ua/2023/04/18/tukeys-range-test/

https://sciences.usca.edu/biology/zelmer/305/anova/novass/mult/

Nochmal auf aov() eingehen...
...
Das [R Paket `{WRS2}`](https://cran.r-project.org/web/packages/WRS2/vignettes/WRS2.pdf) von @mair2020robust erlaubt es auch bei nicht normalverteilten Daten sowie Varianzheterogenität eine ANOVA zu rechnen.

Hier noch ein Satz zum getrimmten Mittelwert.

$\hat{\Psi}$ getrimmte Mittelwertsdifferenzen `psihat`
...
```{r}
#lincon(jump_length ~ animal, data = fac1_tbl) 
```

```{r}
#med1way(jump_length ~ animal, data = fac1_tbl) 
```
...
```{r}
#foo <- mcp2atm(jump_length ~ animal*site, data = fac2_tbl)

#foo$effects
```

```{r}
#med2way(jump_length ~ animal*site, data = fac2_tbl)
```
...
[‘S-values’, ‘p-values’ and conceptualising statistics](https://corplingstats.wordpress.com/2020/11/02/s-values/)

@rothman2021rothman [Rothman Responds to “Surprise!”](https://academic.oup.com/aje/article/190/2/194/5869592?login=false)

@mansournia2022p [P-value, compatibility, and S-value](https://www.sciencedirect.com/science/article/pii/S2590113322000153#bi0005)

@shafer2021testing [Testing by Betting: A Strategy for Statistical and Scientific Communication](https://academic.oup.com/jrsssa/article/184/2/407/7056412?login=false#396831406)

@cole2021surprise [Surprise!](https://academic.oup.com/aje/article/190/2/191/5869593?login=false)

@rafi2020semantic [Semantic and cognitive tools to aid statistical science: replace confidence and significance by compatibility and surprise](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/s12874-020-01105-9)
