...
[The unreasonable effectiveness of public work](https://posit.co/resources/videos/the-unreasonable-effectiveness-of-public-work/)
...
model <- lm(plant_yield ~ nitro + gen + nitro:gen + block, data = oats_tbl)
...
school_class_grid_tbl <- map2_dfr(1:n_school, rpoisnonzero(n_school, 3), 
                                  \(i, y) expand_grid(school_grid_tbl[i,], c_per_s = 1:y)) %>% 
  mutate(c_0 = rnorm(n(), 0, sd_class),
         c_id = 1:n())


school_tbl <- school_class_grid_tbl %>% 
  mutate(trt = rep_len(1:n_trt, length.out = n()),
         t_eff = rep_len(eff_trt, length.out = n())) %>% 
