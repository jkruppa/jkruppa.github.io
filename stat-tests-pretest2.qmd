...
Kommen wir nun zum etwas kontroversen Teil. Der statistischen Überprüfung der Varianzhomogenität oder aber auch der Normalverteilung. Die folgenden Überlegungen stimmen aber im Prinzip auch für andere Vortest auf andere statistische Eigenschaften von Daten.

::: {layout="[15,85]" layout-valign="top"}
![](images/angel_01_small.png){fig-align="center" width="100%"}

> Die folgenden Betrachtungen sind statistisch estwas schief und semantisch falsch. Aber ich nutze jetzt mal die Umgangssprache um die Sachlage besser verständlich zu machen. Ja, wir können nur Nullhypothesen ablehnen und nichts "erkennen", aber darum geht es hier nicht. Klassisches [lying-to-children](https://en.wikipedia.org/wiki/Lie-to-children) was ich hier betreibe.
:::
...
levene_sim_tbl <- map_dfr(c(3,4,5,6,7,8,9,11,12), \(n_val){
  map_dfr(1:1000, \(...){
    ...
    group_by(n) |> 
    dplyr::summarise(rate = mean(levene))
})
```
...
shapiro_sim_tbl <- map_dfr(c(3,4,5,6,7,8,9,11,12), \(n_val){
  map_dfr(1:1000, \(...){
    sim_tbl <- tibble(y = c(rchisq(n_val, 5), rchisq(n_val, 2), rchisq(n_val, 10)),
                      x = gl(3, n_val, labels = c("A", "B", "C")))
    shapiro_p <- sim_tbl |> pull(y) |>  
      shapiro.test() |> pluck("p.value")  
    tibble(n = n_val,
           shapiro = shapiro_p) |> 
      mutate(shapiro = shapiro <= 0.05)
  }) |> 
    group_by(n) |> 
    dplyr::summarise(rate = mean(shapiro))
})
```
...
#| message: false
#| warning: false
#| label: fig-levene-sim-pretest-01
#| fig-align: center
#| fig-height: 5
#| fig-width: 9.25
#| fig-cap: "Simulationsstudie zur Erkennung von einem nicht normalverteilten Messwert und Varianzheterogenität zwischen drei Gruppen mit unterschiedlichen Varianzen. Auf der y-Achse ist der Anteil der erkannten Nichtnormalverteilung sowie Varianzheterogenität in den 1000 Simulationen angegeben. Auf der x-Achse sind die Fallzahlen per Gruppe dargestellt. *[Zum Vergrößern anklicken]*"

p1_levene_sim <- levene_sim_tbl |> 
  ggplot(aes(n, rate)) +
  theme_minimal() +
  geom_smooth(method = "lm", se = FALSE, color = "#E69F00") +
  geom_point() +
  labs(x = "Fallzahl in der Gruppe", y = "Erkannte Varianzheterogenität",
       title = "Varianz heterogene Gruppen", 
       subtitle = "Erkennung mit dem Levene Test",
...
p2_shapiro_sim <- shapiro_sim_tbl |> 
  ggplot(aes(n, rate)) +
  theme_minimal() +
  geom_smooth(method = "lm", se = FALSE, color = "#E69F00",
              formula = y ~ log(x)) +
  ...
  labs(x = "Fallzahl in der Gruppe", y = "Erkannte Nichtnormalverteilung",
       ...
       subtitle = "Erkennung mit dem Shapiro-Wilk Test",
       caption = expression(n[sim]~"="~1000)) +
  scale_y_continuous(breaks = c(0, 0.1, 0.25, 0.5, 0.75, 1),
                     limits = c(0,1),
                     labels = scales::percent_format(accuracy = 1)) +
