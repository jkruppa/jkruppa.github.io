...
**Dieses Kapitel wird in den Wintersemesterferien 2023/2024 grundlegend überarbeitet. Daher können Teile des Codes und des Textes kurzfristig keinen Sinn ergeben oder nicht funktional sein. Es ist geplant eine fertige Version im März 2024 vorliegen zuhaben.**
...
https://www.bookdown.org/rwnahhas/RMPH/mlr-residuals.html

Standardisierte Residuen (eng. *standardized residuals*)

:   ... sind Residuen, die durch eine Schätzung ihrer Standardabweichung geteilt werden, mit dem Ergebnis, dass sie unabhängig von der Skala des Ergebnisses eine Standardabweichung sehr nahe bei 1 haben.

Studentisierte Residuen (eng. *studentized residuals*)

:   ... sind den standardisierten Residuen ähnlich, außer dass für jeden Fall das Residuum durch die Standardabweichung geteilt wird, die aus der Regression ohne diese Beobachtung geschätzt wurde.

Beide Arten von Residuen können wir mit der Standardnormalverteilung vergleichen. Ein standardisiertes Residuum von 2 entspricht zum Beispiel einem Punkt, der 2 Standardabweichungen über der Regressionslinie liegt.
...
ggplot(resid_tbl, aes(value, fct_rev(key))) +
  theme_minimal() +
  stat_halfeye() +
  labs(y = "", x = "Residuen")
```
