...
```{r}
model_grp_sq <- lm(jump_length ~ weight + I(weight^2) + stage,
                   data = flea_model_tbl)
tidy(model_grp_sq)
```
...
model_grp_sq |> 
  slopes(variables = "weight",
         by = "stage") |> 
  select(term, contrast, stage, estimate, std.error, p.value, s.value, conf.low, conf.high)
```
:::
...
# predict(model_grp_sq, newdata = tibble(weight = 5.85, stage = "juvenile"))
...
ggplot(flea_model_tbl, aes(x = weight, y = jump_length,
  ...
  labs(x = "Gewicht in [mg]", y = "Sprungweite in [cm]",
       title = "Quadratischer Zusammenhang") +
  geom_abline(intercept = find_intercept(14.05, 76.58768, 0.671), slope = 0.671,
              linewidth = 0.5, color = cb_pal[8], linetype = "21") +
  geom_abline(intercept = find_intercept(5.85, 58.69993, 0.111), slope = 0.111,
              linewidth = 0.5, color = cb_pal[7], linetype = "21") +
  geom_line(aes(y = predict(model_grp_sq)), linewidth = 1.25,
            fullrange = TRUE) +
  annotate("point", x = 14.05, y = 76.58768, size = 4, shape = 18, 
           color = cb_pal[9]) +
  annotate("point", x = 5.85, y = 58.69993, size = 4, shape = 18, 
           color = cb_pal[9]) +
  geom_richtext(aes(x = 14.05, y = 82, label = "Steigung adult: 0.67"),
                site = 20, color = "black") +
  geom_richtext(aes(x = 5.85, y = 64, label = "Steigung juvenile: 0.11"),
                site = 20, color = "black") +
  scale_color_okabeito()
```
