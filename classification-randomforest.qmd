...
*Letzte Änderung am `r format(fs::file_info("classification-randomforest.qmd")$modification_time, '%d. %B %Y um %H:%M:%S')`*
...
```{r echo = TRUE}
#| message: false
pacman::p_load(tidyverse, tidymodels, magrittr, vip,
...
Leider wurde die Unterstützung von `{vip}` über CRAN eingestellt, so dass ich mich hier mit der Krücke über GitHub helfe. Mit ist es zu viel Arbeit alles umzuschreiben und die Abbildungen dienen ja der Veranschaulichung. Reine Boxplots würden ja auch reichen.

```{r}
devtools::install_github("cran/vip")
```
