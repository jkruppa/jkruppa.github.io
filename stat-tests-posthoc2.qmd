...
![*"Panicles with commercial fruit (%) at 2019 and 2020 harvest. In the same year, different letters indicate significant differences among treatments (Tukey's test p\<0.05)."* Quelle: @sanchez2022hoverfly](images/eda/zerforschen_hoverfly.png){#fig-zerforschen-hoverfly fig-align="center" width="100%"}
...
Fangen wir nochmal mit etwas theoretischen Hintergrund an.
...
Wenn wir von einem liberalen Test sprechen, dann testen wir zu einem höheren Signifikanzniveau als $\alpha$ gleich 5%. Wir werden also mehr signifikante Unterschiede finden als wir mit einer globalen Fehlerrate von 5% erwarten würden.
...
Wir vergleichen natürlich nicht die Sprungweiten in \[cm\] direkt mitteinandern sondern rechnen natürlich vorher die deskriptive Statistiken wie Mittelwert und Standardabweichung aus. Für den Standardfehler nutzen wir die Funktion `standard_error()` aus dem R Paket `{parameters}`. Am Ende vergleichen wir die mittleren Sprungweiten der Hunde-, Katzen-, und Fuchsflöhe miteinander.
...
#| warning: false
#| label: tbl-1fac-mean
#| tbl-cap: "Deskriptive Maßzahlen der Sprungweite in [cm] groupiert nach den drei Floharten."
fac1_tbl |> 
  group_by(animal) |> 
...
Einfaktorielles Modell (eng. *one-way design*)
...
Einfaktorielles Modell (eng. *two-way design*)
...
Das tolle an `{emmeans}` ist, dass wenn du die Option für die Adjustierung auf `tukey` stellst faktisch einen besseren Tukey HSD Test rechnest. Wenn du dann noch in `{emmeans}` die Option mit der Varianzheterogenität wählst, rechnest du einen besseren Games Howell Test. Mehr dazu dann am Ende des Abschnitts, wie du die Ergebnisse präsentierst.
:::
...
Wir können uns auch die Mittelwerte anschauen, dann müssen wir aber nochmal in der Funktion adjustieren.
...
Hier wollen wir dann die Mittelwerte vergleichen und nicht die Mittelwertdifferenzen.
...
```{r}
emm_contrast_fac2_combined_mat <- emm_contrast_fac2_combined_obj |>   
  pwpm()
```

```{r}
emm_contrast_fac2_combined_mat |> 
  write.csv2("data/emm_contrast_fac2_combined_mat.csv")
```

```{r}
emm_contrast_fac2_combined_heat_mat <- emm_contrast_fac2_combined_obj |>   
  pwpm(flip = TRUE, reverse = TRUE, means = FALSE)


p.val.test<-sub("[<>]", "", emm_contrast_fac2_combined_heat_mat)
...
p.matx |> pheatmap::pheatmap(cluster_rows=FALSE, cluster_cols=FALSE,
...
emm_ci_fac2_combined_obj <- emm_fac2_combinded_obj |> 
  ...
  confint(adjust = "bonferroni") |> 
  as_tibble() |> 
  select(contrast, estimate, conf.low = lower.CL, conf.high = upper.CL) 
```
...
Das tolle an `{emmeans}`

Die Analyse wurde in RStudio (Version 2024.12.0.467) und R (Version 4.2.3) gerechnet. Die Gruppenvergleiche wurden mit einem Tukey HSD Test mit dem R Paket emmeans (Version 1.10.0) gerechnet.
...
## Fallbeispiel mit `{emmeans}`

Hier kommt dann zerforschen rein
...
Am Ende wollen wir dann doch noch das *Compact letter display* haben. Ich wähle hier einmal die Gruppe der Tierarten aus. Du kannst das auch für die anderen Faktoren machen, aber hier einmal zur Demonstration. Wir sehen, dass sich alle Tierarten in der Sprungweite unterscheiden, keine Tierart hat den gleichen Buchstaben.
