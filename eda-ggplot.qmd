...
[Wie du die Gerade richtig mit den statistischen Maßzahlen beschriftest findest du dann im @sec-modeling-simple-stat zur linearen Regression.]{.aside}

@fig-scatter-flea-0 zeigt den Scatterplot für die Spalte `flea_count` auf der x-Achse und `jump_length` auf der y-Achse. Mit der Funktion `geom_point()` können wir die Punktepaare für jede Beobachtung zeichnen. In unserem Fall zeichnen wir mit der Funktion `stat_smooth()` noch die entsprechende Grade durch die Punkte. Es handelt sich hierbei um eine Regression. Du kannst im @sec-modeling-simple-stat mehr darüber erfahren.
