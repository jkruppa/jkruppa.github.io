...
p33 <- gg_template %+%
  filter(cross_tbl, x_fct != "C" & y_beta >= 0) + 
  aes(x_fct, y_beta, fill = x_fct) +
  geom_boxplot(width = 0.15, outlier.shape = NA) +
  stat_dots(side = "left", justification = 1.12, binwidth = 0.005,
            dotsize = 5) +
...
p41 <- gg_template %+%
  ...
  aes(x_num, y_grad) +
  geom_point() +
  scale_y_continuous(breaks = c(0, 1, 2), labels = c(1, 2, 3))

p42 <- gg_template %+%
  filter(cross_tbl, y_grad >= 0) + 
  geom_mosaic(aes(x = product(y_grad, x_fct), fill = x_fct)) 
...
p52 <- gg_template %+%
  filter(cross_tbl, y_binom >= 0) + 
  geom_mosaic(aes(x = product(y_binom, x_fct), fill = x_fct)) 
