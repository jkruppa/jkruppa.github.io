...
In diesem Kapitel werden wir *nicht* die Implementierung in dem R Paket `{nlme}` verwenden. Das Paket `{nlme}` hat sinnvolle Funktionen und je nach Fragestellung haben diese auch eine Berechtigung. Es gibt aber neuere R Pakete wie `{lme4}` oder `{glmmTMB}`, die wir dann hier in dem Kapitel nutzen wollen.
:::
...
[R Paket `{agridat}`](https://kwstat.github.io/agridat/reference/index.html)
...
pacman::p_load(tidyverse, magrittr, broom, see, simstudy,
               ...
               mixedup, multilevelmod, conflicted)
...
Dann können wir uns schon das Grid für die Daten erstellen.
...
Jetzt noch den Testscore simulieren.
...
## Einfachst möglicher Datensatz (3-faktoriell)
...
ggplot(dragons_tbl, aes(x = body_length_cat, y = test_score, color = mountain)) +
  stat_halfeye(adjust = 0.5, width = 0.4, .width = 0, 
    justification = -0.3, point_colour = NA) + 
  geom_boxplot(width = 0.15, outlier.shape = NA, 
