...
Dann können wir usn auch schon einmal die Abbildungen unseres Datensatzes zu den Enzymaktivität anschauen.
...
### Kovariates oder kombiniertes Modell
...
Jetzt wollen wir uns einmal ein etwas kompliziertes Beispiel mit den Flohdaten anschauen. Unsere Flohdaten haben insgesamt `r nrow(flea_model_tbl)` Beobachtungen. Das brauchen wir gleich einmal damit auch immer sehen, wie die Daten zusammengefasst werden. Erstmal bauen wir uns wieder unser *Generalized additive model* für die Daten. Wir müssen nur einmal über das `s()` angeben, welche Einflussvariablen nicht linear sein sollen. In unserem Fall sind es die Schlupfzeiten und die Körpergewichte der Flöhe. Dann nehmen wir den Entwicklungsstand als Faktor noch mit ins Modell.

```{r}
#| message: false
#| warning: false
cov2_fac1_fit <- gam(jump_length ~ s(hatched) + s(weight) + stage, data = flea_model_tbl)
```
...
#| message: false
#| warning: false
predictions(cov2_fac1_fit, by = "weight")
```
...
#| message: false
#| warning: false
predictions(cov2_fac1_fit, variables = lst(hatched = fivenum))
```
...
#| message: false
#| warning: false
predictions(cov2_fac1_fit, by = c("stage", "weight"))
```
:::

::: panel-tabset
## Interessantes Raster

```{r}
predictions(cov2_fac1_fit,
  variables = lst(weight = "threenum"),
  newdata = datagrid(stage = unique)
)
```

## Repräsentatives Raster
