...
Im Folgenden wollen wir uns einmal in der Theorie den Levene-Test anschauen. Dafür nutzen wir als erstes die folgende vereinfachte, händische Formel um die Teststatistik zu berechnen. Dabei ist $W$ die Teststatistik, die wir zu einer $F$-Verteilung, die wir schon aus der ANOVA kennen, vergleichen können.
...
Am Ende fehlt uns dann noch der Nenner mit der Summe der einzelnen Abstände $Z_{ij}$ zu den Abstandsmitteln der einzelnen Gruppen $\bar{Z}_{i\cdot}$ mit $\sum_{i=1}^k \sum_{j=1}^{N_i} (Z_{ij}-\bar{Z}_{i\cdot})^2$. Den Wert können wir dann in R direkt einmal berechnen. Wir nehmen also die Vektoren der Einzelwerte und ziehen dort immer den Mittelwert der Abstände der Gruppe ab. Abschließend summieren wir dann einmal auf.
...
Wir können dann alle Zahlen einmal zusammenbringen und in die Formel des Levene-Test einsetzen.
...
Wir erhalten ein $W = 1.69$, was wir direkt als eine F-Statistik interpretieren können. Schauen wir uns das Ergegebnis einmal in der R Funktion `leveneTest()` aus dem R Paket `car` an. Wir brauchen dafür einmal die Werte für die Sprungweiten und müssen dann die Daten in das long-Format umbauen. Dann rechnen wir den Levene-Test. Wie meistens erhalten wir einen etwas anderen Wert in R als in der vereinfachten, händischen Formel. So weit sind die beiden Werte mit $0.97$ und $1.69$ aber nicht auseinander.
...
## Testen der Normalverteilung in mehrern Gruppen
