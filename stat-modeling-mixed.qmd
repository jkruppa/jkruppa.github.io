...
::: callout-tip
## Anwendungsbeispiel: Gruppenvergleich für Boniturnoten in Brokkoli (2-faktoriell)

Dieses Beispiel ist die etwas angepasste und verkürzte Form des [Gruppenvergleich für die ordinale Regression](#sec-mult-comp-ord-reg) in dem Kapitel zur ordinalen Regression. Ebenfalls hilfreich ist das Tutorium [Ordinal regression in R: part 1](https://tdunn.ca/posts/2020-03-15-ordinal-regression-in-r-part-1/) auf das ich gerne verweisen möchte. Im folgenden Beispiel schauen wir uns eine praktische Auswertung von einem agrarwissenschaftlichen Beispiel mit Brokkoli an. Wir haben uns in diesem Experiment verschiedene Dosen `fert_amount` von einem Dünger aufgebracht sowie verschiedene Zeitpunkte der Düngung `fert_time` berücksichtigt. Ziel ist es die Boniturnoten für den Stamm von Brokkoli miteinander zu vergleichen. Auch hier haben wir einige Besonderheiten in den Daten, da nicht jede Faktorkombination vorliegt. Wir ignorieren aber diese Probleme und rechnen einfach stumpf unseren Gruppenvergleich. Wir müssen aber ein paar Anpassungen durchführen. Unsere Noten müssen ein geordneter Faktor sein, daher nutzen wir die Funktion `ordered`. Darüber hinaus schmeißen wir die Düngerzeit `early` aus den Daten, da wir zu dem Zeitpunkt keine hohlen Stämme bonitiert haben. Ebenfalls haben wir sehr viele Brokkoliköpfe mit Eins bonitiert. Auch diese Beobachtungen entfernen wir einmal aus den Daten damit wir dann auch ein paar Effekte sehen.
...
broc_tbl <- read_excel("data/broccoli_weight.xlsx") %>% 
  ...
  filter(stem_hollowness > 1) %>% 
  select(fert_time, fert_amount, block, stem_hollowness, stem_hollowness_num) %>% 
  droplevels() %>% 
  na.omit()
```

Im Folgenden einmal die Tabelle mit `tabyl` aus dem R Paket `{janitor}`. Mehr dazu auf der Hilfeseite zu [tabyls: a tidy, fully-featured approach to counting things](https://cran.r-project.org/web/packages/janitor/vignettes/tabyls.html). Wir sehen, dass wir kaum schlechte Noten erhalten.
...
Die Tabelle sehen wir dann auch nochmal in der @fig-clm-broc-violin als Violinplot visualisiert. Die Brokkoliköpfe mit der Note Eins fehlen, so dass wir hier sogar einen leichten Effekt von der Düngung mit $300$ sehen könnten. Die Düngestufe könnte etwas höhere Noten haben.
...
Wir rechnen jetzt natürlich trotzdem eine ordinale Regression mit der Funktion `clmm()`. Da wir nur einen Behandlungsfaktor mit der Düngung vorliegen haben bleibt noch der Block als zufälligen Faktor übrig. Wir schreiben das Modell wie wir es schon kennen mit einem festen und einem zufälligen Effekt.

```{r}
clmm_fit <- clmm(stem_hollowness ~ fert_amount + (1 | block), 
                 data = broc_tbl)
```
...
Wir sehen, dass der die Menge des Düngers nicht signifikant ist. Wir haben nicht mehr Faktoren in dem Modell gehabt, so dass wir hier auch keine weiteren Aussagen tätigen können. Wir rechnen trotzdem den multiplen Vergleich, damit wir das dann auch gemacht haben.
...
In der @fig-log-mod-ordinal-broc siehst du das Ergebnis der Auswertung in einem Säulendiagramm. In unserem Fall sind die Düngestufen nicht signifikant. Hier ist dann die Varianz zu groß um den Unterschied zwischen der Düngestufe $150$ und $300$ signifikant zu zeigen. Vermutlich wäre es mit der größeren Fallzahl schon wieder anders, aber wir haben aber auch nur eine begrenzte Anzahl an Beobachtungen mit einer Note größer als Eins vorliegen. Ich habe auch darauf verzichtet die ganze Notenskala von 1 bis 9 zu zeigen, dann sieht man nämlich gar nichts mehr. Würde man auch so nicht, aber hier haben wir dann eine etwas bessere Auflösung.
...
#| fig-height: 4
#| fig-width: 4
#| fig-cap: "Säulendigramm der mittleren Boniturnote des Brokkoli aus einer ordinalen Regression. Das `clmm()`-Modell berechnet das mittlere Boniturnote für den Faktor `fert_amount`. Das *compact letter display* wird dann in `{emmeans}` generiert."
