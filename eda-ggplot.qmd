...
Du findest dann auch weitere Optionen in `{ggplot}` im [Kapitel zum `{ggplot}` Cookbook](#sec-ggplot-cookbook).
...
## Stand des Abschnitts: Baustelle (seit 05.2025)

Hier entsteht etwas Neues, was ich toll finde aber noch ein paar Wochen für brauche. Es wird ja überall hier gewerkelt. Deshalb funktioniert alles natürlich noch, aber ich möchte eben auch das R Paket `{tidyplot}` hier im Kapitel mit ergänzen.
:::
...
#| fig-height: 4
#| fig-width: 6
#| fig-cap: "Leere tidyplot() Leinwand mit den Spalten `animal` und `jump_length` aus dem Datensatz `fac1-tbl`. *[Zum Vergrößern anklicken]*"

tidyplot(data = fac1_tbl, 
         x = animal , y = jump_length)
```

Du findest dann auch weitere Optionen in `{tidyplot}` im [Kapitel zum `{ggplot}` Cookbook](#sec-ggplot-cookbook).
