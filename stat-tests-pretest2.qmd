...
ggplot(data.frame(x = c(-3.25, 3.25)), aes(x)) +
  ...
  stat_function(fun = dnorm, xlim = c(-1.6, 1.6),
                geom = "line", fill = "#E69F00", alpha = 0.25)

ggplot(data.frame(x = c(-5.25, 5.25)), aes(x)) +
  theme_minimal() +
  geom_vline(xintercept = 0) +
...
ggplot(data.frame(x = c(0, 7.25)), aes(x)) +
  theme_minimal() +
  geom_vline(xintercept = 0) +
...
```{r}
shapiro.test(norm_stat_tbl$value)
shapiro.test(t_stat_tbl$value)
shapiro.test(chi_stat_tbl$value)
```
