...
pacman::p_load(tidyverse, magrittr, broom, nlraa, modelsummary,
               parameters, performance, see, mgcv, mfp, 
               conflicted)
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Visualisierung der Hühnchengewichte nach Alter in Tagen mit der geschätzen nicht-linearen Regressionsgleichung nach der Funktion `mfp()`."
#| fig-subcap: 
#|   - "... mit `geom_function()`"
...
ggplot(chicken_tbl, aes(age, weight)) +
  geom_function(fun = age_func, color = "red", linetype = 'dashed') +
  geom_point() +
  theme_bw()

ggplot(chicken_tbl, aes(age, weight)) +
  geom_line(aes(y = predict(mfp_fit)), size = 1, color = "blue") +
  geom_point() +
  theme_bw()
```

Und welche der beiden mathematischen Gleichungen ist denn nun besser? Dafür fitten wir nochmal das `nls()` Modell von oben und vergleichen das Modell einmal zu dem `mfp()`-Modell.
...
In der @tbl-model-comp-nls-mfp sehen wir die Funktion `modelsummary()` und den entsprechenden Modellvergleich der beiden Modelle zu Anpassung einer nicht-linearen Regression. Welche der beiden Modelle und damit mathematischen Gleichungen beschreibt unsere Daten besser?
...
Und am Ende nochmal, wie gut war die Anpassung des Modells an die Daten eigentlich? Hier können wir dann wieder das Bestimmtheitsmaß $R^2$ nutzen. Wie immer siehst du Bestimmtheitsmaß $R^2$ auch in der `summary()` aber hier dann einmal als direkter Aufruf.
...
[GAM and LOESS smoothing](https://andrewirwin.github.io/data-visualization/smoothing.html) [Local Regression (LOESS)](https://thomasleeper.com/Rcourse/Tutorials/localregression.html)

```{r}
loess_fit <- loess(weight ~ age, data = chicken_tbl)
```
...
ggplot(chicken_tbl, aes(age, weight)) +
  geom_line(aes(y = predict(loess_fit)), size = 1, color = "red") +
  geom_point() +
  theme_bw()
```
...
```{r}
predict(loess_fit, newdata = test_tbl)
```

::: callout-note
## Gam und Loess in `{ggplot}`

Natürlich können wir auch einfach eine GAM oder Loess-Funktion in `ggplot()` direkt anwenden ohne über zusätzliche Pakete oder Funktionen zu gehen.
