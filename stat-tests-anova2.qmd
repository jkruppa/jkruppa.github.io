...
pacman::p_load(tidyverse, readxl, knitr, kableExtra, Hmisc, plyr,
               patchwork, ggforce, see, conflicted)
set.seed(202434)
...
foo_tbl <- tibble(rsp = c(1,2,3, 7,8,9, 10,11,12),
...
```{r}
lm(rsp ~ fa, foo_tbl, contrasts = list(fa = "contr.sum")) |> 
  coef() |> round(2)
```
...
```{r}
lm(rsp ~ fa, foo_tbl, contrasts = list(fa = "contr.treatment")) |> 
  coef() |> round(2)
```
:::
...
#| echo: false
#| warning: false
#| label: fig-ggplot-anova-effect-coding
#| fig-align: center
#| fig-height: 4
#| fig-width: 4
#| fig-cap: "Visualisierung *[Zum Vergrößern anklicken]*"
...
stat_tbl |> 
  ...
  theme_minimal() +
  annotate("rect", xmin = 1-0.15, xmax = 1+0.15, ymin = 0, ymax = Inf,
           alpha = 0.25, fill = "gray") +
    annotate("rect", xmin = 2-0.15, xmax = 2+0.15, ymin = 0, ymax = Inf,
           alpha = 0.25, fill = "gray") +
    annotate("rect", xmin = 3-0.15, xmax = 3+0.15, ymin = 0, ymax = Inf,
           alpha = 0.25, fill = "gray") +
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 7, color = "#CC79A7") +
  geom_hline(yintercept = 0, color = "gray50") +
  geom_vline(xintercept = 1-0.2, color = "gray50") +
  scale_y_continuous(limits = c(0, NA), 
                     breaks = c(0, 7, stat_tbl$mean)) +
  scale_x_continuous(breaks = c(1, 2, 3), labels = c("A.1", "A.2", "A.3")) +
...
#           alpha = 0.25, fill = "gray") +
  ...
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 2, color = "#CC79A7") +
  geom_hline(yintercept = 0, color = "gray50") +
  geom_vline(xintercept = 1, color = "gray50") +
  scale_y_continuous(limits = c(0, NA), 
                     breaks = c(0, 2, stat_tbl$mean)) +
  scale_x_continuous(breaks = c(1, 2, 3), labels = c("A.1", "A.2", "A.3")) +
...
stat_tbl |> 
  ...
  geom_point() +
  geom_line() +
  geom_hline(yintercept = mean(foo_tbl$rsp), color = "#CC79A7") +
  scale_y_continuous(limits = c(0, NA), 
                     breaks = c(0, mean(foo_tbl$rsp), stat_tbl$mean)) +
  scale_x_continuous(breaks = c(1, 2, 3), labels = c("A.1", "A.2", "A.3")) +
  geom_segment(x = 1, y = 2, xend = 1, yend = 7, color = "#E69F00",
               linewidth = 0.5, linetype = 2) +
  annotate("label", x = 1.125, y = (2+7)/2, label = "-5", size = 4,
...
foo_tbl <- tibble(rsp = c(20,21,22, 15,16,17, 5,6,7, 
                          3,4,5, 9,10,11, 14,15,16),
                  fa = rep(c("A.1", "A.2", "A.3"), each = 3, times = 2),
                  fb = rep(c("B.1", "B.2"), each = 9))
mean(foo_tbl$rsp)
...
lm(rsp ~ fa + fb + fa:fb, foo_tbl, contrasts = list(fa = "contr.sum", fb = "contr.sum")) |> 
  coef() |> round(2)

stat_tbl |> 
  ggplot(aes(x = pos, y = mean, color = fb)) +
  theme_minimal() +
  geom_point() +
  geom_line() +
  geom_hline(yintercept = mean(foo_tbl$rsp)) +
  scale_y_continuous(limits = c(0, NA), breaks = c(0, mean(foo_tbl$rsp), stat_tbl$mean)) +
  scale_x_continuous(breaks = c(1.5, 3.5, 5.5), labels = c("A.1", "A.2", "A.3")) 
...
Wenn um die Anwendung der ANOVA in R geht, dann haben wir eine Menge Pakete zur Auswahl. Wie immer macht die Fragestellung und das gewählte Modell den Großteil der Entscheidungsindung aus. Ich zeige dir später in der Anwendung dann auch alle Pakete einmal, gebe dir dann aber auch immer eine Empfehlung mit.
...
pacman::p_load(tidyverse, magrittr, broom, WRS2, scales,
               ...
               interactions, effectsize, afex, conflicted)
conflicts_prefer(dplyr::mutate)
conflicts_prefer(dplyr::summarize)
