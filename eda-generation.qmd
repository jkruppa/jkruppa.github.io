...
```{r}
base_prob <- c(0.1, 0.2, 0.4, 0.2, 0.1)
eff_prob <- c(0.05, 0.1, 0.3, 0.4, 0.15)
```

```{r}
ord_fac1_tbl <- tibble("1" = sample(1:5, 100, replace = TRUE, prob = base_prob),
                       "2" = sample(1:5, 100, replace = TRUE, prob = eff_prob))  |> 
  gather(key = "f_a", value = "y")
```
...
ggplot(ord_fac1_tbl, aes(x = y, fill = f_a)) +
  ...
  geom_density(alpha = 0.5) +
  scale_fill_okabeito() +
  scale_x_continuous(labels = c("trifft gar nicht zu", "trifft nicht zu", "weder noch",
                                 "trifft zu", "trifft voll zu"),
                     guide = guide_axis(n.dodge = 2))
```
