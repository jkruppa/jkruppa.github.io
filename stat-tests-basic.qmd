...
| H$_0$ ablehnen | $T_{D} \geq T_{\alpha = 5\%}$ | $Pr(\geq T_{D}| H_0) \leq \alpha$ | $\Delta_{A-B}$: enthält [*nicht*]{.underline} **0** |
...
Nun berechnen wir die Mittelwerte und die Standardabweichungen aus der obigen Datentabelle für die Sprungweiten getrennt für die Hunde- und Katzenflöhe. Die Werte setzen wir dann in die Formel ein und berechnen die Teststatistik $T_{D}$ aus unseren experiementellen Daten $D$.
...
ggplot(data.frame(x = c(-3.25, 3.25)), aes(x)) +
  ...
  labs(x = "", y = "",
       ...
       subtitle = expression(Die~berechnete~Teststatistik~T[D]~ist~größer~als~der~kritische~Wert~T[k]))
...
|                   | $T_{D}$ | $Pr(\geq T_{D}|H_0)$ | $KI_{1-\alpha}$ |                     | $T_{D}$ | $Pr(\geq T_{D}|H_0)$ | $KI_{1-\alpha}$ |
|:-----------------:|:-------:|:--------------------:|-----------------|:-------------------:|:-------:|:--------------------:|-----------------|
