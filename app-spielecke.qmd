...
![Auszug aus dem Ergebnis zu der Kohlenhydrat- und Fatverbrennung aus der Arbeit von @venables2008green. Auch hier leigt der Fokus auf der Veränderung von Placebo zu Grünen Teeextrakt.](images/eda/zerforschen_green_tea_effect_03.png){#fig-zer-green-tea-03 fig-align="center" width="80%"}

![Die Abbildung 1.A (eng. *FIGURE 1.A*) aus der Arbeit von @venables2008green mit der ein Unterschied in der Fettverbrennung von 17% von Placebo zu Grünen Teeextrakt gezeigt werden soll. Der Text schreibt von einem Unterschied in der Einheit $g/min$. In der Abbildung werden als Einheit $kJ/min$ angegeben. Entweder ist die Referenz nicht korrekt oder aber die Umrechung nicht nachzuvollziehen. Fett hat einen Energiegehalt von ca. $37kj/g$. Abkürzungen: *energy expenditure (EE)*; *total energy expenditure (TEE)*.](images/eda/zerforschen_green_tea_effect_02.png){#fig-zer-green-tea-02 fig-align="center" width="100%"}

[*Thus, for example, fats and ethanol have the greatest amount of food energy per unit mass, 37 and 29 kJ/g (9 and 7 kcal/g), respectively*](https://en.wikipedia.org/wiki/Food_energy)

Ich habe den Datensatz etwas anders gebaut, damit ich die Standardabweichung nutzen kann und nicht den Standardfehler. Da ich sowieso nur drei Beobachtungen genommen habe, kommt es dann auf diesen Unterschied auch nicht mehr direkt an. Der Standardfehler wurde aber in der Arbeit von @venables2008green benutzt und ist auch ein besseres Maß, wenn es um die [Abschätzung der statistischen Signifikanz geht](#sec-desc-stat).
...
placebo_fit <- green_tea_tbl |> 
  filter(trt == "PLA") |> 
  lm(fat_ox_r_kj_min ~ time, data = _) 

f <- \(x) predict(placebo_fit, newdata = tibble(time = x)) 
integrate(f, 10, 30)
```

$$
\cfrac{98.6\; kJ/min}{37\; kJ/g} = 2.66 \; g/min 
$$
...
f <- \(x) predict(gte_fit, newdata = tibble(time = x)) 
integrate(f, 10, 30)
```

$$
\cfrac{114.67\; kJ/min}{37\; kJ/g} = 3.10 \; g/min 
$$
:::

$$
\cfrac{3.10}{2.66} = 1.17\; (\uparrow 17\%)  
$$

```{r}
green_fit <- lm(fat_ox_r_kj_min ~ trt + time_fct + trt:time_fct, data = green_tea_tbl)
```

```{r}
green_fit |> 
  emmeans(~ trt | time_fct) |> 
  pairs()
```
...
![Ursprüngliche Abbildung, die nachgebaut werden soll. Zwei lineare Regressionen mit den jeweiligen Regressionsgleichungen.](images/eda/zerforschen-fleas.png){#fig-zerforschen-fleas fig-align="center" width="90%"}

$$
T_{calc} = \cfrac{30.4 - 19.9}{9.1 \cdot \sqrt{2/450}} = \cfrac{10.5}{0.61} = 17.21
$$
...
[History of the Ozone Hole](https://ozonewatch.gsfc.nasa.gov/facts/history_SH.html)

![Ursprüngliche Abbildung, die nachgebaut werden soll. Zwei lineare Regressionen mit den jeweiligen Regressionsgleichungen.](images/eda/zerforschen-nasa-ozone.png){#fig-nasa-ozone fig-align="center" width="100%"}
...
```{r}
pacman::p_load(ggh4x, gganimate, broom)

nasa_tbl <- read_excel("data/zerforschen_nasa_ozone.xlsx") |> 
  mutate(method = as_factor(method))
```

[Animate ggplots with gganimate::CHEAT SHEET](https://rstudio.github.io/cheatsheets/gganimate.pdf)
