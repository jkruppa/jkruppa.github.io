...
Wir laden als erstes wieder den Datensatz, den ich mir aus der obigen Abildung erstellt habe. Wie immer beim Zerforschen habe ich nicht so genau drauf geachtet nur das die Zahlen so grob stimmen. Die Erstellung der Daten kann hier recht langwierig sein, aber hier geht es ja mehr um die Nutzung von `ggplot`. Also mach dir keien Gedanken, wen die Punkte nicht so perfekt passen.
