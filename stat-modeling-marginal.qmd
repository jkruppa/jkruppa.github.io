...
In den folgenden Abschnitten werden wir dann immer wieder Teile der Flohdaten nutzen um die *Marginal effect models* besser zu verstehen und anzuwenden. Ich werde nicht den ganzen Datensatz auswerten sondern immer weider schauen, welcher Teilaspekte mehr von Interesse sind.
...
Beginnen wir einmal mit einem kontinuierlichen Messwert wie die Sprungweite und lassen uns dann verschiedene kontrafaktische Vergleiche berechnen.
...
```{r}
jump_fit <- lm(jump_length ~ stage + feeding + stage:feeding + weight, 
               data = jump_tbl)
```
...
comparisons(jump_fit, comparison = "difference",
            ...
            newdata = datagrid(feeding = "sugar_water", stage = unique))
```
:::
...
Wir entfernen hier einmal die Interaktion um das Modell etwas einfacher zu machen.

```{r}
infected_fit <- glm(infected ~ feeding + stage + weight, 
...
Mit `transform = "exp"` erhalten wir den Faktor der Änderung

```{r}
comparisons(infected_fit, transform = "exp",
            variables = list("weight" = 2), 
            newdata = datagrid(feeding = unique, stage = unique))
```

Ohne die Transformation erhalten wir dann die Änderung in der Wahrscheinlichkeit infiziert zu sein.
