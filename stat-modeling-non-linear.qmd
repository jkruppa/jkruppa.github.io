...
pacman::p_load(tidyverse, magrittr, broom, nlraa, modelsummary,
               ...
               gratia, readxl, nlstools, janitor, ggeffects, conflicted)
...
marg_pred_tbl <- predictions(duckweeds_gam_fit, newdata = duckweeds_tbl) %>% 
  as_tibble() %>% 
  select(duckweeds_density, sensor, estimate, conf.low, conf.high)
marg_pred_tbl 
```
...
gratia_pred_tbl %>% 
  select(duckweeds_density, est, lower, upper)
```
:::
