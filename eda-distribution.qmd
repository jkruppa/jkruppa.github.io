...
In der @fig-gummisample-4 sehen wir dann nochmal das Histogramm der Verteilung der Farben. Ich habe die Verteilung einmal auf der originalen Skala dargestellt und einmal auf der $log_{10}$-Skala. Im zweiten Fall siehst du dann auch besser die drei Tütchen mit der einen Farbe als Balken dargestellt - sonst geht der Balken ja sehr unter. Spannenderweise sieht unsere linke @fig-gummisample-4(A) ziemlich genau so aus, wie unsere @fig-pois-01(B) für unsere erhobenen Gummibärchendaten.
...
#| fig-width: 10
#| cache: false
#| fig-cap: "Histogramm der Anzahl an Farben in einer Tüte. Es gibt nicht mehr als sechs Farben. **(A)** Auf der originalen Skala. **(B)** Auf der $log_{10}$ Skala. *[Zum Vergrößern anklicken]*"
...
p2 <- p +
  ...
  scale_x_continuous(breaks = seq(1, 6, by = 1), limits = c(0.5, 6.5)) +
  labs(x = "", y = expression(log[10(Anzahl)])) +
  geom_label(stat='count', aes(label=..count..), size = 3) +
  theme(axis.text.y = element_blank())
