...
In der @fig-time-salad-roll-1 siehst du einmal das Ergebnis der drei rollenden Maßzahlen. Im Folgenden habe ich zuerst den `ggplot` für das rollende Mittel erstellt und dann über den Operator `%+%` die beiden anderen Datensätze zu dem rollenden Maximum un der rollenden Summe ersetzt. Die rollende Summe habe ich noch auf der $\log$-transformierten $y$-Achse dargestellt.
