...
Wir nutzen die deskriptive Statistik um Zahlen zusammenzufassen. Das heißt wir haben einen Datensatz vorliegen und wollen jetzt den großen Datensatz in wenige Zahlen wiedergeben. Wichtig ist zu wissen, dass wir *mehrere Zahlen* durch meist eine oder zwei Zahlen beschreiben wollen. Wir brauchen die statistischen Maßzahlen aus diesem Kapitel später um große Datensätze darstellen zu können. Ebenso werden wir die Maßzahlen aus diesem Kapitel dafür verwenden statistische Tests und Modelle zu rechnen.
...
::: column-margin
Kommt dir der R Code für die Auswahl der Zeilen bekannt vor? Schaue sonst nochmal ins @sec-dplyr.
...
Beginnen wir aber mit einem einfachen Beispiel und dem Datensatz `flea_dog_cat.xlsx`. In @tbl-dog-cat-desc sehen wir nochmal den Datensatz. Wir wollen uns jetzt aber erstmal nur die Spalte `jump_length` für die Hunde anschauen.

```{r}
#| echo: false
#| label: tbl-dog-cat-desc
#| tbl-cap: Tabelle  der Sprunglängen [cm], Anzahl an Flöhen, Boniturnote sowie der Infektionsstatus von Hunde- und Katzenflöhe. 
