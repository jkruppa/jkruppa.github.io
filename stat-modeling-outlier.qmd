...
Eine weitere Möglichkeit Ausreißer zu finden bietet das R Paket `{olsrr}`, wenn wir uns nur auf normalverteilte Daten konzentrieren. Das R Paket `{olsrr}` hat eine Vielzahl an Funktionen, die wir nutzen können um auffällige Beobachtungen zu finden. Schau dir einfach einmal das [R Paket und die Webseite](https://olsrr.rsquaredacademy.com/articles/influence_measures) näher an.

```{r}
#| echo: true
#| message: false
#| label: fig-beeswarm-out-olsrr-0
#| fig-align: center
#| fig-height: 5
#| fig-width: 6
#| fig-cap: "Der Beeswarm ist ein Dotplot für eine große Anzahl an Beobachtungen. Hier schauen wir uns einmal das Alter und die Körpergröße aufgeteilt nach Geschlecht an. Die gefundenen Ausreißer sind mit $\\bullet$ markiert, die nromalen Beobachtungen als $\\circ$ dargestellt."
...
#|   - "Körpergröße nach Geschlecht"
#| layout-nrow: 1
model <- lm(age ~ gender + height, data = gummi_tbl)
ols_plot_dfbetas(model) 
foo <- ols_plot_resid_lev(model)

out_tbl <- pluck(foo, "data") 
...
gummi_tbl |> 
  ...
  geom_beeswarm(size = 1) +
  theme_minimal() +
  scale_color_okabeito(order = c(2, 7, 1, 3), guide = "none") +
  scale_shape_manual(name = "Ausreißer", values = c(1, 19, 3, 4)) +
  labs(x = "Geschlecht", y = "Alter in [Jahren]") 
