...
[simstudy update: ordinal data generation that violates proportionality](https://www.rdatagen.net/post/2022-01-25-simstudy-update-ordinal-data-without-the-proportionality-assumption/)
...
Dann stellt sich noch die Frage, ob wir denn auch die voreingestellten Parameter wieder aus einer ordinalen Regression herausbekommen.

$$
log \left(\cfrac{Pr(Resp \leq  i)}{Pr(Resp > i)} | f_A \right) = \alpha_i - \beta*I(f_A=A.2)  \ , \ i \in \{1, 2, 3\}
$$
...
```{r}
logOdds_A.1 <- qlogis(cumsum(ord_fac1_dt[f_a == "A.1", prop.table(table(antwort))]))[1:3]
```
...
$\text{logistic}()$ oder $\text{invlogit}()$

$$
\text{invlogit}(Pr) = \cfrac{1}{1 + exp(-Pr)}
$$

```{r}
c(0, plogis(logOdds_A.1), 1) |> diff() |> round(2)
```
