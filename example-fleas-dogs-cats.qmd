...
-   Die Art der Fütterung `feeding` mit den Leveln Blut `blood`, Zuckerwasser als Kontrolle `sugar_water` und Ketchup `ketchup`.
-   Den Entwicklungsstand `stage` mit den zwei Leveln für juvenile Flöhe `juvenile` und ausgewachsenen Flöhen `adult`.
-   Das Gewicht `weight` der einzelnen Flöhe zum Zeitpunkt der Messung.
...
-   Exterieurbeurteilung `bonitur`
-   Anzahl an Haaren am rechten bein `count_leg`
-   Infektionsstatus mit Flohschnupfen `infected`
...
sigma <- matrix(c(4, 4, 3, 3, 2, 0,
                  4, 4, 3, 2, 3, 0, 
                  3, 3, 4, 3, 4, 0,
                  3, 2, 3, 4, 1, 0,
                  2, 3, 4, 1, 4, 5,
                  0, 0, 0, 0, 5, 4), ncol=6, byrow = TRUE)

lab_tbl <- rbind(rmvnorm(n = 8, mean = c(40, 20, 10, 110, 50, 5), sigma = sigma),
                 rmvnorm(n = 8, mean = c(30, 10, 5, 120, 60, 5), sigma = sigma),
                 rmvnorm(n = 8, mean = c(20, 20, 5, 120, 20, 15), sigma = sigma),
                 rmvnorm(n = 8, mean = c(40, 10, 10, 140, 20, 5), sigma = sigma),
                 rmvnorm(n = 8, mean = c(20, 20, 15, 100, 15, 5), sigma = sigma),
                 rmvnorm(n = 8, mean = c(30, 20, 15, 80, 15, 25), sigma = sigma)) |> 
  as_tibble() |> 
  set_names(c("K", "Mg", "CRP", "Hb", "BSG", "M")) |> 
  round(2)
```

Hb gleich der rote Blutfarbstoff Hämoglobin BSG ist die Blutsenkung
