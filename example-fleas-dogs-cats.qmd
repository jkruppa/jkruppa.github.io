...
bar <- read_excel("data/fleas_model_data_backup.xlsx") |>  
...
table(bar$bin_bonitur)
table(bar$infected, bar$stage)
```

```{r}
ggplot(bar, aes(x = feeding, y = jump_length, fill = stage)) +
...
```{r}
ggplot(bar, aes(x = feeding, y = log(count_leg), fill = stage)) +
  geom_boxplot()
