...
`comparisons()` Funktion

$$
\begin{align*}
\hat{Y}_{X=x+1} - \hat{Y}_{X=x} && \text{Increase of one unit}\\
\hat{Y}_{X=x+\sigma_X} - \hat{Y}_{X=x} && \text{Increase of one standard deviation}\\
...
In the special case where the predicted outcome $\hat{Y}_X$ is a probability

$\hat{Y}_{X=b}-\hat{Y}_{X=a}$ is called a risk difference

$\hat{Y}_{X=b}/\hat{Y}_{X=a}$ a risk ratio

$\frac{\hat{Y}_{X=b}}{1 - \hat{Y}_{X=b}} \bigg/ \frac{\hat{Y}_{X=a}}{1 - \hat{Y}_{X=a}}$ an odds ratio
