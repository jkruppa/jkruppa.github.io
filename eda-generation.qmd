...
Die drei Normalverteilungen $y_1$ bis $y_3$ mischen wir jetzt. Dafür nutzen wir die folgende Schreibweise in `{simstudy}`. Wir wollen 30% von $y_1$ also schreiben wir `y_1 | 0.3`. Dann noch 40% von $y_2$ mit `y_2 | 0.4` sowie 30% von $y_3$ mit `y_3 | 0.3`. Wir nennen dann die Spalte `y_null` mit der gemischten Verteilung damit wir gleich den Vergleich zwischen den Verteilungen machen können.
