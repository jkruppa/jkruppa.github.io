...
most_comm_mat <- read_excel("data/gummibears.xlsx") %>% 
  select(year, gender, height, most_liked) %>% 
  mutate(string = str_c(gender, height, most_liked, sep = "-")) %>% 
  na.omit() %>% 
...
Gibt es eigentlich eine typische weibliche Person oder männliche Person in den Daten? Ja, gibt es. Unsere meist vertretende Frau ist `r most_comm_mat[2, 2]`cm groß und mag am liebsten Gummibärchen der Farbe `r col_switch[most_comm_mat[2, 3]]`. Auf der anderen Seite ist unser meist vertretender Mann `r most_comm_mat[1, 2]`cm groß und mag am liebsten Gummibärchen der Farbe `r col_switch[most_comm_mat[1, 3]]`.
...
ggplot(data = gummi_3_tbl, aes(x = count_bears, y = fct_rev(as_factor(year)),
  ...
  scale_fill_okabeito() +
  annotate("label", x = 11.5, y = c(1, 2, 3, 4, 5) + 0.7, label = str_c("n = ", count_tbl$n))
