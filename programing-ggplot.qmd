...
[R Paket `{distributional}`](https://pkg.mitchelloharawild.com/distributional/index.html)
...
ggplot_tbl |> 
  expand(feeding, stage) %>%
  augment(jump_feeding_fit, newdata = ., se_fit = TRUE) |> 
  ...
  geom_point(data = ggplot_tbl, aes(x = jump_length),
             pch = "|", size = 2, position = position_nudge(y = -.15))
```
