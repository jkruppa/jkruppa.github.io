...
|           |     |            **Damaged**            |                                   |               |
|           |     |             *Yes (1)*             |             *No (0)*              |               |
| **Group** | *A* | $\cfrac{41 \cdot 33}{65} = 20.82$ | $\cfrac{24 \cdot 33}{65} = 12.18$ | $\mathbf{33}$ |
|           | *B* | $\cfrac{41 \cdot 32}{65} = 20.18$ | $\cfrac{24 \cdot 32}{65} = 11.82$ | $\mathbf{32}$ |
|           |  C  |                                   |                                   |               |
|           |  D  |                                   |                                   |               |
|           |     |           $\mathbf{41}$           |           $\mathbf{24}$           |   $n = 65$    |

: Eine 2x2 Tabelle als Beispiel für unterschiedliche Flohinfektionen bei Hunden und Katzen. Dargestellt sind die zu *erwartenden* Werte. {#tbl-chi-square-exp-2}
...
-   $\hat{p}_2$ ist der Anteil der 2. Stichprobe
-   $n_1$ ist die Anzahl der Datenproben in der ersten Stichprobe
-   $n_2$ ist die Anzahl der Datenproben in der 2.
-   $\hat{p}$ ist der Mittelwert der beiden Stichproben. Wenn $k1$ der Erfolg von $n1$ Daten in Stichprobe 1 und $k2$ der Erfolg von $n2$ Daten in Stichprobe 2 ist, dann kann der Wert von $\hat{p}$ wie folgt berechnet werden:
