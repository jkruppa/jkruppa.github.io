...
bsc_tbl |> 
  pivot_longer(cols = Seiten:Referenzen,
               names_to = "type",
               values_to = "count") |> 
  ggplot(aes(y = count, x = type, color = type)) +
  theme_minimal() +
...
bsc_tbl |> 
  ...
  geom_vline(xintercept = min(bsc_tbl$Seiten, na.rm = TRUE) - 2, linetype = 2) +
  geom_hline(yintercept = min(bsc_tbl$Referenzen, na.rm = TRUE) - 2, linetype = 2) +
