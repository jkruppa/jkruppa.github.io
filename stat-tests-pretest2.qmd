...
$$
\underbrace{\mbox{Messwert}\; y}_{normalverteilt} \sim \overbrace{f_A + f_B}^{varianzhomogen}
$$ mit
...
Insgesamt haben $`r pull(filter(stat_gummi_tbl, gender == "m"), n)`$ Männer und $`r pull(filter(stat_gummi_tbl, gender == "w"), n)`$ Frauen bei der Gummibärchenumfrage mitgemacht.
