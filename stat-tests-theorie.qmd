...
Wir schauen uns die ersten zehn nach Bonferroni adjustoerten p-Wert nach der Anwendung der Funktion `p.adjust()` einmal an.

```{r}
round(p.adjust(p, "bonferroni"), 2)[1:10] %>% round(5)
```
...
```{r}
round(p.adjust(p, "BH"), 2)[1:10] %>% round(5)
```
