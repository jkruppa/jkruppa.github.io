...
Es gibt zwei Haupteinstiegspunkte für Hypothesentests in `{marginaleffects}`. Wenn wir das einigermaßen unterschieden, dann wird auch vieles klarer.

1)  **Funktionen in `{marginaleffects}`**: Wir verwenden die Option `hypothesis` in den entsprechenden Funktionen.
2)  **Andere R-Objekte und Modelle**: Wir verwenden die Funktion `hypotheses()`.

#### Die `b`-Schreibweise der Koeffizienten {.unnumbered .unlisted}

[Koeffizienten als `b` abkürzen](https://marginaleffects.com/bonus/hypothesis.html#coefficients-1)

### Hypothesentest

Das statistische Testen haben wir schon in einem eigenen Kapitel bearbeitet. Du kannst mehr über die Hintergründe im [Kapitel zur Testentscheidung](#sec-stat-entscheidung) lesen. Wenn es um die Koeffizienten eines linearen Modells geht und deren statistische Tests und Interpretation hilft dir das [Kapitel zur linearen Regression](#sec-modeling-simple-stat) weiter. Wir wollen uns hier nochmal kurz auf die Prinzipien zurückerinnern und dann einmal zeigen was die Funktion `hypotheses()` so alles kann. Dann schauen wir auch nochmal auf die Anwendung in verschiedenen Beispielen.
...
$$
Z = \cfrac{\hat{\beta_1}-H_0}{\sqrt{\widehat{V}[\beta_1]}} = \cfrac{70.00562 - 0}{7.516627} = 9.313435
$$
...
```{r}
hypotheses(feeding_fit, hypothesis = 0)
```
