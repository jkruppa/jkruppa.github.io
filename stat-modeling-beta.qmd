...
pacman::p_load(tidyverse, magrittr, broom, betareg, car,
               see, performance, parameters, agridat,
...
hunting_tbl %>% 
  ggplot(aes(grass_cover, proportion)) +
  theme_minimal() +
  geom_point() +
  ylim(0, 1)
```
...
```{r}
hunting_lm_fit <- lm(proportion ~ grass_cover, data = hunting_tbl)
```
...
```{r}
hunting_beta_fit <- betareg(proportion ~ grass_cover, data = hunting_tbl)
```
...
```{r}
hunting_log_fit <- glm(cbind(hunting_tbl$success, hunting_tbl$fail) ~ grass_cover, 
...
Hier greifen wir auf das R Paket `{rcompanion}` zurück. Wir rechnen auck kein *echtes* Bestimmtheitsmaß $R^2$ aus, sondern den Verlgeich zum Null-Modell.
...
hunting_tbl %>% 
  ...
  theme_minimal() +
  geom_point() +
  geom_line(aes(y = predict(hunting_log_fit, type = "response")), color = cb_pal[2]) +
  geom_line(aes(y = predict(hunting_lm_fit, type = "response")), color = cb_pal[3]) +
  geom_line(aes(y = predict(hunting_beta_fit, type = "response")), color = cb_pal[4])
```
