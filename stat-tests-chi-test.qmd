...
Nehmen wir ein etwas konstruiertes Beispiel zur Erdbeerernte. Wir haben einen neuen Roboter entwickelt, der Erdbeeren erntet. Nun stellen wir fest, dass von 100 Erdbeeren 76 heile sind. Jetzt lesen wir im Handbuch, dass der Ernterobotor eigentlich 84% der Erdbeeren heile ernten sollte. Sind jetzt 76 von 100, also 76%, signifikant unterschiedlich von 84%? Oder können wir die Nullhypothese der Gleichheit zwischen den beiden Wahrscheinlichkeiten nicht ablehnen? Damit können wir den Test auf Anteile nutzen, wenn wir eine beobachtete Wahrscheinlichkeit oder Anteil $p_1$ gegen eine gegebene Wahrscheinlichkeit oder Anteil $p_0$ vergleichen wollen.

In unswerem Fall haben wir mit $p_1$ die Wahrscheinlichkeit vorliegen, dass die Erdbeeren in unserem Experiment heile sind. Wir wissen also, dass $p_1 = \cfrac{x}{n} = \cfrac{76}{100} = 0.76 = 76\%$ ist. Damit ist die Wahrscheinlichkeit $p_1$ auch die *beobachte* Wahrscheinlichkeit. Wir *erwarten* auf der anderen Seite die Wahrscheinlichkeit $p_0 = 0.84 = 84\%$. Die Robotorfirma hat uns aj zugesagt, dass 84% der Erdbeeren heile bleiben sollen.
...
Jetzt müssen wir nur noch die *beobachteten* Anteile mit den zu *erwarteten* Anteilen durch die $\mathcal{X}^2$-Formel in Kontext setzten. Wir subtrahieren von jedem beobachten Anteil den zu erwartenden Anteil, quadieren und addieren auf. Dann erahlten wir die $\mathcal{X}^2$-Statistik.
...
$$
Z = \cfrac{\hat{p}_1 - \hat{p}_2}{\sqrt{\hat{p}(1-\hat{p})\left(\cfrac{1}{n_1}+\cfrac{1}{n_2}\right)}}
$$

-   $\hat{p}_1$ ist der Anteil der ersten Stichprobe
-   $\hat{p}_2$ ist der Anteil der 1. Stichprobe
-   $n_1$ ist die Anzahl der Datenproben in der ersten Stichprobe
-   $n_2$ ist die Anzahl der Datenproben in der zweiten Stichprobe
-   $\hat{p}$ ist der Mittelwert der beiden Stichproben. Wenn $k_1$ der Erfolg von $n_1$ Daten in Stichprobe 1 und $k_2$ der Erfolg von $n_2$ Daten in Stichprobe 2 ist, dann kann der Wert von $\hat{p}$ mit $\hat{p} = (k_1 + k_2)/(n_1 + n_2)$ berechnet werden.

$\hat{p}_1 = 130/200 = 0.65, n_1 = 200$

$\hat{p}_2 = 50/100 = 0.5, n_2 = 100$

$\hat{p} = (130 + 50)/(200 + 100) = 180/300 = 0.6$

$$
\begin{aligned}
Z &= \cfrac{(0.65 - 0.5)}{\sqrt{0.6\cdot(1-0.6)\cdot\left(\cfrac{1}{200} + \cfrac{1}{100}\right)}}\\
  &= \cfrac{0.15}{\sqrt{0.6\cdot0.4\cdot(0.005 + 0.01)}}\\
  &= \cfrac{0.15}{\sqrt{0.24\cdot0.015}}\\
  &= \cfrac{0.15}{\sqrt{0.49\cdot0.1225}}\\
&= \cfrac{0.15}{0.06} = 2.5
\end{aligned}
$$
...
: Eine 4x2 Tabelle für die vier Erntearten und der beschädigten Erdbeeren. Dargestellt sind die zu *beobachteten* Werte. {#tbl-chi-square-exp-2}
