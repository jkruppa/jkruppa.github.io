...
Und da ist das Monster einer Gleichung. Ob das jetzt wirklich *besser* ist, werden wir dann gleich sehen. Aber komplexer ist die Gleichung auf alle Fälle.

$$
freshmass \sim 37.55 - 578.20 \cdot \left(\cfrac{k\_in\_shoot}{10}\right)^{3} - 1370.91 \cdot \left(\cfrac{k\_in\_shoot}{10}\right)^{3} \cdot \log\left(\cfrac{k\_in\_shoot}{10}\right)
$$

Ich breche mir dann noch einen ab, diese Gleichung dann einmal in R zu überstetzen. Sieht dann gar nicht mehr so schlimm aus, ist aber dennoch ein Brocken.
...
Jetzt stellt sich natürlich die Frage des Maximums unserer Gradengleichung. Das ist jetzt der Vorteil unserer recht einfachen Gleichung. Wir können diese Ableiten und dann Null setzen. So können wir schnell das Maximum berechnen. Das wäre natürlich bei komplizierteren Gleichungen dann schwerer.
...
Dann die erste Ableitung $f'(x)$ Null setzen und nach $x$ auflösen.

$$
x = \frac{-111.95}{-24.32} = 4.6
$$

Wir kriegen fast den gleichen Wert mit $4.6$ wie auch in der ursprünglichen Abbildung.
...
![Ursprüngliche Abbildung, die nachgebaut werden soll. Eine nicht-lineare Regressionen mit der jeweiligen Regressionsgleichung und dem Bestimmtheitsmaß $R^2$.](images/eda/zerforschen_pseudo_time.jpeg){#fig-simple-regression fig-align="center" width="100%"}
...
p1 <- time_shoot_tbl |> 
  ...
  labs(x = "Tag der Messung", y = TeX(r"(Shoot fresh mass \[g plant$^{-1}$\])"),
       color = "") +
...
#| warning: false
#| message: false
stat_t4_tbl <- time_shoot_tbl |> 
  filter(date == "2023-06-03") |> 
  group_by(date, trt) |> 
  summarise(mean = mean(freshmatter),
            sd = sd(freshmatter),
            cld_pos = mean + sd + 4)
...
```{r}
p2 <- ggplot(stat_t4_tbl, aes(x = date, y = mean, 
                       fill = trt)) + 
  theme_minimal() +
  ...
  labs(x = "", y = "") +
  annotate("text", 
           x = c(0.75, 0.975, 1.2, 1.425), 
           y = stat_t4_tbl$cld_pos, 
           label = c("a", "ab", "b", "c")) +
  theme(legend.position = "none") +
  scale_fill_okabeito() +
  scale_x_discrete(labels = c("03. Juni")) 
```
...
#| warning: false
#| message: false
#| label: fig-zerforschen-timeseries-shoot
#| fig-align: center
#| fig-height: 5
#| fig-width: 7
#| fig-cap: "Visualisierung ."
