...
![](images/caution.png){fig-align="center" width="20%"}
...
Abschließend dann nochmal etwas idealisierte Daten zu einer Zeitreihe, damit wir auch einige der komplexeren Funktionen später nutzen können. Teilweise sind unsere landwirtschaftlichen Daten nicht lang genug erhoben worden. In dem R Paket `timetk` findet sich der künstliche Datensatz `m4_monthly`. Hier schauen wir uns den Datensatz zu dem M4 Wettbewerb an. Die [Makridakis-Wettbewerbe](https://mofc.unic.ac.cy/m4/) (eng. *M Competitions*) sind eine Reihe von offenen Wettbewerben, die von dem Prognoseforscher Prof. Spyros Makridakis organisiert und geleitet werden, um die Genauigkeit verschiedener Prognosemethoden zu bewerten und zu vergleichen. Deshalb sind die folgenden Daten eher als eine Simulation anzusehen denn als echte Daten. Dennoch basiert der Datensatz auf Daten, die auch in echt beobachtet werden könnten. Wir nehmen hier aber nur die `M750` Daten.
...
In der @tbl-time-m4 siehst du einen Auszug aus den Daten. Die Zahlen und das Datum sind eigentlich biologisch uninteressant, wir nutzen diese Zahlen und Werte nur um die Modelle später anzuwenden und zu testen. Daher haben die Werte keine echte biologische Bedeutung. Das Schöne hier ist, dass unser Datum schon das richtige Format hat, so dass wir uns hier nicht weiter um die Daten kümmern müssen.
...
#| warning: false
#| label: tbl-time-m4
#| tbl-cap: "Auszug aus den Daten zum M4 Wettbewerb und der ID M750."

raw_m750_tbl <- m750_tbl %>% 
  mutate_all(as.character)

rbind(head(raw_m750_tbl , n = 4),
      rep("...", times = ncol(raw_m750_tbl)),
      tail(raw_m750_tbl, n = 4)) %>% 
  kable(align = "c", "pipe")
```
...
Wir erkennen ganz gut, dass wir hier einen Effekt der Session haben oder aber der Jahreszeit. Wir haben zyklische Peaks der Milchleistung über das Jahr verteilt. Gegen Ende unserer Zeitreihe sehen wir aber eine Art Plateau der Milchleistung. In der folgenden Analyse wollen wir einmal schauen, ob wir die zukünftige Milchleistung anhand der bisherigen Daten vorhersagen können.
...
In der @fig-time-m4-1 siehst du einmal die M4 Daten dargestellt. Wir konzentrieren uns hier auf die Subgruppe `m750`. Auch hier nutzen wir jetzt einmal aus Gründen des Platzes einmal die schnelle Funktion `plot_time_series()` aus dem R Paket `timetk`. Wir haben hier in unseren M4 Daten auch einen zyklischen Verlauf über die Jahre mit verschiedenen Plateaus vorliegen. Auch hier wollen wir einmal schauen, ob wir dann eine Vorhersage über die zukünftigen Werte erhalten können.
...
#| fig-height: 4
#| fig-width: 7
#| fig-cap: "Die Darstellung der `m750` Daten durch das R Paket `timetk` und der Funktion `plot_time_series()`, die durch die Option `.interactive = TRUE` intern dann `plotly` aufruft."
...
### Das R Paket `zoo` und `ts` (schnelle & einfache Analysen)

```{r}

m750_ts <- m750_tbl %$%
  ts(value, date, deltat = 1/12) 

decomp <- decompose(m750_ts)
...
Rollender Mittelwert und Co.

Das R Paket `zoo` hat die Funktion `rollmean()` sowie `rollmax()` und `rollsum()`. Es gibt aber noch eine Reihe weiterer Funktionen. Du musst hier einfach mal die Hilfeseite `?rollmean()` für mehr Informationen aufrufen.

```{r}
#| message: false
#| warning: false

rollmean_tbl<- csh_tbl %$%
  zoo(g_tm_plot, day) %>% 
  rollmean(k = 7) %>% 
  tk_tbl()
```
...
### Das R Paket `tktime` (komplexe Analysen)

[timetk for R](https://business-science.github.io/timetk/)

### Vergleich zweier zeitlicher Verläufe
