...
Laut dem Algorithmus sollen wir eine `Standardized Yeo-Johnson Transformation` durchführen. Was immer das jetzt konkret ist. Wir leben damit. Okay, wie machen wir das jetzt? Wir hätten zwar die Parameter in der Ausgabe angegeben und die könnten wir dann auch in einer Veröffentlichung angeben, aber wenn es schneller gehen soll, dann können wir die Funktion `predict()` nutzen, die uns die transformierten `y` Werte wiedergibt.
...
```{r}
y_trans_back <- predict(bn_obj, newdata = y_trans, inverse = TRUE)
```

Einmal dann der Vergleich ob alle `y_trans_back` Werte den ursprünglichen `y` Werten entsprechen. Ja, das sieht gut aus.

```{r}
all.equal(y_trans_back, fac1_tbl$hatch_time)
```

Und zum Abschluss nochmal eine Abbildung der transformierten `y` Werte. In der folgenden Abbildung siehst du dann einmal das entsprechende Histogramm. Das sieht doch sehr gut aus und wir mussten nicht zig verschiedene Algorithmen selber testen.
...
#| fig-height: 3.5
#| fig-width: 8
#| fig-cap: "Histogramm der transformierten Werte mit der Standardized Yeo-Johnson Transformation aus dem R Paket `{bestNormalize}` für die Schlupfzeiten. *[Zum Vergrößern anklicken]*"
...
p2 <- ggplot(as_tibble(y_trans), aes(y_trans)) +
  theme_minimal() +
  geom_density(fill = cbbPalette[3], color = "black") +
  labs(x = "Zeit bis zum Schlüpfen nach Yeo-Johnson Transformation", y = "")  
...
#| fig-height: 3.5
#| fig-width: 8
#| fig-cap: "Histogramm der transformierten Werte mit der $\\lambda = 0.05$ Transformation aus dem R Paket `{rcompanion}` für die Schlupfzeiten. *[Zum Vergrößern anklicken]*"
...
p2 <- ggplot(as_tibble(tt_obj), aes(tt_obj)) +
  theme_minimal() +
  geom_density(fill = cbbPalette[3], color = "black") +
  labs(x = "Zeit bis zum Schlüpfen mit $\\lambda = 0.05$ Transformation", y = "")  
