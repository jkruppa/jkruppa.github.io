...
## Auswertung von Pflugdaten {#sec-plowing-data}
...
cutting_tbl <- map(c("trieblange", "blutenzahl", "blattanzahl", "fruchtanzahl"),
    function(x) {
      print(x)
      cutting_raw_tbl %>% 
        select(trt, block, matches(x)) %>% 
        gather(key, value, -trt, -block) %>% 
        mutate(week = as.numeric(str_match(key, "\\d+")),
               week = as.numeric(as.factor(week)),
               key = x,
               block = as_factor(block)) %>% 
        select(trt, block, week, key, value)
      }) %>% 
  bind_rows()

cutting_tbl %<>% 
  mutate(key = recode(key, trieblange = "Trieblänge", 
                      blutenzahl = "Blütenanzahl", 
                      blattanzahl = "Blätteranzahl", 
                      fruchtanzahl = "Fruchtanzahl"))

cutting_tbl %>% 
  ggplot(aes(week, value, color = trt, linetype = block)) +
  theme_minimal() +
