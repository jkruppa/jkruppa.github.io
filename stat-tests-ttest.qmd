...
| Messung 1 | Messung 2 | Differenz $\boldsymbol{\Delta}$ |
|:---------:|:---------:|:-------------------------------:|
| $y_{11}$  | $y_{21}$  |  $\Delta_1 = y_{11} - y_{21}$   |
| $y_{12}$  | $y_{22}$  |  $\Delta_2 = y_{12} - y_{22}$   |
| $y_{13}$  | $y_{23}$  |  $\Delta_3 = y_{13} - y_{23}$   |
| $\vdots$  | $\vdots$  |            $\vdots$             |
| $y_{1n}$  | $y_{2n}$  |  $\Delta_n = y_{1n} - y_{2n}$   |
|           |           |         $\bar{\Delta}$          |

: test {#tbl-t-test-verbunden}

Wir nutzen folgende Formel für den gepaarten t-Test für verbundene Stichproben.
...
Dann berechnen wir einmal den Mittelwert der Differenzen $\Delta$.
...
Jetzt brauchen wir natürlich noch einen kritischen Wert $T_{\alpha = 5\%}$ und der wäre in diesem Fall gleich $`r round(qt(0.95, (5 + 5 - 2), lower.tail = TRUE), 2)`$. Damit können wir dann die Nullhypothese ablehnen, dass wir keinen Effekt der Behandlung vorliegen haben.
