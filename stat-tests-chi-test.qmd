...
fac1_tbl <- read_excel("data/flea_dog_cat_length_weight.xlsx") |> 
  select(animal, sex, jump_length, flea_count) |> 
  filter(animal != "fox") |> 
  mutate(jump_target_reached = ifelse(jump_length > 20, 1, 0),
         jump_target_reached = as_factor(jump_target_reached),
         infected = ifelse(flea_count > 1, 1, 0),
         infected = as_factor(infected))

fac1_tbl |> 
  janitor::tabyl(jump_target_reached, animal)
...
```{r}
fac1_tbl |> 
  janitor::tabyl(jump_target_reached, sex)
```
...
```{r}
chi_d <- fac1_tbl |> 
  specify(jump_target_reached ~ animal, succes = "1") |> 
  calculate(stat = "Chisq", order = c("dog", "cat"))
chi_d
```
...
```{r}
null_dist_data %>%
  get_p_value(obs_stat = chi_d, direction = "two-sided")
```
...
visualize(null_dist_data) +
  theme_minimal() +
  shade_p_value(obs_stat = chi_d, direction = "two-sided",
                color = "#CC79A7") +
  labs(x = "Statistik", y = "Absolute Häufigkeit") 
```
