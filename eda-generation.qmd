...
Wir können jetzt in `{simstudy}` beide Varianten durchführen. Wir können also einmal davon ausgehen, dass wir den gleichen Effekt in die gleiche Richtung für alle Antwortwahrscheinlichkeiten vorliegen haben. Wir rechnen dann mit der *Proportional odds* Annahme. Wir können aber auch den Fall vorliegen haben, dass wir die Antwortwahrscheinlichkeiten individuell verschieben. Dann haben wir die Annahme der *Non-proportional odds* getroffen. Ich zeige dir dann in beiden Tabs einmal die Durchführung in `{simstudy}` und wie wir dann die zufälligen ordinalen Werte erhalten.

:::: panel-tabset
## *Proportional odds*

Als erstes brauchen wir wieder einmal unser Klassenwahrscheinlichkeiten oder eben wie wahrscheinlich tritt eine der Antwortkategorien auf. Um die Sache etwas zu vereinfachen habe ich hier nur vier Antwortmöglichkeiten gewählt und die entsprechenden Wahrscheinlichkeiten festgelegt.
...
Jetzt brauchen wir noch unsere latente Variable $z$ um unseren Wahrscheinlichkeiten in feste Werte zu übersetzen. In der folgenden Abbildung siehst du dann einmal den Logit der Wahrscheinlichkeiten. Der Logit beschreibt die Wahrscheinlichkeiten als logarithmierte Chancen. Das klingt jetzt etwas wild, aber wir transformieren im Prinzip die Flächen so, dass wir dann die Wahrscheinlichkeiten auch durch die x-Werte beschreiben können.
...
Was ist die Logitfunktion einmal mathematisch? In der folgenden Formel siehst du einmal den Zusammenhang. Der Logit einer Wahrscheinlichkeit $Pr$ ist dsd logarithmierte Chancenverhältnis. Damit können wir Wahrscheinlichkeiten in feste Werte überstetzen, die wir dann durch Addition oder Subtraktion verschieben können.
...
ord_fac1_nonprop_dt <- genOrdCat(ord_fac1_pre_dt, baseprobs = null_prob, adjVar = "z",
                         catVar = "r", npVar = "f_a", npAdj = shift_z)
```
...
#| message: false
#| warning: false
clmFit <- clm(r ~ f_a, data = ord_fac1_dt)
```
...
```{r}
logOdds_A.1 <- qlogis(cumsum(ord_fac1_dt[f_a == "A.1", prop.table(table(r))]))[1:3]
```
