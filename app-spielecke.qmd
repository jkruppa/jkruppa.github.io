...
![Ursprüngliche Abbildung, die nachgebaut werden soll. Zwei lineare Regressionen mit den jeweiligen Regressionsgleichungen.](images/eda/zerforschen_green_tea_effect_01.png){#fig-zer-green-tea-01 fig-align="center" width="80%"}

![Ursprüngliche Abbildung, die nachgebaut werden soll. Zwei lineare Regressionen mit den jeweiligen Regressionsgleichungen.](images/eda/zerforschen_green_tea_effect_03.png){#fig-zer-green-tea-03 fig-align="center" width="80%"}

![Ursprüngliche Abbildung, die nachgebaut werden soll. Zwei lineare Regressionen mit den jeweiligen Regressionsgleichungen.](images/eda/zerforschen_green_tea_effect_02.png){#fig-zer-green-tea-02 fig-align="center" width="100%"}
...
```{r}
green_fit <- lm(fat_ox_r_kj_min ~ trt + time + trt:time, data = green_tea_tbl)
```
...
$$
\cfrac{97\; kJ/min}{37\; kJ/g} = 2.62 \; g/min 
$$
...
$$
\cfrac{116.67\; kJ/min}{37\; kJ/g} = 3.15 \; g/min 
$$
:::

$$
\cfrac{3.15}{2.62} = 1.2 \approx\; \uparrow 20\%  
$$
...
```{r}
stat_tbl <- tibble(trt = as_factor(c("Placebo", "GTE")),
                   mean = c(0.35, 0.41),
                   sd = c(0.03, 0.03))
```
...
#| warning: false
#| message: false
#| label: fig-zerforschen-green-tea-02
#| fig-align: center
#| fig-height: 5
#| fig-width: 6
#| fig-cap: "Visualisierung der Sensorwerte nach Wasserlinsendichte. Pro Dichtewert liegen drei Sensormessungen vor."
...
library(writexl)
tiger_tbl |> 
  mutate(f7gruppengroesse = round(rexp(nrow(tiger_tbl), 0.2)) + 2,
         f5dauer_verletzung = round(rexp(nrow(tiger_tbl), 0.01) + 20 - 2.75 * f7gruppengroesse),
         f6einkommen_jahr = round(rexp(nrow(tiger_tbl), 0.001)) + 100,
         f8eis_verkauf= round(rexp(nrow(tiger_tbl), 0.1) + 10 + 0.25 * f7gruppengroesse)) |> 
  #select(matches("f5")) |> 
