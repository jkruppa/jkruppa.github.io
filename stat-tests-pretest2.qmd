...
#| fig-height: 6
#| fig-width: 8.5
#| fig-cap: "foo. **(A)** Zweigipflige Verteilung aus vermutlich zwei oder mehr Verteilungen. **(B)** Eine zu schmale Verteilung aber dennoch approximativ normalverteilt. **(C)** Eine linksschiefe Verteilung mit einer linken Verteilungsschulter. *[Zum Vergrößern anklicken]*"

set.seed(20250513)
norm_stat_small_tbl <- tibble(
  y = c(rnorm(5,  3, 2), rnorm(20, 3, 2), rnorm(40, 3, 2)),
  x = c(gl(1, 5, labels = "A"), gl(1, 10, labels = "B"), gl(1, 20, labels = "C")))
...
p1_small <- norm_stat_small_tbl |>
  ...
  labs(title = "Kleine Fallzahl (n = 5)",
...
p4_small <- norm_stat_small_tbl |>
  ...
  labs(title = "Moderate Fallzahl (n = 20)",
       subtitle = "Nicht normalverteilt") +
  theme(plot.title = element_text(size = 17),
        plot.subtitle = element_text(size = 12, face = "italic"),
        plot.caption = element_text(size = 12))
...
p7_small <- norm_stat_small_tbl |>
  ...
  labs(title = "Große Fallzahl (n = 40)",
       subtitle = "Nicht normalverteilt") +
  theme(plot.title = element_text(size = 17),
        plot.subtitle = element_text(size = 12, face = "italic"),
        plot.caption = element_text(size = 12))
...
p3_small + p6_small  + p9_small  +
    plot_layout(ncol = 3, heights = c(4, 4, 2, 4, 4, 2, 4, 4, 2)) +
