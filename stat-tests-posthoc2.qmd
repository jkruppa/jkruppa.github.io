...
Beginnen wir also mit der Festlegung welche Art der Analyse wir rechnen wollen. Wir schauen uns jetzt einmal das einfaktorielle sowie das zweifaktorielle Modell näher an. Du könntest auch mehr als zwei Faktoren auswerten wollen, aber dann musst du einmal in ein anderes Kapitel wechseln. Wir beginnen hier mit dem einfacheren Fall oder auch den häufigsten. Es kann auch sein, dass du eine numeriasche Kovariate mit in dem Modell hast.
...
Auf der Seite DSFAIR gibt es noch einen Artikel zu `emmeans` und der Frage [Why are the StdErr all the same?](https://schmidtpaul.github.io/dsfair_quarto/ch/summaryarticles/whyseequal.html)

::: callout-tip
## Literatur zur Überprüfung der Varianzhomogenität
...
#| fig-height: 4
#| fig-width: 4
#| fig-cap: "Barplot oder Säulendigramm der mittleren Sprungweiten in [cm] für drei Floharten und der entsprechenden Messorte. Die Fehlerbalken stellen die Standardabweichung dar. Die Buchstaben das Bonferroni adjustierte *Compact Letter Display*."
...
Eben gerade haben wir das `{emmeans}` Objekt für unsere zweifaktorielle Analyse für die getrennte Analyse über den zweiten Faktor genutzt. Jetzt zeige ich dir in den folgenden Tabs einmal wie du die p-Werte, das *Comapct letter display* und die 95% Konfidenzintervalle für die gemeinsame Analyse über den zweiten Faktor berechnest.
...
```{r}
eff_size(emm_fac1_obj, sigma = sigma(fac1_aov), edf = 23)
```
