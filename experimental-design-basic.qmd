...
Ein Alpha-Versuchsplan ist dafür gedacht ein Versuchsplan mit unvollständigen Blöcken abzubilden. Die Blöcke sind dann aber wiederum zu vollständigen Wiederholungen gruppiert. Solche Versuchspläne werden dann als auflösbar bezeichnet, also als berechenbar. Das Modell was wir dann anpassen würden muss daher einen Effekt für die vollständige Wiederholungen haben sowie Effekte für die unvollständige Blöcke. Die unvollständigen Blöcke müssen innerhalb der Wiederholungen verschachtelt sein.
...
Das Augmened design basiert darauf, dass wir neben den eigentlichen Behandlungen noch Standardbehandlungen vorliegen haben. Die Standardbehandlungen randomisieren wir voll auf die Blöcke, wohingegen die anderen Behandlungen unvollständig randomisiert werden.
...
```{r}
augmented_obj <- design.dau(trt1 = checks,
                            trt2 = trt_gen,
                            r = n_rep,
                            seed = 42)
```
...
```{r}
get_shades_of_blue <- colorRampPalette(colors=c("skyblue", "royalblue3"))
blue_15  <- get_shades_of_blue(15) 
orange_3 <- c("darkorange", "darkorange2", "darkorange3") 
blue_15_and_orange_3 <- c(blue_15, orange_3) 

names(blue_15_and_orange_3) <- augmented_book_tbl %>% 
...
desplot(trt ~ rows + cols, flip = TRUE,
        ...
        col.regions = blue_15_and_orange_3,  
        text = trt, cex = 1, shorten = "no",
        data = augmented_book_tbl,
        main = "Augmented design mit zwei Standards", 
        show.key = F)
```
