...
#| warning: false
#| message: false
pacman::p_load(tidyverse, readxl, knitr, kableExtra, see, modelr,
...
```{r}
fac2_tbl <- expand_grid(fa = 1:3, fb = 1:2, rep = 1:1000) |> 
  mutate(fa = as_factor(fa),
         fb = as_factor(fb))
...
eff_vec <- c(10, 2, 4, 6, 0, 10)

eff <- fac2_mat %*% eff_vec 

foo <- fac2_tbl |> 
  mutate(y = rpois(length(eff), eff))

foo |> 
  group_by(fa, fb) |> 
  summarise(round(mean(y)))
```
...
```{r echo = TRUE}
#| message: false
pacman::p_load(tidyverse, simstudy, simdata, modelr, conflicted)
```
