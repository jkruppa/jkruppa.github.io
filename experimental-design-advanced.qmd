...
Beide Behandlungsfaktoren sind ineinander und dann natürlich im Block genested. Hier aber erstmal das Modell mit den drei Faktoren für den besseren Überblick. Wir haben einmal die Düngung `fert`, dann den Faktor Boden `soil` sowie die verschiedenen Blöcke mit `block`.
...
flowchart LR
    A(fert):::factor --- B(((nested))) --> C(soil):::factor --- D(((nested))) --> E(cols) --- F(block):::factor
    classDef factor fill:#56B4E9,stroke:#333,stroke-width:0.75px
```
...
splitplot_long_tbl <- expand_grid(block = 1:2, 
  ...
  mutate(block = factor(block, labels = str_c("Block ", 1:2)),
         soil = factor(soil, labels = str_c("soil_", 1:4))) %>% 
  group_by(block) %>% 
  mutate(soil = sample(soil)) %>% # Randomiseren von soil in block
  expand_grid(fert = 1:6) %>%   
  mutate(fert = sample(fert),     # Randomiseren von fert in soil
         fert = factor(fert, label = str_c("fe_", 1:6)))
...
%%| fig-cap: "text"
flowchart LR
    A(tray) --- B(((nested))) --> C(trt):::factor --- D(((nested))) --> E(cols) --- F(block):::factor
    classDef factor fill:#56B4E9,stroke:#333,stroke-width:0.75px
```
...
%%| fig-cap: "text"
flowchart LR
    C(trt):::factor -.- D(((nested))) -.-> E(block):::factor
    classDef factor fill:#56B4E9,stroke:#333,stroke-width:0.75px
```
...
%%| fig-cap: "text"
flowchart LR
    A(trt_gen):::factor --- B(((nested))) ---> C(rows) --- E(block):::factor
    F(trt_nitro):::factor --- G(((nested))) ---> H(cols) --- E
    classDef factor fill:#56B4E9,stroke:#333,stroke-width:0.75px
```
