...
Können wir uns auch signifikante Unterschiede anzeigen lassen?

```{r}
sig_letters <- c("Haus", "Dog", "Cat", "zeit") # as.character(as.roman(1:5))
sig_letters
```
...
small_fit %>% 
  emmeans(~ trt) %>%
  cld(adjust = "bonferroni", signif = TRUE) %>% 
  pull(.signif.set) %>% str_trim() %>%  str_split("") %>% unlist() %>% as.roman
```
:::
