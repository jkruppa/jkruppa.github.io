...
Dann können wir die Funktion `genCorData()` nutzen um uns korreltierte Kovariaten zu erstellen. Ich habe hier einmal eintausend Beobachtungen gewählt. Dann müssen wir noch den Mittelwert in allen Kovariaten festlegen sowie die Streuung. Wir nehmen dann die Korrelatiuonsmatrix mit in die Funktion und benennen noch die Kovariaten entsprechend. Wir erstellen uns dann den Messwert separat mit den gewollten Effekten.
...
Wir bauen uns hier erstmal das Datenraster für den Faktor auf, da wir dann das Raster mit dem R Paekt `{simstdy}` erweitern und mit einem poissonverteilten Messwert ergänzen.. Das heißt, wir nutzen die Funktion `expand_grid()` um uns für die drei Level jeweils eintausend Beobachtungen oder Wiederholungen zu erstellen. Da wir gleich mit `{simstudy}` weiterarbeiten brauche ich das Datenraster als ein `data.table` Objekt.
...
Jetzt kommen wir zu den etwas interessanteren Design mit dem zweifakotriellen Experiment ohne Interaktion. Wir haben jetzt hier zwei Faktoren $f_A$ und $f_B$ mit jeweils drei oder zwei Leveln vorliegen. Die beiden Faktoren sind untereinander unabhängig, daher haben wir hier keien interaktion vorliegen. Wir wollen jetzt einmal eintausend poissonverteilte Messwerte generieren. Dafür nutze ich wie immer einmal das R Paket `{simstudy}` sowie den Weg über die Funktion `rpois()`. In Excel können wir keine Possiondaten erschaffen. Hier müssen wir dann die Daten aus einer Normalverteilung entsprechend runden.
...
Eine Frage des Nestings oder der Schachtelung.
...
```{r}
addColumns(y_pois_def, fac2_dt) |> as_tibble()
```
:::

#### Zweifaktoriell mit Interaktion $f_A + f_B + f_A \times f_B$ {.unnumbered .unlisted}

Bitte wie die Normalverteilung und am Ende runden.
...
#### Zero-inflated {.unnumbered .unlisted}

[Adding a "mixture" distribution to the simstudy package](https://www.rdatagen.net/post/adding-mixture-distributions-to-simstudy/)

```{r}
zeroinf_def <- defData(varname = "y_0", formula = 0, dist = "nonrandom")
zeroinf_def <- defData(zeroinf_def, varname = "y", formula = 2, dist = "poisson")
...
p1_zeroinf <- zeroinf_dt |> 
  ...
  labs(x = "Poisson", y = "Anzahl", fill = "") +
  scale_x_continuous(breaks = 0:7) +
  ylim(0, 310)

p2_zeroinf <- zeroinf_dt |> 
  ggplot(aes(x = y_zeroinf)) +
  theme_modeling() +
  geom_histogram(fill = "#56B4E9", size = 3) +
  labs(x = "Zero inflated Poisson", y = "Anzahl", fill = "")  +
  scale_x_continuous(breaks = 0:7) +
  ylim(0, 310)
