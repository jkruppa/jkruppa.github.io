...
Da wir in unseren Schweine Daten jetzt keine Spalten mit einer sehr kleinen Varianz haben, passiert auch nichts, wenn wir die Funktion auf unsere Daten anwenden würden.

## Standardisieren $\mathcal{N}(0,1)$ {#sec-preprocess-standard}

In dem Kapitel zu der [Transformation von Daten](#sec-eda-transform) haben wir ja schon von der Standardisierung gelesen und uns mit den gänigigen Funktion beschäftigt. Deshalb hier nur kurz die Schritte, die wir machen könen. Zum einen können wir nur die Daten mit der Funktion `step_scale()` skalieren, dass heißt auf eine Standardabweichung von 1 bringen. Oder aber wir nutzen die Funktion `scale_center()` um die Daten alle auf einen Mittelwert von 0 zu schieben. Das können wir dann einfach auf allen *numerischen* Prädiktoren durchführen.
...
Wenn wir aber darauf keine Lust haben, gibt es auch die etwas schief benannte Funktion *step_normalize()*, die beide Schritte kombiniert und uns damit die Daten auf eine Standardnormalverteilung transformiert. Ich persönlich nutze dann meist die zweite Variante, dann hat man alles in einem Schritt zusammen.
...
Jetzt können wir noch die Daten generieren und sehen, dass wir alle numerischen Spalten in eine Standardnormalverteilung transformiert haben.
...
Auh hier möchte ich wieder auf das Kapitel zum [Transformation von Daten](#sec-eda-transform) verweisen. In dem `tidymodels` Universum heißt dann das Normalisieren, also die Daten auf eine Spannweite zwischen 0 und 1 bringen, dann eben `step_range()`. Das ist natürlich dann schön generalisiert. Wir könnten uns auch andere Spannweiten überlegen, aber hier nehmen wir natürlich immer den Klassiker. Unsere Daten liegen dann nach der Normalisierung mit der Funktion `step_range()` zwischen 0 und 1. Wir können die Normalisierung natürlich nur auf numerischen Variablen durchführen.
...
Auch hier können wir dann die Daten generieren und uns einmal anschauen. Im Gegensatz zu der Standardisierung treten jetzt in unseren Spalten keine negativen Werte mehr auf.
...
Dann können wir uns auch schon die Daten generieren. Wir sehen, dass wir keine fehlenden Werte mehr in unseren Daten vorliegen haben.
