...
```{r}
#| message: false
snake_tbl <- tibble(mass = c(6, 8, 5, 7, 9, 11, 12, 10),
...
In der @tbl-snakes ist der Datensatz `snake_tbl` nochmal dargestellt. Wir haben die Schlangenlänge `svl` als Outcome $y$ sowie das Gewicht der Schlangen `mass`, die Sammelregion `region` und die Farbe der Schlangen `color`. Dabei ist `mass` eine kontinuierliche Variable, `region` eine kategorielle Variable als Fakotr mit zwei Leveln und `color` eine kategorielle Variable als Faktor mit drei Leveln.
...
```{r}
fit_1 <- lm(svl ~ mass + region + color, data = snake_tbl) 
fit_1 %>% coef %>% round(2)
...
```{r}
model.matrix(svl ~ mass + region, data = snake_tbl) %>% as_tibble
```
...
```{r}
fit_2 <- lm(svl ~ mass + region, data = snake_tbl) 
fit_2 %>% coef %>% round(2)
...
$$
 ...
 =
  \begin{pmatrix}
  31.23 & \phantom{0}6 \cdot 1.6 & 0 \cdot 6.53  \\
  31.23 & \phantom{0}8 \cdot 1.6 & 0 \cdot 6.53  \\
  31.23 & \phantom{0}5 \cdot 1.6 & 0 \cdot 6.53  \\
  31.23 & \phantom{0}7 \cdot 1.6 & 1 \cdot 6.53  \\
  31.23 & \phantom{0}9 \cdot 1.6 & 1 \cdot 6.53 \\
  31.23 & 11\cdot 1.6 & 1 \cdot 6.53 \\
  31.23 & 12\cdot 1.6 & 1 \cdot 6.53 \\
  31.23 & 10\cdot 1.6 & 1  \cdot 6.53 \\
 \end{pmatrix} +
  \begin{pmatrix}
  -0.80 \\
  +1.01 \\
...
```{r}
c(31.23 +  6*1.6 + 0*6.53 - 0.80,
  31.23 +  8*1.6 + 0*6.53 + 1.01,
  31.23 +  5*1.6 + 0*6.53 - 0.21,
  31.23 +  7*1.6 + 1*6.53 + 2.07,
  31.23 +  9*1.6 + 1*6.53 - 0.12,
  31.23 + 11*1.6 + 1*6.53 + 1.68,
  31.23 + 12*1.6 + 1*6.53 + 1.09,
  31.23 + 10*1.6 + 1*6.53 - 4.72) %>% round()
```
