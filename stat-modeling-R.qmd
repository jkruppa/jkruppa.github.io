...
#| fig-height: 1.5
#| fig-width: 7
#| fig-cap: "Statistische Modellschreibweise in R mit dem Messwert auf der linken Seite und den Einflussvariablen auf der rechten Seite der Tilde. Die Platzhalter $Y$ und $X$ werden durch die Spaltennamen im Datensatz erstetzt. *[Zum Vergrößern anklicken]*"
...
::: callout-important
## Worte und Bedetungen im statistischen Modellieren
...
|----|:--:|----|----|
| *LHS* | $Y$ | Messwert / Endpunkt | Response / outcome / endpoint |
| *RHS* | $X$ | Einflussvariable / Erklärende Variable | risk factor / explanatory |
...
: Zusammenfassende Tabelle des Sprachgebrauchs in meinem statistischen Modellieren. {#tbl-namen-regression tbl-colwidths="\[14, 10, 38, 38\]"}
:::
...
#| fig-height: 2.5
#| fig-width: 7
#| fig-cap: "Übersicht der Namen der Funktionen in R für das lm(), glm() und glmer(). *[Zum Vergrößern anklicken]*"
...
```{r}
fac1_tbl <- tibble(f_a = gl(2, 5),
                   y = 2 + 2 * as.numeric(f_a) + rnorm(10, 0, 1))
```
...
```{r}
fac1_tbl <- tibble(f_a = gl(3, 5),
                   y = 2 + 2 * as.numeric(f_a) + rnorm(15, 0, 1))
```
...
cov1_fac1_tbl <- tibble(c_1 = rnorm(15, 0, 1),
                        f_a = gl(3, 5),
                        y = 2 + 1 * c_1 + 2 * as.numeric(f_a) + rnorm(15, 0, 1))
```
...
#| fig-height: 2.5
#| fig-width: 9.5
#| fig-cap: "Wie bilde ich den Namen einer Regression? Erst beschreiben wir das $x$, dann das $y$. Am Ende müssen wir noch sagen, ob wir ein gemischtes Modell vorliegen haben oder nicht. *[Zum Vergrößern anklicken]*"
