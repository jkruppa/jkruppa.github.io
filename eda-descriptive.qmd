...
: Zusammenfassung und Übersicht von Variablennamen und deren Bennung in R, in der Informatik allgemein, als Skalenniveau und die dazugehörige Verteilungsfamilie. {#tbl-skalenniveau-desc}
:::
...
```{r}
#| warning: false
dog_cat_tbl <- read_excel("data/flea_dog_cat.xlsx")  
```

```{r}
gtsummary::tbl_summary(dog_cat_tbl, by = animal)
```

```{r}
modelsummary::datasummary_skim(dog_cat_tbl)
```

```{r}
modelsummary::datasummary_skim(dog_cat_tbl, type = "categorical")
```
...
f <- jump_length ~ animal * (mean + sd)

datasummary(f, data = dog_cat_tbl, output = 'table.docx')
```
