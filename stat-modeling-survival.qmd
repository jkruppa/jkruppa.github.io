...
fit <- survfit(Surv(dol, death) ~ trt, data = dodo_tbl)
...
fit <- survfit(Surv(time, status) ~ trt, data = fruitfly_tbl)
# Visualize with survminer
...
fit <- survfit(Surv(time, status) ~ weight_bin, data = fruitfly_tbl)
# Visualize with survminer
ggsurvplot(fit, data = fruitfly_tbl, risk.table = TRUE,
           surv.median.line = "hv",
           ggtheme = theme_light(),
...
# Visualize with survminer
ggsurvplot(fit, data = lung_tbl, risk.table = TRUE,
           surv.median.line = "hv",
           ggtheme = theme_light(),
           palette = cbbPalette[2:8])
```
