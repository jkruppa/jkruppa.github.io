...
Du findest dann auch weitere Optionen in `{ggplot}` im [Kapitel zum `{ggplot}` Cookbook](#sec-ggplot-cookbook). Ich habe einen Teil der Optionen dann in das Programmieren ausgelagert, da wir dann dort mehr machen können und dieses Kapitel nicht zu voll wird. Es ist auch so, dass du in allen anderen Kapiteln zur Anwendung und statistsichen Analyse dann entsprechende Visualisierungen in `{ggplot}` findest. Du musst dann immer schauen, was zu deiner wissenschaftlichen Fragestellung passt.
