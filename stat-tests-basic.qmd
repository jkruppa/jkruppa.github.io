...
#| echo: false
#| message: false
pacman::p_load(tidyverse, readxl, knitr, kableExtra, broom, patchwork, conflicted)
...
#| fig-height: 7
#| fig-width: 7
#| fig-cap: "Work in progress. *[Zum Vergrößern anklicken]*"
...
In @fig-test-basic-dist-01 ist die Verteilung aller möglichen $T_{calc}$ Werte unter der Annahme, dass die Nullhypothese wahr ist, dargestellt. Wir sehen, dass die t-Verteilung den Gipfel bei $T_{calc} = 0$ hat und niedrigere Werte mit steigenden Werten der Teststatistik annimmt. Wenn $T = 0$ ist, dann sind auch die Mittelwerte gleich. Je größer unsere berechnete Teststatistik $T_{calc}$ wird, desto unwahrscheinlicher ist es, dass die Nullhypothese gilt.
...
#| fig-height: 4.5
#| fig-width: 7
#| fig-cap: "Die t-Verteilung aller möglichen Teststatistiken $T$ wenn die Nullhypothese wahr ist zusammen mit einer signifikanten Teststaistik $T_{calc}$. Wenn kein Effekt vorliegt sind die beiden Mittelwerte $\\bar{y}_1$ und $\\bar{y}_2$ in etwa gleich groß. Die Differenz der Mittelwerte wäre 0 und somit auch die Teststatistik $T$. Je größer der $T_{calc}$ wird desto weniger können wir davon ausgehen, dass die beiden Mittelwerte gleich sind. Liegt der $T_{calc}$ über dem kritischen Wert von $T_k$ dann können wir die Nullhypothese ablehnen. *[Zum Vergrößern anklicken]*"
...
ggplot(data.frame(x = c(-3.25, 3.25)), aes(x)) +
  ...
  labs(x = "", y = "",
       title = expression(Signifikante~Teststatistik~T[calc]),
       subtitle = expression(Die~berechnete~Teststatistik~T[calc]~ist~größer~als~der~kritische~Wert~T[k]))
...
#| fig-height: 3.5
#| fig-width: 7
#| fig-cap: "Die t-Verteilung aller möglichen $T$ wenn die Nullhypothese wahr ist. Daher liegt kein Effekt vor und die beiden Mittelwerte $\\bar{y}_1$ und $\\bar{y}_2$ wären in etwa gleich groß. Die Differenz der Mittelwerte wäre 0. Je größer der $T_{calc}$ wird desto weniger können wir davon ausgehen, dass die beiden Mittelwerte gleich sind. Liegt der $T_{calc}$ über dem kritischen Wert von $T_{\\alpha = 5\\%}$ dann wir die Nullhypothese abgelehnt. *[Zum Vergrößern anklicken]*"
...
ggplot(data.frame(x = c(-3.25, 3.25)), aes(x)) +
  ...
  annotate("text", x = 2.5, y = 0.15, label = expression(alpha==2.5*'%'), size = 6,
           color = "#CC79A7") +
  annotate("text", x = 2.5, y = 0.25, label = expression(Pr(T[calc]*'|'*H[0])), size = 6,
           color = "#0072B2") +
...
#| fig-height: 7
#| fig-width: 11
#| fig-cap: "Wir vergleichen bei der Entscheidung mit dem $p$-Wert nicht die berechnete Teststatistik $T_{calc}$ mit dem kritischen Wert $T_{\\alpha = 5\\%}$ sondern die Flächen rechts von den jeweiligen Teststatistiken mit $A_K = 5\\%$ und $A_{calc}$ als den $p$-Wert. Die Flächen links und rechts von $T_{\\alpha = 2.5\\%}$ sind nochmal separat dargestellt. An dem Flächenvergleich machen wir dann die Testentscheidung fest. *[Zum Vergrößern anklicken]*"

p1 <- ggplot(data.frame(x = c(2.5, 4)), aes(x)) +
  theme_minimal() +
  stat_function(fun = dnorm, color = "#CC79A7", linewidth = 1) + 
  ...
  scale_x_continuous(breaks = 2.5, label = expression(+T[k])) +
  labs(x = "", y = "")
...
p3 <- ggplot(data.frame(x = c(2.5, 4)), aes(x)) +
  ...
  theme(axis.text.y = element_blank(),
        axis.text = element_text(size = 16)) +
  scale_x_continuous(breaks = 3.1, label = expression(T[calc]),
                     limits = c(1.5, 4)) +
...
|                   | $T_{calc}$ | $Pr(\geq T_{calc}|H_0)$ | $KI_{1-\alpha}$ |                     | $T_{calc}$ | $Pr(\geq T_{calc}|H_0)$ | $KI_{1-\alpha}$ |
|:-----------------:|:----------:|:-----------------------:|-----------------|:-------------------:|:----------:|:-----------------------:|-----------------|
