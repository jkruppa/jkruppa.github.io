...
Wir können auch hier für eine mögliche Varianzheterogenität anpassen und entsprechend eine Varianzschätzer mit der Option `vcov.` wählen. Ich habe hier den Standard gewählt. Am Ende dieses Kapitels kannst du dann nochmal über andere Varianzschätzer lesen. Leider sehen wir die Anpassung nicht in der Ausgabe von `glht`, was immer etwas nervig ist.

```{r}
multcomp_fac1_obj <- glht(fac1_fit, linfct = mcp(animal = "Tukey"), vcov. = sandwich::vcovHC) 
```
