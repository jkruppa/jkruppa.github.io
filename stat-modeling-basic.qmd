...
#| fig-width: 7
#| fig-cap: "foo"
#| label: fig-model-c2-plotly

plot_ly(flea_model_tbl, x = ~weight, y = ~jump_length, 
               z = ~count_leg, size = 1) |> 
  add_markers() |> 
  layout(scene = list(xaxis = list(title = 'Gewicht in [mg]'),
                      yaxis = list(title = 'Sprungweite in [cm]'),
                      zaxis = list(title = 'Mittlere Anzahl Beinhaare')))
```
