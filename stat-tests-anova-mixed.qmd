...
Hier möchte ich dann nur noch kurz auf das R Paket `{MANOVA.RM}` eingehen, da wir hier dann nur die Informationen aus dem Abschnitt `ANOVA-Type Statistic (ATS)` brauchen. Wir erhalten ähnliche Ergebnisse wie schon in den anderen Algorithmen. Leider erhalten wir hier dann keinen Effektschätzer mehr, sondern eben nur die deskriptiven Statistiken.
