...
ologit_fit %>% tbl_regression() %>% as_flex_table()
```
...
multinom_fit %>% tbl_regression() %>% as_flex_table()
```
...
#| message: false
#| warning: false
pig_lst <- list(robust_frail = filter(pig_tbl, frailty_fac %in% c("robust", "frail")),
                robust_prefrail = filter(pig_tbl, frailty_fac %in% c("robust", "pre-frail")),
