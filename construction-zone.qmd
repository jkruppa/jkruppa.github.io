...
#| message: false
#| warning: false
pacman::p_load(tidyverse, gtsummary, marginaleffects, emmeans, 
               conflicted)
...
p2 <- ggplot(flea_model_tbl, aes(x = weight, y = jump_length)) +
  ...
  geom_richtext(aes(x = 15, y = 112, 
                    label = "f(x) = 0 + 10.62x - 0.32x²<br>f'(x) = 10.62 - 0.64x")) +
...
ggplot(flea_model_tbl, aes(x = weight, y = jump_length)) +
  ...
  geom_function(fun = \(x) 0 + 10.62 * x -0.3152609 * x ^2, color = "blue") + 
  labs(x = "Gewicht in [mg]", y = "Sprungweite in [cm]") 
