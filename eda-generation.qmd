...
```{r}
cov_mat <- matrix(c(3, 2.5, 2.5, 3), ncol=2,
                  dimnames = list(c("c_1", "c_2"), 
                                  c("c_1", "c_2")))
```
...
#| message: false
#| warning: false
cov2_dependent_tbl <- rmvnorm(n= 500, mean = c(1, 2), sigma = cov_mat) |> 
  as_tibble() |> 
  set_names(c("c_1", "c_2")) |> 
  mutate(y = 2 + 1.5 * c_1 + 3 * c_2 + rnorm(length(c_1), 0, 1))
```

```{r}
correlation(cov2_dependent_tbl, select = c("c_1", "c_2"))
```

```{r}
lm(y ~ c_1 + c_2, data = cov2_dependent_tbl)
```
