...
Im Folgenden schauen wir uns einmal eine Auswahl an Kriterien für die Detektion von Ausreißern an. Ich nutze dazu dann gleich einmal einen Dummy-Datensatz in dem ich mir einfach mal zwei Ausreißer hineinbaue und wir schauen dann mal, was die Algorithmen so wiedergeben. Du findest noch mehr Möglichkeiten in dem Tutorium [Outliers detection in R](https://statsandr.com/blog/outliers-detection-in-r/). Wir wollen uns hier aber auf ein paar der gängigsten Varianten konzentrieren. In den folgenden Abschnitten danach, schauen wir dann nochmal spezifisch in einzelne R Paket hinein.
...
Veranschaulichen wir uns einmal den Zusammenhang an zwei Beispieldaten in der @tbl-cook-tables. Ich nehme dazu einfach das Gewicht in \[mg\] von zehn Flöhen und die jeweilige Sprunglänge in \[cm\]. Als erstes einmal einen Datensatz ohne Ausreißer und dann einen Datensatz mit den Sprungweiten von $56$cm und $2$cm als Ausreißer.
...
no_out_tbl <- tibble(weight = c(1.1, 2.3, 2.1, 3.7, 4.1, 5.4, 7.6, 4.3, 5.8, 8.1),
                     jump_length = c(22, 23, 24, 23, 19, 22, 18, 26, 24, 22))
...
| $\mu \pm 5 \cdot \sigma$ |           $99.999943\%$           |  $1:1.7 \cdot 10^6$   |
| $\mu \pm 6 \cdot \sigma$ |          $99.9999996\%$           |  $1:2.5 \cdot 10^8$   |

: Test {#tbl-outlier-sigma}
