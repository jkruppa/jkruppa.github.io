...
Wir berechnen einen Mittelwert von den Epsilons mit $\bar{\epsilon} = `r mean(epsilon_vec)`$. Ein Mittelwert nahe Null bzw. von Null wundert uns nicht. Wir haben die Gerade ja so gebaut, das nach oben und unten die gleichen Abstände sind. Die Varianz der $y$ ist `r var(val_vec)` und die Varianz von $\epsilon$ ist `r var(epsilon_vec)`. In beiden Fällen ist die Zahl gleich. Wir können uns merken das die Epsilons einen Mittelwert von 0 haben und eine Varianz von $s_y^2$.
