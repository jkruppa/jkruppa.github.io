...
In diesem Kapitel wollen wir uns mit "pseudo" Zeitreihen beschäftigen. Wenn dich echte Zeitreihen interessieren, dann schaue bitte einma in das nächste Kapitel [Zeitreihen (eng. *time series*)](#sec-time-series). Wenn dir jetzt unklar ist, was da der Unterschied sein soll, dann versuche ich dir gleich einmal die Sachlage zu erklären. Zuerst haben wir ganz einfach auf der $x$-Achse einer potenziellen Visualisierung die Zeit dargestellt. Wir wollen dann auswerten, ob es über den zeitlichen Verlauf einen Trend gibt oder wir ein gutes Modell für den Verlauf der Beobachtungen anpassen können. Dabei haben wir dann aber meistens nicht einen super simplen Verlauf, sondern Spitzen oder Täler in den Daten, so dass wir hier die Daten entsprechend glätten (eng. *to smooth*) müssen.
...
```{r}
#| cache: true
#| label: time-series-zer-cow-milk
#| eval: false
cow_milk_tbl <- read_excel("data/time_series_cow_data.xlsx") %>% 
  mutate(time = as.Date(time),
         cow_id = as_factor(cow_id),
         milk_log10 = log10(milk)) %>% 
  filter(time >= "2011-01-01" & time < "2021-01-01") %>% 
  filter(milk > 0)
```
