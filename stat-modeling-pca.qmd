...
#| fig-height: 4.5
#| fig-width: 4.5
#| fig-cap: "Übersicht über die Hauptkomponentenanalyse und deren ersten beiden Hauptkomponenten."
#| fig-subcap: 
...
pacman::p_load(tidyverse, magrittr, readxl, patchwork,
               ...
               janitor, corrplot, HDclassif, klaR,
               conflicted)
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Zusammenhang von den `cos2`-Werten und den jeweiligen Hauptkomponenten"
#| fig-subcap: 
#|   - "Tierdaten `animals_df`"
#|   - "Kreaturendaten `std_creature_df`"
#| layout-nrow: 1
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Zusammenhang von den `cos2`-Werten und die ersten beiden Hauptkomponenten `Dim.1` und `Dim.2`."
#| fig-subcap: 
#|   - "Tierdaten `animals_df`"
#|   - "Kreaturendaten `std_creature_df`"
#| layout-nrow: 1
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Korrelationsdiagramm der Variablen."
#| fig-subcap: 
#|   - "Tierdaten `animals_df`"
#|   - "Kreaturendaten `std_creature_df`"
#| layout-nrow: 1
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Korrelationsdiagramm der Variablen eingefärbt den Gruppen aus dem k-NN Algorithmus zur Clusterbildung."
#| fig-subcap: 
#|   - "Tierdaten `animals_df`"
#|   - "Kreaturendaten `std_creature_df`"
#| layout-nrow: 1
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Beitrag der einzelnen Variablen zu den ersten beiden Hauptkomponenten."
#| fig-subcap: 
#|   - "Tierdaten `animals_df`"
#|   - "Kreaturendaten `std_creature_df`"
#| layout-nrow: 1
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Streudiagramm der individuellen Beobachtungen für die ersten beiden Hauptkomponenten."
#| fig-subcap: 
#|   - "Tierdaten `animals_df`"
#|   - "Kreaturendaten `std_creature_df`"
#| layout-nrow: 1
...
#| fig-height: 5
#| fig-width: 5
#| fig-cap: "Biplot für die Individuen und Variablen eingefäerbt nach Clustern."
#| fig-subcap: 
#|   - "Tierdaten `animals_df`"
#|   - "Kreaturendaten `std_creature_df`"
#| layout-nrow: 1
...
Dann einmal ein schneller Blick auf die Daten. Wir werden hier die Daten nicht standardisieren, obwohl es sicherlich Sinn macht das zu tun. Aber dann wird das hier alles noch länger als sowieso schon.
...
Die folgenden Methoden der Diskriminanzanalyse werden beschrieben:

-   **Lineare Diskriminanzanalyse (LDA):** Verwendet Linearkombinationen von Prädiktoren zur Vorhersage der Klasse einer gegebenen Beobachtung. Es wird davon ausgegangen, dass die Prädiktorvariablen (p) normal verteilt sind und die Klassen identische Varianzen (bei univariater Analyse, p = 1) oder identische Kovarianzmatrizen (bei multivariater Analyse, p \> 1) haben.
-   **Quadratische Diskriminanzanalyse (QDA):** Sie ist flexibler als die LDA. Hier gibt es keine Annahme, dass die Kovarianzmatrix der Klassen gleich ist.
-   **Mischungsdiskriminanzanalyse (MDA):** Es wird angenommen, dass jede Klasse eine Gaußsche Mischung von Unterklassen ist.
-   **Flexible Diskriminanzanalyse (FDA):** Es werden nichtlineare Kombinationen von Prädiktoren verwendet, wie z. B. Splines.
-   **Regulierte Diskriminanzanalyse (RDA):** Die Regularisierung (oder Schrumpfung) verbessert die Schätzung der Kovarianzmatrizen in Situationen, in denen die Anzahl der Prädiktoren größer ist als die Anzahl der Stichproben in den Trainingsdaten. Dies führt zu einer Verbesserung der Diskriminanzanalyse.
...
partimat(class ~ flavanoids + nonflavanoid_phenols + od280_od315_of_diluted_wines, data = wine_tbl, 
         method = "lda")
...
(eng. *Quadratic discriminant analysis* )
...
(eng. *Mixture discriminant analysis*)
...
Achtung, Wir wählen nur die ersten beiden Dimensionen!
...
(eng. *Flexible Discriminant Analysis*)
...
(eng. *Regularized discriminant anlysis*)
