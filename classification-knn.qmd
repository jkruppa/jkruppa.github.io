...
::: column-margin
Wenn wir eine ID Spalte haben, dann können wir auch über die Funktion `sample_frac()` und dem Anteil der ausgewählten Beobachtungen und der Funktion `anti_join()`, die Trainings- und Testdaten erstellen.

```{r}
train <- pig_tbl %>% 
  sample_frac(0.70)
test  <- anti_join(pig_tbl,
                   train, 
                   by = 'pig_id')
```
:::
