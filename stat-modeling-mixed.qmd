...
In der @fig-mixed-yates-1-inter sehen wir einmal die Interaktionsplots für die verschiedenen möglichen Interaktionen zwischen den Faktoren der Stickstoffdünung, der Weizensorte und den Blöcken. @fig-mixed-yates-1-inter-1 zeigt kalr, dass es keine Interaktion zwischen der Stickstoffdüngung und den Sorten gibt. Die Graden laufen parallel zueinander. Wir haben einen mittleren Effekt der Stickstoffdünung, da wir einen Anstieg beobachten. Dennoch ist die Ordnung der Sorten pro Level der Stickstoffdüngun gleich. Würden sich die graden überschneiden, hätten wir eine Interaktion vorliegen.
...
```{r}
## das wäre dann das Modell mit gestrichelt anzeigen lassen???
library(modelbased)

model <- lm(plant_yield ~ nitro + gen + nitro*gen, data = oats_tbl)
means_lm <- estimate_means(model)
lm_oats_tbl <- means_lm  %>% as_tibble %>% 
  add_column(.id = "naive_lm")

means_lmer <- estimate_means(oats_lmer_fit)
lmer_oats_tbl <- means_lmer  %>% as_tibble %>% 
  add_column(.id = "lmm")


bind_rows(lm_oats_tbl, lmer_oats_tbl) %>% 
  mutate(.id = as_factor(.id)) %>% 
  ggplot(aes(nitro, Mean, line_type = .id, color = gen, group = gen)) +
  stat_summary(fun = mean, geom = "point") +
  stat_summary(fun = mean, geom = "line") 
