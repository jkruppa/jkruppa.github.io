...
Wie du in der Vignette [Modify components of a theme](https://ggplot2.tidyverse.org/reference/theme.html) nachlesen kannst, geht immer eine Menge, wenn es um die Änderung der Abbildung geht. Ich nutze dann immer folgendes Themplate, wenn es dann um meine Abbildungen geht. Daher verwende ich dann statt `theme_minimal()` in meinen Abbildungen immer `theme_modeling()`, wenn es um Modelle geht. Ich habe dann noch andere Themen in anderen Kapiteln definiert, aber das hier ist dann mein Template.
