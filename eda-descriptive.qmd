...
Nun ist es so, dass wir nicht jeden Vektor an Zahlen gleich zusammenfassen können. Wir du schon in dem [Kapitel über die Zahlen und Buchstaben in R](#sec-letter-number) gesehen hast unterscheidet R Zahlen je nach Eigenschaften der Zahlen. Das macht aber nicht nur R so, sondern jedes statistische Programm. Nicht jede statistische Maßzahl passt zu jeder Zahl. Es gibt es nicht nur "Zahlen", sondern auch kontinuierliche Zahlen oder aber kategoriale Zahlen. Eine Besonderheit stellen Zahlen dar, die eine Eigenschaft von Tieren nur mit zwei Ausprägungen beschreiben.
...
Soweit so kompliziert. Wir gehen jetzt aber mal die gängigen statistischen Maßzahlen für verschiedene Zahlen, die wir so erheben können, durch. Dabei fokussieren wir einmal auf die händische Berechnung und ich zeige dann aber auch die Berechnung in R. Dabei nehmen wir erstmal den Hundeflohdatensatz als Beispiel für die händische Berechung und für die Automatisierung am Ende des Kapitels unseren Gummibärchendatensatz.
...
```{r}
#| warning: false
flea_dog_tbl <- read_excel("data/flea_dog_cat.xlsx") %>% 
  filter(animal == "dog")
```
...
```{r}
#| label: tbl-dog-desc
#| tbl-cap: Tabelle  der Sprunglängen [cm], Anzahl an Flöhen, Boniturnote sowie der Infektionsstatus von Hundeflöhen.
#| echo: false
#| warning: false
flea_dog_tbl %>%
  kable(align = "c", "pipe")
...
Im Bereich der Agrarwissenschaften kommt der Variationskoeffizient eher selten vor. Der Variationskoeffizient ist eben eine statistische Maßzahl des Labors. Aber dennoch kommt der Variationskoeffizient immer mal wieder vor.

## Häufigkeiten

Neben der Darstellung von kontinuierlichen Zahlen, wollen wir auch häufig kategoriale Zahlen sinnvoll darstellen und zusammenfassen. Zwar können wir auch ohne weiteres die mittlere Note berechnen, aber meistens wollen wir dann doch wissen, wie viele Noten jeweils vergeben worden. Auch wenn wir uns mit einer $0/1$ Variable beschäftigen, also zum Beispiel infiziert ja/nein, wollen wir diese Variablen zusammenfassen. Wir wollen hier also Häufigkeiten (eng. *frequency*) berechnen und darstellen.

Fangen wir also einmal mit der Variable `infected` an. Wir haben in dieser Variable abgelegt, ob der Hund in unserem Beispiel mit einem Floh infiziert ist `1` oder eben nicht infiziert ist `0`. Wie du siehst, kodieren wir das "Ja" mit einer 1 und das "Nein" mit einer 0.
...
Schauen wir uns als einmal die drei Möglichkeiten die kategoriallen Daten als Häufigkeiten abzubilden an. Wir können zum einen die absoluten Häufigkeiten berichten. Wir sagen also ganz einfach, wie viele Hunde sind von der Gesamtheit infiziert.

Absolute Häufigkeit

:   Wir zählen die Anzahl an infizierten Tieren und berichten die Anzahl zusammen mit der gesmat Anzahl an Tieren. Also haben wir dann als absolute Häufigkeit $f_{absolut} = n_{infiziert} / n_{gesamt}$ zu berichten.
...
Neben der klassischen Häufigkeit, die wir durch die prozentuale Angabe kennen, gibt es auch noch die Chance. Die Chance infiziert zu sein, ist was anderes als die Wahrscheinlichkeit infiziert zu sein. Die Wahrscheinlichkeit ist eine relative Häufigkeit. Die Chance berechnet sich dabei etwas anders.
...
$$
y_{infected} = 3 : 4 = 0.75
$$

Wenn wir die Häufigkeiten berechnen wollen, dann nutzen wir auch wieder R. Es gibt viele Funktionen in R, die wir nutzen könne. Wir erstellen eine Tabelle in R mit der Funktion `tabyl()` aus dem R Paket `janitor`. Es gibt noch andere Möglichkeiten, die schauen wir uns aber gleich im Anschluss an.
...
Wir erhalten das Objekt `gummi_tbl` mit dem Datensatz in @tbl-data-dist-gummi nochmal als Auszug dargestellt. Wir brauchen aber nicht alle Spalten aus dem ursprünglichen Datensatz und somit ist die Tabelle etwas übersichtlicher.
...
[Hilfeseite von `gtsummary`](https://www.danieldsjoberg.com/gtsummary/)
...
gummi_tbl %>%
  tbl_summary(
    ...
    label = count_bears ~ "Anzahl Bären",
...
Fehlende Daten müssen raus

[Hilfeseite von `modelsummary`](https://modelsummary.com/index.html)
...
[`datasummary`: Crosstabs, frequencies, correlations, balance (a.k.a. "table 1"), and more](https://modelsummary.com/articles/datasummary.html)

Und auch ganz einfach rausschreiben [`datasummary` als \*.docx abspeichern](https://modelsummary.com/articles/datasummary.html#output)

```{r}
f <- height ~ gender * (mean + sd)
```

```{r}
#| message: false
#| warning: false
datasummary(f, data = gummi_clean_tbl) #, output = 'table.docx')
```

::: callout-tip
## Was gibt es noch an Möglichkeiten?

[Alternativen zu `gtsummary` und `modelsummary`](https://modelsummary.com/index.html#alternative-packages)
:::
