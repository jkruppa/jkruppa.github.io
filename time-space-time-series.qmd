...
-   [timetk for R](https://business-science.github.io/timetk/) -- Hier hast du das aktuelle R Paket, welches ich auch in diesem Kapitel teilweise vorstelle. Leider gibt es hier aber nciht sehr viel Erklärtext, daher musst du dich dann auf andere Quellen verlassen. Die Funktionen und die Anwendbarkeit ist aber sehr schön.
...
-   [Timeseries analysis in R](https://www.r-bloggers.com/2021/04/timeseries-analysis-in-r/) -- Kurz und schmerzlos ohne großartigen erklärenden Text werden hier die Funktionen einer Zeitreihenanalyse nebeneinander gesetzt. Eigentlich eher ein Glossar der Funktionen als ein Tutroium.
:::
...
Als aller erstes brauchen wir einen Datensatz, der auch eine *echte* Zeitreihe über mehrere zeitliche Zyklen enthält. Das ist dann meistens die Herausvorderung so eine Zeitreihe in einer Abschlussarbeit zu erzeugen. In ein paar Monaten einen zyklischen Verlauf zu finden ist schon eine echte Leistung. Deshalb nehmen wir hier als Beispiel einmal unsere künstlichen Daten zur Milchleistung von Kühen. Wie du in der @fig-time-milk-1 klar erkennen kannst, haben wir hier Zyklen über die einzelnen Jahre hinweg. Wir wollen jetzt modellieren und vorhersagen, wie die zukünftigen Zyklen aussehen könnten. Wenn du solche Daten nicht vorliegen hast, dann kannst du noch andere Dinge tun, aber das kommt gleich in den folgenden Abschnitten.
...
#|   - "Kreaturendaten `std_creature_df` foo"
#| layout-nrow: 1
#| column: page

p_mean <- ggplot(roll_mean_tbl, aes(index, value, color = type)) +
  theme_bw() +
  geom_point2() +
  stat_smooth(se = FALSE) +
  scale_color_okabeito() 
