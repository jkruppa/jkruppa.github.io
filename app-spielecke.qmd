...
```{r}
library(mfp)
mfp_fit <- mfp(freshmass ~ fp(k_in_shoot), data = shoot_k_tbl)
```
...
```{r}
shoot_k_nls_func <- \(x) {-12.16 * x^2 + 111.95 * x - 172.01}
```

```{r}
shoot_k_mfp_func <- \(x) {-12.16 * x^2 + 111.95 * x - 172.01}
```
...
shoot_k_tbl |> 
  ...
  geom_function(fun = shoot_k_nls_func, color = "black", size = 1,
                xlim = c(3.8, 6.8)) + 
...
#  geom_segment(aes(x = 4.6, y = shoot_k_func(4.6), xend = 4.6, yend = 20), linetype = 'dashed',
 #                 arrow = arrow(length = unit(0.3, "cm"), type = "closed")) +
  ...
  annotate("text", x = 4.7, y = 30, label = "4.6%",
           hjust = "left", size = 5) +
  annotate("segment", x = 4.6, y = shoot_k_nls_func(4.6), xend = 4.6, yend = 20, linetype = 'dashed', 
           size = 1) +
