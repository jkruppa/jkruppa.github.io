...
f1_ancova_theo_tbl |> 
  select(fa, cov, rsp) |> 
  set_names(c("Faktor A", "Kovariate", "Messwert $y$")) |>  
...
```{r}
mean(f1_ancova_theo_tbl$cov)
```

```{r}
mean(f1_ancova_theo_tbl$rsp)
```

```{r}
model.matrix(rsp ~ fa + cov, data = f1_ancova_theo_tbl)
```
...
ggplot(f1_ancova_theo_tbl, aes(cov, rsp)) +
  ...
  scale_x_continuous(breaks = c(0, 1.6, 4.0, 6.4),   
                     labels = c(0, expression(bar(c)[A.1]~"="~1.6), 
                                expression(bar(c)[A.1]~"="~4.0), 
                                expression(bar(c)[A.1]~"="~6.4)),
                     limits = c(0, 7.5)) +
