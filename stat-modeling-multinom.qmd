...
Um die Fläche rechts von dem $t$-Wert zu berechnen, können wir zwei Funktionen nutzen. Die Funktion `pnorm()` nimmt eine Normalverteilung an und die Funktion `pt()` vergleicht zu einer $t$-Verteilung. Wenn wir *rechts* von der Verteilung schauen wollen, dann müssen wir die Option `lower.tail = FALSE` wählen. Da wir auch zweiseitig testen, müssen wir den ausgrerechneten $p$-Wert mal zwei nehmen.

`pnorm(1.96, lower.tail = FALSE) * 2`

`pt(3.51, lower.tail = FALSE, df = 3) * 2`
...
coef_tbl %>% 
  mutate(p_n = pnorm(t_value, lower.tail = FALSE) * 2,
         p_n = pt(t_value, lower.tail = FALSE, df = 3) * 2) %>% 
