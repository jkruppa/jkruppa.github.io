...
pacman::p_load(tidyverse, magrittr, broom, readxl,
               infer, see, tidyplots, rstatix, conflicted)
...
Bevor wir einen statistischen Test rechnen, wollen wir uns erstmal die Daten, die dem Test zugrunde liegen, visualisieren. Klassischerweise wollen wir einen t-Test rechnen, da wirzwei Säulen in einem Barplot miteinander vergleichen wollen. Dafür schauen wir uns in @fig-barplot-ttest einmal den Barplot und den Boxplot für die Sprungweiten getrennt nach Hund und Katze an.
...
p2 <- ggplot(flea_tbl, aes(x = animal, y = jump_length, 
                     fill = animal)) +
  geom_boxplot() +
  labs(x = "Tierart", y = "Sprungweite [cm]") +
  theme_minimal() +
...
Im folgenden Datenbeispiel in @tbl-data-ttest-paired haben wir eine verbundene Stichprobe. Das heißt wir haben nicht zehn Flöhe gemessen sondern fünf Flöhe jewiels zweimal. Einmal haben wir die Sprungweite der Flöhe im ungefütterten Zustand `unfed` und einmal im gefütterten Zustand `fed` gemessen. Wir wollen nun wissen, ob der Fütterungszustand Auswirkungen auf die Sprungweite in \[cm\] hat.
...
```{r}
paired_tbl <- tibble(id = 1:5,
                     unfed = c(5.2, 4.1, 3.5, 3.2, 4.6),
                     fed = c(6.1, 5.2, 3.9, 4.1, 5.3)) 
```
...
```{r}
t.test(paired_tbl$fed, paired_tbl$unfed, paired = TRUE)
```
...
```{r}
stats_tbl <- paired_tbl |> 
  pivot_longer(cols = fed:unfed,
               values_to = "weight",
               names_to = "trt") |> 
  arrange(id) |> 
  t_test(weight ~ trt, paired = TRUE) |> 
  add_significance() |> 
  add_xy_position()
```
...
paired_tbl |> 
  pivot_longer(cols = fed:unfed,
               values_to = "weight",
               names_to = "trt") |> 
  tidyplot(x = trt, y = weight, color = trt, fill = NA) |> 
  add_line(group = id, color = "grey") |> 
  add_data_points() |>
  add_mean_dash() |> 
  add_sd_errorbar() |> 
  add(ggpubr::stat_pvalue_manual(stats_tbl, size = 7/.pt, label = "p")) |> 
  remove_legend() |> 
  adjust_font(fontsize = 9) |>
  adjust_x_axis_title("Flohart") |>
  adjust_y_axis_title("Sprungweite in [cm]") |> 
