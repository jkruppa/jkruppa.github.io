...
Was haben wir gemacht? Als erstes haben wir die Spalte `animal` aus dem Datensatz `flea_tbl` mit dem Dollarzeichen `$` *herausgezogen*. Mit dem `$` Zeichen können wir uns eine einzelne Spalte aus dem Datensatz `flea_tbl` raus ziehen. Über das `$` Symbol kannst du im @sec-dollar mehr erfahren.

![Darstellung des Datensatzes `flea_tbl` als Kleiderschrank für die Variablen `jump_length`, `flea_count` und `weight`. Es werden nur die ersten vier Beobachtungen dargestellt.](images/programing/schrank/schrank_flea_tbl.png){#fig-flea-tbl fig-align="center" width="50%"}
...
-   [Warum `tibble` besser sind als `data.frame`](https://tibble.tidyverse.org/reference/tibble.html)
