...
Dann stellt sich natürlich die Frage, wie ist denn jetzt eigentlich der Umfang der Bachelorarbeiten an der Fakultät Agrarwissenschaften und Landschaftsarchitektur (AuL)? Für die Landschaftsarchitektur ist es etwas schwieriger, da ich die Auswertung im Rahmen des Moduls "Spezielle Statistik und Versuchswesen" habe anonym durchführen lassen. Damit ist der Fokus eher auf der Pflanzenbiologie, mag aber dennoch eine Richtung weisen. Ein paar andere Fachrichtungen haben sich dann dennoch eingeschlichen. Die Angaben sind natürlich ohne Gewähr und ich habe auch nicht übermäßig darauf geachtet Doppelungen zu vermeiden. In der @fig-abschlussarbeit-bsc1 siehst du dann einmal die Anzahl der Seiten und der Referenzen in einer Auswahl an Bachelorarbeiten. Spannenderweise sind die Umfänge sehr heterogen. Daher schaue dir an, was du wo bei wem machen willst.
...
bsc_tbl |> 
  ...
  theme_minimal() +
  geom_dotplot(binaxis = "y", stackdir = "center", show.legend = FALSE, dotsize = 0.75) +
  scale_fill_okabeito() +
  labs(x = "", y = "Anzahl")
```
