...
#| fig-height: 3
#| fig-width: 7
#| fig-cap: "Visualisierung einer schwachen und starken Streuung der Messwerte von sieben Beobachtungen um den jeweiligen Mittelwert."
...
p_var_02 <- var_sim_tbl |> 
  ...
  annotate("text", x = 6.5, y = 9, hjust = "left", color = "#E69F00", size = 4,
           label = expression(frac(sum((y-y[i])^2), n-1))) +
  annotate("text", x = 6, y = 5.25, hjust = "left", color = "#56B4E9", size = 4,
           label = expression(frac(sum((y-y[i])^2), n))) +
  annotate("text", x = 7, y = 2.75, hjust = "left", color = "#009E73", size = 4,
           label = expression(frac(sum("|"~y-y[i]~"|"^2), n-1))) +
