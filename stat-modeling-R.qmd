...
1)  Wähle ein Modell aus der [Liste der Modelle mit der entsprechenden Funktion](https://parsnip.tidymodels.org/reference/index.html#models).
2)  Entscheide dich mit der Funktion `set_engine()` für einen Algorithmus (eng. *engine*) mit dem du das Modell rechnen willst.
3)  Passe das Modell mit der entsprechenden Formelschreibweise in der Funktion `fit()` an.
...
Eine detailliertere Einführung mit mehr Beispielen für die Nutzung vom [R Paket `{parsnip}`](https://parsnip.tidymodels.org/articles/Examples.html)
