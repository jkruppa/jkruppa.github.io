...
pacman::p_load(tidyverse, simstudy, simdata, modelr, 
               ggpmisc, janitor, emmeans, conflicted)
set.seed(20250820)
conflicts_prefer(ggplot2::annotate)
```
...
Dann kombinieren wir die Modellmatrix mit den Information wo der Effekt angewendet werden soll mit dem Vektor der Effekte. Wir erhalten dann unsere Messwerte in dem Vektor `y`. Hier sind natürlich pro Faktor die Werte immer gleich. Wir haben ja noch keine Streuung hinzugefügt.
...
In der folgenden Abbildung habe ich dir eimal normalverteilte Messwerte in Excel aus einem einfaktoriellen Design generiert. Zuerst legen wir den allgemeinen Intercept fest, dann noch die Effekte der einzelnen Gruppen. Am Ende brauchen wir noch den Fehler oder die Streuung mit der Funktion `NORMINV()`. Hier habe ich eine Standardabweichung von $2$ voreingestellt.
...
Da wir es hier auch wieder mit einem linearen Modell zu tun haben, können wir uns die Sachlage auch einmal mit den Koeffizienten des linearen Modells anschauen. Wir haben hier einen linearen Ansteig in unserem ersten Faktor und einen Unterschied von $3$ in unserem zweiten Faktor. Der Intercept ist dabei die Faktorkombination $A.1.B.1$ als niedrigste Levelkombination. Wir schauen uns hier nur das *Treatment coding* an.
