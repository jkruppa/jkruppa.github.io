...
## Neuronale Netze mit `neuralnet` oder `nnet`
...
```
Wenn wir das Modell haben, dann können wir uns hier ganz einfach mal das beste neuronale Netzwerk anschauen. Also die Wiederholung mit dem kleinsten Fehler. In Abbildung @fig-class-nn-02 sehen wir das Netzwerk einmal dargestellt. Die blauen Knoten stellen die Biasknoten dar. Die Zahlen an den Kanten stellen dann die Gewichte dar, die von dem jeweiligen Knoten weitergegeben werden. Die Interpretation des Netzwerks ist so schwer, es ist eben nur eine visuelle Darstellung. 
...
```{r}
neuralnet_pred <- round(neuralnet::compute(neuralnet_fit, gummi_test_tbl)$net.result)
```


```{r}
neuralnet_cm <- conf_mat(data = data.frame(.pred_class = as.factor(neuralnet_pred [,2]),
...
Wir können aber das R Paket `nnet` mit unserer bekannten Rezotumgebung nutzen und uns damit das Leben einfacher machen. 

```{r}
nnet_mod <- mlp(penalty = 0, epochs = 100) %>% 
  set_engine("nnet") %>% 
...
::: callout-note
## Warum jetzt Keras / Tensorflow?

Ja, wir haben zwar eben auch andere R Pakete genutzt, aber wir bleiben in der Welt von `parsnip` und nutzen die Algorithmen, die in dem R Paket implementiert sind. Dann sind wir in einem Workflow und müssen nicht dauernd alles anpassen. Gerne kannst du aber auch R Pakete wie `nnet` oder `neuralnet` für deine neuronalen Netze nutzen.
:::
