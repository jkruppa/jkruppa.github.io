...
Wenn wir jetzt aber eine nicht-lineare Regression rechnen wollen, dann müssen wir hier ein Polynom einfügen. Damit schreiben wir nicht mehr $x$ sondern $x^b$ und erhalten folgende, beispielhafte Formel. Warum beispielhaft? Je nach Fragestellung brauchen wir verschiedene Parameter die wir dann für den Kurvenverlauf bestimmen müssen.

$$
y \sim \beta_0 + \beta_1 \cdot x^b
$$

Dieses $b$ (eng. *power*) und die anderen Koeffizienten müssen wir jetzt irgendwie berechnen. Manchmal fällt das $\beta_1$ dann auch weg oder aber wir haben eben eine noch komplexere Art der Darstellung. Wir gehen daher hier dann mal verschiedene Lösungen für das Problem durch. Nicht alle Lösungen liefern auch eine Geradengleichung, die du dann auch aufschreiben kannst.

Und damit kommen wir auch zu dem zentralen Problem. Woher weiß ich, welche Gerade oder Kurve ich berechnen will? Eine super Hilfestellung liefert die Seite [A collection of self-starters for nonlinear regression in R](https://www.statforbiology.com/2020/stat_nls_usefulfunctions/).
...
[Prediction interval vs. confidence interval in linear regression analysis](https://stats.stackexchange.com/questions/225652/prediction-interval-vs-confidence-interval-in-linear-regression-analysis)

Die Ermittlung von Vorhersageintervallen (eng. *prediction interval*) also für zukünftige Beobachtungen ist etwas anderes als die Ermittlung von Konfidenzintervallen (eng. *confidence interval*) also für unbekannte Populationsparameter.

[Prediction intervals for Generalized Additive Models (GAMs)](https://mikl.dk/post/2019-prediction-intervals-for-gam/)
...
pred_plim_tbl %>% print(n = 5)
```

Aus der mittleren Abweichung des Medians zu der unteren 2.5% Grenze sowie zu der oberen 97.5% Grenze lässt sich leicht noch eine Konstante $c$ errechnen, die wir dann zu unserer mathematischen Formel ergänzen können. Dann hätten wir auch die mathematische Formel für die obere und untere Kurve des Prädiktionsintervalls. Im Prinzip ist das Prädiktionsintervall ja nur eine verschobene Kurve der ursprünglichen Geradengleichung.
...
pred_clim_tbl <- nlsBootPredict(nls_boot_obj, newdata = duckweeds_tbl, interval = "confidence") %>% 
  ...
  clean_names() %>% 
  mutate(duckweeds_density = duckweeds_tbl$duckweeds_density)
pred_clim_tbl %>% print(n = 5)
```

Aus der mittleren Abweichung des Medians zu der unteren 2.5% Grenze sowie zu der oberen 97.5% Grenze lässt sich leicht noch eine Konstante $c$ errechnen, die wir dann zu unserer mathematischen Formel ergänzen können. Dann hätten wir auch die mathematische Formel für die obere und untere Kurve des Konfidenzintervalls. Im Prinzip ist das Konfidenzintervall ja nur eine verschobene Kurve der ursprünglichen Geradengleichung.
...
Damit sind wir schon durch mit GAM. In `{ggplot}` musst du aber gar nicht den langen Weg gehen, wenn du nur mit GAM eine Kurve in deine Punkte zeichnen willst. Das geht sehr einfach mit der Funktion `stat_smooth()`. Du hast dann auch dort die Möglichkeit neben GAM auch andere Arten der Anpassung einer Funktion an deine Daten zu wählen.

[How to solve common problems with GAMs](https://www.r-bloggers.com/2021/03/how-to-solve-common-problems-with-gams/)
...
[GAM -- Estimate a Generalized Additive Model](https://marginaleffects.com/vignettes/gam.html)
...
[R Paket `{gratia}`](https://gavinsimpson.github.io/gratia/reference/confint.gam.html)
