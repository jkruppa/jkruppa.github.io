...
```{r}
boxplot_mult_tbl <- read_excel("data/zerforschen_boxplot_mult.xlsx", sheet = "substrate") %>% 
...
boxplot_mult_tbl <- read_excel("data/zerforschen_boxplot_mult.xlsx", sheet = "substrate") |> 
  pivot_longer(cols = fresh_weight:dry_weight,
               values_to = "rsp",
...
boxplot_mult_tbl |> 
  ...
  #scale_y_continuous(breaks = seq(60, 160, by = 20), limits = c(50, 160)) +
  stat_summary(fun.y = mean, geom = "point", shape = 23, size = 4, fill = "red") +
  facet_wrap(~ plant_measure, nrow = 1, scales = "free_y")
