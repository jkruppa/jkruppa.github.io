...
Der U-Test ist auch unter den Namen Wilcoxon-Mann-Whitney-Test, Mann-Whitney-U-Test oder Wilcoxon-Rangsummentest bekannt. Aus Gründen der Kürze werde ich hier immer von dem U-Test schreiben.
...
Da mir die Nullhypothese mit $Pr(X<Y) + \cfrac{1}{2} Pr(X = Y) = 0.5$ nicht ganz einleuchtete habe ich einmal die Sachlage visualisiert. Die Nullhypothese besagt ja, dass *es gleich wahrscheinlich ist, dass ein zufällig aus der einen Population ausgewählter Wert größer oder kleiner ist als ein zufällig ausgewählter Wert aus der anderen Population*. Das mache ich im folgenden einmal mit einem Hundefloh $d_1$ und vergleiche den Hundefloh zu einer Population von sieben Katzenflöhen.

In der oberen Abbildung siehst du, dass drei Katzenflöhe zufällig nicht so weit springen wie der Hundefloh sowie vier Katzenflöhe weiter springen. Damit liegt der Hundefloh in der Mitte der Sprungweiten der Katzenflöhe und wir hätten eine Wahrscheinlichkeit von ungefähr 0.5 das der Hundefloh kürzer oder weiter springt. Der andere Fall wäre, dass der Hundefloh an einem Ende der Sprungweiten der Katzenflöhe liegt. Oder andersherum alle Katzenflöhe springen weiter. Der Hundefloh hat eine Wahrscheinlichkeit von 0, dass ein Katzenfloh kürzer springt. Du findest in dem Tutorium [The Mann-Whitney U Test](https://www.statstutor.ac.uk/resources/uploaded/mannwhitney.pdf) nochmal ein anschauliches Beispiel.
...
#| fig-height: 3.75
#| fig-width: 7
#| fig-cap: "Vereinfachte Darstellung der Null- und Alternativehypothese in einem U-Test. Hier vergleichen wir eine Sprungweite eines Hundeflohs $d_1$ mit einer Population von sieben Katzenflöhen. Wie wahrscheinlich gehört der Hundefloh und die Katzenflöhe im Bezug auf die Sprungweiten zu einer gleichen Population? Oder unterscheiden sich die beiden Floharten? **(A)** Nullhypothese ist wahr. Die Hälfe der Katzenflöhe springt nicht so weit wie der Hundefloh. **(B)** Alternativehypothese ist wahr. Alle Katzenflöhe springen weiter als der Hundefloh. *[Zum Vergrößern anklicken]*"
...
Beginnen wir also mit unserem einfaktoriellen Datensatz. Wir haben hier die Sprungweite in \[cm\] von Hunde- und Katzenflöhen gemessen. Unser Faktor ist hierbei die Flohart. Entweder ein Hundefloh oder eben ein Katzenfloh. Wir wollen jetzt wissen, ob sich die beiden Floharten hinsichtlich ihrer Sprungweite unterscheiden.
...
Die Wahl der Nullhypothese durch die Annahmen an die Verteilungsfromen der beiden zu vergleichenden Gruppen hat natürlich einen gewaltigen Einfluss auf die Interpretation des p-Wertes. Also wenn man es statistisch genau nimmt. Deshalb hier auch einmal die prosarische Prosavariante für Einsteiger.
:::

Bitte lesen nochmal oben den Abschnitt zu dem Problem der Nullhypothese im U-Test nach. Je nachdem wie tief du jetzt hier einsteigen willst, habe ich dir mal die drei Varianten angebenen. Am Ende findest du die simpelste Prosavariante gar nicht so selten in Tutorien. Die fortgeschrittene Variante trifft es schon sehr gut. Die mathematische Formel mag dann mehr Fragen als Antworten liefern.
...
Wir vergleichen hier zwei Populationen oder Gruppen $X$ und $Y$ miteinander. Die Formel repräsentiert die fortgeschrittene Prosa in dem vorherigen Tab.
...
Leider kann der `wilxoc.test()` nicht mit Bindungen in den Daten umgehen.
