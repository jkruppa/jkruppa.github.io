...
3)  Effektschätzer, die einen *Wirkungsgrad* beschreiben.
...
**Dieser Abschnitt ist teilweise eine Baustelle und wird es vermutlich auch über das Sommersemester 2024 bleiben. Daher können Teile des Codes und des Textes kurzfristig keinen Sinn ergeben oder nicht funktional sein. Es ist geplant eine fertige Version im Juni 2024 vorliegen zu haben.**

------------------------------------------------------------------------

Neben den klassischen Effektmaßzahlen, die sich aus einem Mittelwert oder einem Anteil direkt berechnen, gibt es noch andere Effektmaße. Einer dieser Effektmaße ist der Wirkungsgrad für zum Beispiel ein Pflanzenschutzmittel. Wir können hier aber auch weiter denken und uns überlegen in wie weit wir eine Population von Schaderregern durch eine Behandlung reduzieren können. Unabdingbar ist in diesem Fall eine positive Kontrolle in der nichts gemacht wird sondern nur der normale Befall gemessen wird.

::: callout-tip
## Übersicht über Wirkungsgrade

-   Die Seite [LdP Line by Ehab Bakr](https://www.ehabsoft.com/ldpline/onlinecontrol.htm) liefert nochmal die Formeln und Berechungsmöglichkeiten für andere Wirkungsgrade mit einem praktischen Online-Rechner.
...
| Befall oder lebende Individuen     |       Abbott        |                Henderson-Tilton                 |
| Sterblichkeit oder tote Individuen |  Schneider-Orelli   |                   Sun-Shepard                   |

### Wirkungsgrad nach @henderson1955tests

$$
WG_{ht} = \left(\cfrac{T_a}{T_b}\cdot\cfrac{C_b}{C_a}\right) 
$$
...
### Wirkungsgrad nach Sun-Shepard

$$
WG_{ss} = \left(\cfrac{Trt_{mortality}^{\%} \pm \Delta_{ctrl}^{\%}}{100 \pm\Delta_{ctrl}^{\%}}\right) 
$$

und

$$
\Delta_{ctrl}^{\%} = \left(\cfrac{Ctrl_n^{after} - Ctrl_n^{before}}{Ctrl_n^{before}}\right) 
$$

mit

-   $Trt_{mortality}^{\%}$ Sterberate in der Behandlung
-   $Ctrl_n^{after}$ Anzahl lebend in der Kontrolle *nach* der Behandlung
-   $Trt_n^{before}$ Anzahl lebend *vor* der Behandlung

### Wirkungsgrad nach Schneider-Orelli

$$
WG_{so} = \left(\cfrac{Trt_{mortality}^{\%} - Ctrl_{mortality}^{\%}}{100 - Ctrl_{mortality}^{\%}}\right) 
$$

mit

-   $Trt_{mortality}^{\%}$ Sterberate in der Behandlung
-   $Ctrl_{mortality}^{\%}$ Sterberate in der Kontrolle

### Wirkungsgrad nach @abbott1925method

Wir berechnen hier den Wirkungsgrad nach @abbott1925method mit der Anpassung von @finner1989berechnung. Der Wirkungsgrad $WG$ eines Schutzmittels im Vergleich zur Kontrolle berechnet sich wie folgt.

$$
WG_{abbott} = \left(\cfrac{C_a - T_a}{C_a}\right) = \left(1-\cfrac{T_a}{C_a}\right)
$$
...
Natürlich ist die Formel wieder sehr abstrakt, deshalb haben wir zwei Beispieldaten. Zuerst schauen wir uns einen Datensatz zu dem Befall mit Trespe, einem Wildkraut, an. Wir haben also Parzellen in denen sich die Trespe ausbreitet und haben verschiedene Behandlungen durchgeführt. Wichtig hierbei, wir haben auch Parzellen wo wir nichts gemacht haben, das ist dann unsere positive Kontrolle (*ctrl*). Da unsere Daten nicht im Long-Format vorliegen müssen wir die Daten erst noch anpassen und dann die Spalte `block` in einen Faktor umwandeln.
...
read_excel("data/raubmilben_data.xlsx", sheet = "trespe") %>% 
  kable(align = "c", "pipe")
```
...
In der @fig-effect-1 sehen wir nochmal die orginalen, untransformierten Daten sowie die $log$-transformierten Daten. Wir nutzen die Funkrion `log1p()` um die Anzahl künstlich um 1 zu erhöhen, damit wir Nullen in den Zähldaten zum Logarithmieren vermeiden.
