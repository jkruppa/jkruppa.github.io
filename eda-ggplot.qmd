...
### Ein einfache Barplot

![Ursprüngliche Abbildung, die nachgebaut werden soll.](images/eda/zerforschen_barplot_simple.png){#fig-ur-barplot fig-align="center" width="80%"}
...
https://stackoverflow.com/questions/67034852/create-line-and-curve-line-user-defined-values

![Ursprüngliche Abbildung, die nachgebaut werden soll.](images/eda/zerforschen_regression.jpeg){#fig-ur-barplot fig-align="center" width="80%"}
...
```{r}
ki_func <- function(x) {-104 * x^2 + 445 * x}
kio3_func <- function(x) {-117 * x^2 + 374 * x}
```
...
ggplot(data = regression_tbl, aes(x = iodine, y = content,
                                  color = type)) +
  theme_bw() +
  geom_line(stat = 'function', fun = ki_func, color = cbbPalette[2], linetype = 'dashed') +
  geom_line(stat = 'function', fun = kio3_func, color = cbbPalette[3], linetype = 'dashed') +
