...
Als erstes müssen wir nochmal das `emm_obj` in einen `tibble` umwandeln, damit wir dann das *compact letter dispaly* direkt in die Grafik setzen können. Hier könnte man dann noch die Mediane aus den Daten berechnen um die Position etwas besser hinzukriegen. Aber es sollte so erstmal reichen.
