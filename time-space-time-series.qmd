...
```{r}
cor_erupt_mat <- cor(erupt_tbl, use = "complete.obs")
...
ggpairs(erupt_tbl, 
        lower = list(continuous = wrap(cor_func, method = 'pearson', 
                                       symbol = expression('\u03C1 ='))),
        upper = list(continuous = wrap(lower_fun)),
        columnLabels = c("Original", "Lag 1", "Lag 2", "Lag 3"),
        axisLabels = "internal") +
  theme_bw()

corrplot.mixed(cor_erupt_mat, upper = "ellipse")
...
erupt_ts %>% 
  acf(main = 'Correlogram', lag.max = 5, ylim = c(-1, 1))
