...
#death_tbl <- tibble(dol = 1:10,
#                    death = c(rep(0, 5), rpois(5, 2)),
#                    count = cumsum(c(25, -1*death[-1])),
#                    trt = "none") %>% 
  ## uncount(ifelse(death == 0, death + 1, death), .remove = FALSE) 
#  uncount(death) %>% 
#  mutate(death = c(rep(1, length(.$dol))))

#alive_tbl <- tibble(dol = max(death_tbl$dol),
#                    death = rep(0, last(death_tbl$count)),
#                    count = last(death_tbl$count),
#                    trt = "none")
...
foo <- bind_rows(death_tbl, alive_tbl)

fit <- survfit(Surv(dol, death) ~ trt,
               data = foo)
# Visualize with survminer
ggsurvplot(fit, data = foo, risk.table = TRUE,
           surv.median.line = "hv",
           ggtheme = theme_light(),
...
dodo_tbl <- dodo_raw_tbl %>% 
  mutate(death_uncount = ifelse(death == 0, 1, death)) %>% 
  uncount(death_uncount) %>% 
