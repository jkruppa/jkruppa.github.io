...
fit <- glm(log_count ~ sorte + block, data = mite_tbl, family = poisson)

mite_tbl %>% 
  filter(sorte == 8 | sorte == "ctrl") %>% 
  mutate(count = count + 1)

((5*2*1*1*2)/(47*33*63*91*21))^(1/5)

0.24/10

foo <- mite_tbl %>% 
  select(sorte, block, count) %>% 
...
mite_tbl %>% 
  ...
  mutate(GM = 1 - (prod/prod[9])^(1/5),
         a = sqrt(2/5) * 0.9621 * 1.694 + log((prod/prod[9])^(1/5)),
         KI = 100*(1 - exp(a)))
```

```{r}
lm(log_count ~ sorte + block, data = mite_tbl) %>% 
  glance() %>% 
  pull(sigma)
```
...
```{r}
qt(p = 0.05, df = 32, lower.tail = FALSE)
```

```{r}
a <- sqrt(2/5) * 0.9621 * 1.694 + log(0.04036)
a
```
