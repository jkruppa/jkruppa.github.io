# Daten lesen & schreiben {#sec-programming-import}
...
Wenn du schon Daten hast, dann macht es eventuell mehr Sinn eine **neue** Exceldatei anzulegen in der du dann die Daten in das Long-Format kopierst. Wir können aber mit einem Wide-Format nicht mit `ggplot()` die Daten aus der @tbl-imp-cat-dog-wide visualisieren. Deshalb müssen wir entweder das Wide-Format in das Long-Format umwandeln oder die Daten gleich in Excel im Long-Format erstellen.
...
Dann müssen wir ja auch manchmal Daten in Excel schreiben. Also wir wollen dann eine `xlsx`-Datei erstellen. Dafür nutze ich dann gerne die beiden folgenden R Pakete. Je nachdem, ob ich dann alles in ein Tabellenblatt schreiben möchte oder aber in mehrere. Leider sind die Funktionen sehr ähnlich. Also hier genau schauen, welche Funktion du aufrufst.

### ... in ein Tabellenblatt

Das [R Paket `{writexl}`](https://docs.ropensci.org/writexl/reference/write_xlsx.html) erlaubt es einfach eine Datei in ein Tabellenblatt zu schreiben. Das ist sehr einfach und geht super schnell.

```{r}
#| eval: false
data_tbl |> 
  write_xlsx("~/data/multiple_sheets.xlsx")
```

### ... in mehrere Tabellenblätter

Das [R Paket `{openxlsx}`](https://ycphs.github.io/openxlsx/) erlaubt es schnell und einfach eine Liste in mehrere Tabellenblätter zu schreiben. Wenn du also eine Liste vorliegen hast, dann kannst du einfach jeden Listeneintrag in einen Tab schreiben.

```{r}
#| eval: false
data_lst |> 
  write.xlsx("~/data/multiple_sheets.xlsx")
```
